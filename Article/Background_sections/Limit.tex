...
The dual of the proposition above states that a coequalizer is an epimmorphism.
...
\begin{definition}[Kernel Pair]
    A \emph{kernel pair} for an arrow $f: A \to B$ is an object $K_f$ together with two arrows $\pi_1^f, \pi_2^f : K_f \to A$, denoted as $(K_f, \pi_1^f, \pi_2^f)$, such that the following square is a pullback.
    \[
        \begin{tikzcd}
            K_f \ar[r, "{\pi_1^f}"] \ar[d, "{\pi_2^f}" swap] & A \arrow[d, "f"] \\
            A \ar[r, "f"swap] & B
        \end{tikzcd}
    \]
\end{definition}
...
\begin{proof}
    ...
    \[
        \begin{tikzcd}
        A \arrow[drr, bend left=20, "f"] \arrow[ddr, bend right=20,"g" swap] \arrow[dr, dashed, "u"] & & \\
        & M  \arrow[d, "{id_M}" swap] \arrow[r, "{id_M}"] & M \arrow[d, "m"] \\
        & M  \arrow[r, "m" swap] & X
        \end{tikzcd}
    \]
...
\begin{cor}\label{cor:kermono}
	Let $(K_f, \pi^f_i), \pi^f_2)$ be a kernel pair for $f\colon X\to Y$. Then for every mono $m\colon Y\to Z$, $(K_f, \pi^f_1, \pi^f_2)$ is a kernel pair also for $m\circ f$.
\end{cor}
\begin{proof}
...
\begin{cor}
    Let $\cat C$ be a category with pullbacks and $\phi : D \to D'$ be a natural transformation between two functors $D, D': \cat{I \to C}$. If $\phi_i$ is a regular epi for every $i$, then $\phi$ is a regular epi.
\end{cor}
