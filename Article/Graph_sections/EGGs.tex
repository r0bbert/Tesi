...
\begin{definition}[Category of E-Graphs]\label{def:cat_of_eggs}
	$\EGG$ is the full subcategory of $\EqGrph$ in which objects are e-graphs. %and, given two e-graphs $\eqgraph{G, H}$, $\EGG(\eqgraph{G, H}) = \EqGrph(\eqgraph{G, H})$.
\end{definition}
...
By the previous results, we can deduce what follows from \Cref{prop:inc_creat}.
...
\begin{lemma}\label{lem:I_pres_pushouts_of_monos}
	Let $\eqgraph{G}_1 = (A_1, B_1, C_1, s_1, t_1, q_1)$, $\eqgraph{G}_2 = (A_2, B_2, C_2, s_2, t_2, q_2)$ and $\eqgraph{G}_3 = (A_3, B_3, C_3, s_3, t_3, q_3)$ be e-graphs,  $h: \eqgraph{G}_1 \to \eqgraph{G}_2$ and $m: \eqgraph{G}_1 \to \eqgraph{G}_3$ be two morphisms with $m$ mono. Then, the pushout of $h$ and $m$ is again an e-graph.	
\end{lemma}

\begin{proof}
	Let $\eqgraph{P} = (A, B, C, s, t, q)$, together with $k: \eqgraph{G}_3 \to \eqgraph{P}$ and $n: \eqgraph{G}_2 \to \eqgraph{P}$, the pushout of $h$ and $m$.
	...
	\[
		\begin{split}
			...
			\end{split}
	\]
	By universal property of pushouts, we deduce $q \circ t \circ u_1 = q \circ t \circ u_2$, and the thesis follows.
\end{proof}
