...
\begin{example}
    It is easy to see that taking sets as objects and total \todo{Ha senso mettere totale qui?}functions as arrows, we obtain a category. In fact, given two functions $f: A \rightarrow B$ and $g: B \rightarrow C$, it is possible to compose them obtaining an arrow $g \circ f : A \rightarrow C$, and the composition is associative. For each set $A$ there exists an identity function $id_A: A \rightarrow A$ such that $id_A(a) = a$ for each $a\in A$.
...
\todo{la 1.1 è un po' povera come sezione, se sposti questa sezione sui funtori prima, poi puoi arricchire la 1.1 con la sottosezione sulle comma}
...
\begin{proof}
    ...
    Let $R$ be the right adjoint as in ~\Cref{def:right_adjoint}. Given $f\colon X \rightarrow Y$, we can define $L(f)$ as the unqiue arrow  $L(X) \rightarrow L(Y)$ whose image through $R$ fits in the diagram below.
    \[
    \begin{tikzcd}
...
The graph with equivalence $\eqgraph{G} = (G, \sim_G)$ is another representation of the graph\todo{sposta questo discorso dopo il calcolo dei limiti, così che sia appena prima della dimostrazione sugli aggiunti} $G/_{\sim_G}$. Such graph is called \emph{quotient graph}, and it can be expressed by the action of a functor over a graph with equivalence. Such functor is defined as follow.
...
\begin{prop}\label{prop:limits_in_EqGrph}
    COME SI CALCOLANO I LIMITI IN $\mathbf{EqGrph}$
    % GUESS: limiti calcolati componente per componente su E e V, e restrizioni su Q (?) come in Graph (Dipende da prefasci).
\end{prop}
\begin{proof}
    TO DO
\end{proof}
...
\begin{proof}
    ...
    \todo{stesso problema di prima. Io riscriverei anche questa metà perché non si capisce la notazione e ad un certo punto occorre usare le proprietà dei quozienti in Set, cosa che non è evidente.}
...
\begin{proof}
    ...
    \[
        ...
        \end{tikzcd}
    \]
    and let $(\mathcal P, p_{Q(f)}, p_{Q(g)})$ be the pullback. We want to show that, if  $(\eqgraph{P}, p_f, p_g)$ is the pullback of the arrows $f$ and $g$ in $\mathbf{EqGrph}$, then $Q(\eqgraph{P}) = \mathcal{P}$, $Q(p_f) = p_{Q(f)}$ and $Q(p_g) = p_{Q(g)}$. 
    \color{green}{
        Since limits are computed pointwise in $\mathbf{EqGrph}$ on $E$ and $V$ components, we have that 
        % IDEA: come si calcola un pullback in EgGrph? --> componente per componente, e poi su Q la least equiv relation such that ..., quindi applicando il funtore quoziente ottengo esattamente il pullback dei quozienti.
    }\color{black}
...
\begin{proof}
    1) --> Q preserva pushout along reg monos ()
\end{proof}
