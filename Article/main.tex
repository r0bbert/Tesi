...
\begin{obs}\label{obs:funct_creat_lim_then_lift}[~\cite{Adámek_Herrlich_Strecker_2009}]
    It holds that if a functor creates limits, then lifts uniquely limits.
\end{obs}
...
% Verificare la M-adesività utilizzando la definizione data sopra può essere particolarmente ostico. Per questa ragione, è possibile ricorrere al seguente risultato. [A new crit for MN-adh]
...
These examples show how some kind of limits are computed among graphs, but we have already said something similar about categories of presheaves (Observation~\ref{obs:limits_in_presh}). In fact, it is possible to show the equivalence between the category of graphs and $[\bullet \rightrightarrows \bullet , \Set]$. The easy way of computing depends on the fact that in a category of presheaves, limits are computed objectwise [citation needed], as we implicitly have done in the previous examples.
...
Coequalizers are computed in the same way. Computing 
The two previous examples shows that the category $\mathbf{EqGrph}$ has both equalizer and finite products, so, by Theorem~\ref{th:limit}, we can conclude that $\mathbf{EqGrph}$ has finite limits.
