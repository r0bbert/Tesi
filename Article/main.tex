...
\end{prop}
\todo{Devi mostrare che eta risulta naturale (viene in automatico ma conviene scriverlo per la dimostrazione successiva).}
\begin{proof}
...
\begin{proof}
    ...
    A right adjoint of $Q$ is the functor $T: \mathbf{Graph \rightarrow EqGrph}$ \todo{stesso problema di prima. Io riscriverei anche questa metà perché non si capisce la notazione e ad un certo punto occorre usare le proprietà dei quozienti in Set, cosa che non è evidente.} such that, for each graph $\mathcal{G}$ of $\mathbf{Graph}$, $T(\mathcal{G}) = (\mathcal{G}, \approx_{\mathcal{G}})$, where $\approx_{\mathcal{G}}$ is the total relation among vertices of $\mathcal{G}$. We have then the following situation:
    \[
        \begin{tikzcd}
...
\begin{proof}
    ...
    \[
        ...
        \end{tikzcd}
    \]
    and let $(\mathcal P, p_{Q(f)}, p_{Q(g)})$ be the pullback. We want to show that, if  $(\eqgraph{P}, p_f, p_g)$ is the pullback of the arrows $f$ and $g$ in $\mathbf{EqGrph}$, then $Q(\eqgraph{P}) = \mathcal{P}$, $Q(p_f) = p_{Q(f)}$ and $Q(p_g) = p_{Q(g)}$. \color{green}{continuare}\color{black}
    Hence, from \Cref{cor:quot_preserves_co_lim}, we can conclude that $Q$ creates pullbacks.
\end{proof}

\begin{lemma}
    The quotient functor creates pullbacks along regular monomorphism.
\end{lemma}
