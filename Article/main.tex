...
\begin{definition}[Quotient functor]
    The \emph{quotient functor} $Q: \mathbf{EqGrph \rightarrow Graph}$ is the functor that maps each graph with equivalence $\mathcal{G} = (G, \sim_G)$ to its \emph{quotient graph} $G/_{\sim_G} = (V_G/_{\sim_G}, E_G, s_G', t_G')$, with $s_G'([e]_{\sim_G}) = [v]_{\sim_G}$ if $s_G(e) = v$ and the same for $t_G'$, and each homomorphism between graphs with equivalences $h: \mathcal{G \rightarrow H}$ to $Q(h)$ defined by $Q(h_V)([v]_{\sim_G}) = [h_V(v)]_{\sim_H}$.
\end{definition}
