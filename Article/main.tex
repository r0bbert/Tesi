...
\begin{remark}\label{rem:small_cats}
    ...
    Another clarification must to be done, still considering $\Set$. Given two sets $A$ and $B$, $\Set(A, B)$ is also a set. A category $\cat C$ where, for each pair of objects $A$ and $B$, $\cat C (A, B)$ is a set is said to be \emph{locally small}.
\end{remark}
...
\begin{definition}[Monomorphism]\label{def:mono}
    An arrow $f:B\rightarrow C$ in a category $\cat{C}$ is a \emph{monomorphism} if, for any pair of arrows of $\cat{C}$ $g:A \rightarrow B$, $h: A \rightarrow B$, the equality $f \circ g = f \circ h$ implies $g = h$. The class of all monomorphisms of $\cat C$ is denoted $\Mono(\cat C)$.
\end{definition}
...
Starting from a category $\cat{C}$, it is possible to construct some other categories with interesting properties.
...
\begin{definition}[Subcategory]
    ...
    Another way to express that composites are the same (point~\ref{comp}) is to say that if $f, g \in \Hom(\cat D)$ are composable, then $g \circ g \in \Hom(\cat D)$, i.e., $\Hom(\cat D)$ is \emph{closed under composition}.
\end{definition}
...
\begin{definition}[Full functor, faithful functor, fully faithful functor]
    let $F: \cat C \rightarrow \cat D$ be a functor, and consider the induced function
    $$
        F_{X, Y} : {\cat C}(X, Y) \rightarrow {\cat D}(F(X), F(Y))
    $$
    If, for each $X$, $Y$ objects of $\cat C$, $F_{X, Y}$ is surjective, then $F$ is said to be \emph{full}, if it is injective, $F$ is said to be \emph{faithful}, if it is both injective and surjective, $F$ is said to be \emph{fully faithful}.
\end{definition}

\begin{example}
    Let $\cat C$ be a category and $\cat D$ a subcategory. The inclusion functor $I: \cat{D \rightarrow C}$, mapping each object and each arrow onto itself. $I$ is a faithful functor, because, given any pair of objects $X$ and $Y$ of $\cat D$, $I_{X, Y}$ is injective. If $\cat D$ is a full subcategory, then $I$ is fully faithful.
\end{example}
...
\begin{obs}\label{obs:terminal_are_isomorph}
    ...
    The reason why this argument works is because initial and terminal objects have the \emph{universal property} (Definition~\ref{def:univ_prop}), i.e., 
\end{obs}
...
\begin{example}
    $\Set$ have both products and coproduts. Given two sets $A$ and $B$, the categorical product is the set-theoretic cartesian product $A \times B$, together with the two projections $\pi_A$ and $\pi_B$, while the coproduct is the disjount sum $A \coprod B = \{ (x, 0) \mid x \in A\} \cup \{(y, 1) \mid y \in B \}$, together with the two canonical injections $\iota_A$ and $\iota B$, where $\iota_A(a) = (a, 0)$ and $\iota_B(b) = (b, 1)$. 
\end{example}
...
\begin{prop}\label{prop:eq_are_mono}
    Let $e: E \rightarrow A$ be the arrow that equalize $f, g : A \rightarrow B$ in a category $\cat C$. Then, $e$ is a monomorphism.
\end{prop}
...
Of all monomorphism, an intrersting subclass of them is the one that contains only the equalizers.
...
\begin{definition}[Pullback, Pushout]
        ...
        \end{tikzcd}
    \]
    We can refer to this situation also saying that \emph{$\pi_g$ is a pullback of $g$ along $f$}.
...
\begin{example}\label{ex:pb_in_set}
...
\[
    ...
    \end{tikzcd}
\]
\begin{definition}\label{def:stab_under_po_pb}[Stability under pullbacks, pushouts]
    Given a category $\cat C$, a subclass $\mathcal{A} \subseteq \Hom(\cat C)$ is said to be \emph{stable under pullbacks} if, for every square as the one in \eqref{diag:p_square}, if $n \in \mathcal{A}$, then $m \in \mathcal{A}$.
    $\mathcal A$ is said to be \emph{stable under pushouts} if, for every square as the one in \eqref{diag:p_square}, if $m \in \mathcal{A}$, then $n \in \mathcal{A}$.
\end{definition}
...
\begin{proof}
    ...
    What we want now is to show that $(L, (p_i))_{i \in \cat I}$ is the limit of $D$, namely, show that conditions given at the beginning are valid.
...
In the next sections, we will work on a special kind of diagrams with certain properties. In particular, we are interested in how a functor behave whit respect to the constructions defined so far.
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \item \emph{lifts limits} of $D$ if, given a limit $(L, l_i)_{i \in \cat I}$ for $F \circ D$, there exists a limit $(L', l_i')_{i \in \cat I}$ such that $(F(L'), F(l_i'))_{i \in \cat I} = (L, l_i)_{i \in \cat I}$.
        \item \emph{creates limits} of $D$ if $D$ has a limit and $F$ preserves and reflects limits along it.
    \end{enumerate}
    The dual notions are obtained in the obvious way, namely, substituting the words \textit{"limits"} and \textit{"cones"} with \textit{"colimits"} and \textit{"cocones"}.
\end{definition}
...
\begin{definition}(Van Kampen property)
    ...
    \begin{itemize}
        ...
        \item whenever the top and the left squares are pullbacks then the outer square is a pushout if and only of the right and the down squares are pullbacks.
    \end{itemize}
\end{definition}
...
\begin{definition}
    Let $\cat C$ be a category and $\mathcal M$ a subclass of $\Mono(\cat C)$ containing all isomorphisms, closed under composition and stable under pullbacks and pushouts (Definition~\ref{def:stab_under_po_pb}).
    Then $\cat C$ is \emph{$\mathcal M$-adhesive} if
    \begin{enumerate}
        \item it has pullbacks along arrows of $\mathcal M$;
        \item it has pushouts along arrows of $\mathcal M$;
...
This chapter is about graphs, and how it is possible to formalize them using categories in order to point out their properties from an abstract point of view. Starting from set-theoretical definition of graphs, we will give an abstraction via functor categories, in which a graph is nothing but a functor between a category to another.
...
\begin{definition}[Quotient functor]
    The functor $Q: \mathbf{EqGrph \rightarrow Graph}$, which maps each graph with equivalence $\mathcal{G} = (G, \sim_G)$ to its \emph{quotient graph} $G/_{\sim_G} = (V_G/_{\sim_G}, E_G, s_G', t_G')$, with $s_G'([e]_{\sim_G}) = [v]_{\sim_G}$ if $s_G(e) = v$ and the same for $t_G'$, and each homomorphism between graphs with equivalences $h: \mathcal{G \rightarrow H}$ to $Q(h)$ defined by $Q(h_V)([v]_{\sim_G}) = [h_V(v)]_{\sim_H}$ is called \emph{quotient functor}.
\end{definition}
