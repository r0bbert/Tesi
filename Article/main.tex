...
%     The \emph{quotient functor} $Q: \mathbf{EqGrph \rightarrow Graph}$ is the functor that maps
%     each graph with equivalence $\mathcal{G} = (G, \sim_G)$ to its \emph{quotient graph} 
%     $G/_{\sim_G} = (V_G/_{\sim_G}, E_G, s_G', t_G')$, with $s_G'([e]_{\sim_G}) = [v]_{\sim_G}$ if 
%     $s_G(e) = v$ and the same for $t_G'$, and each homomorphism between graphs with equivalences 
%     $h: \mathcal{G \rightarrow H}$ to $Q(h)$ defined by $Q(h_V)([v]_{\sim_G}) = [h_V(v)]_{\sim_H}$.
% \end{definition}

% \begin{definition}[Quotient functor]
%     The \emph{quotient functor} $Q: \mathbf{EqGrph \rightarrow Graph}$ is the functor that maps each $\mathcal{G} = (G, \sim)$ onto its quotient graph $G/_{\sim} = (V_G/_{\sim},E_G, s_G', t_G')$, such that, for each edge $e$:
%     \begin{itemize}
%         \item $s_G'([e]_{\sim}) = [v]_{\sim_G}$
%     \end{itemize}
% \end{definition}
...
The graph with equivalence $\mathcal{G} = (G, \sim_G)$ is another representation of the graph $G/_{\sim_G}$. Such graph is called \emph{quotient graph}, and it can be expressed by the action of a functor over a graph with equivalence. Such functor is defined as follow.
...
Graphs with equivalence are more complex objects. We can think of $\mathbf{EqGrph}$ as a subcategory of the category $[E \rightrightarrows V \rightarrow Q, \Set]$, in which each object $\mathcal{G}$ is a functor mapping the arrow $q: V \rightarrow Q$ onto an epimorphism (i.e., a surjective function). The component $\mathcal{G}(q)$ here is intended to be the function discussed in Remark~\ref{rem:eq_as_surj}.
...
\begin{proof}
    ...
    A good candidate to be the left adjoint is the functor $I: \mathbf{Graph \rightarrow EqGrph}$, which sends each graph $G$ to $\mathcal{G} = (G, =_G)$ (the graph in which each node is equivalent only to itself). To see that this functor is the adjoint we were looking for, we can apply the definition of left adjoint. The unit of the adjunction $\eta: Q \dot\rightarrow I$ is the one mapping $\mathcal{G} = (G, \sim_G)$ onto $(G/_{\sim_G}, =)$,  and, given a morphism $f: \mathcal{G} \rightarrow I(\mathcal{H})$ in $\mathbf{EqGrph}$ -- that is, a morphism from a graph with equivalence onto a graph with all non-equivalent nodes, we can find only one arrow $h$ in $\mathbf{Graph}$ such that $I(h) \circ \eta_{G} = f$.
    In particular, $I(\mathcal{H})$ is a graph with equivalence which maps the morphism $q:V\rightarrow Q$ onto the identity $id_V: V \rightarrow V$, hence a morphism from $\mathcal{G}$ to $\mathcal{H}$ is a natural transformation $f = (f_E, f_V, f_Q = f_V)$. Then, the arrow $I(h): I(Q(\mathcal{G})) \rightarrow I(\mathcal{H})$ is given by $I(h_V, h_E) = (f_V, f_E, f_V)$.

    A similar argument shows that the functor mapping each graph onto another wit all equivalent nodes is a right adjoint of $Q$.
\end{proof}
...
Regular monos in $\mathbf{EqGrph}$ are then natural transformations $\eta: \mathcal{G \dot\rightarrow H}$ such that $\eta_Q$ is mono (Proposition~\ref{prop:reg_mono_in_EG_are_mono_in_graph}). Since limits are computed pointwise, pullbacks preserve regular monomorphisms (Proposition~\ref{prop:monos_pres_by_pullback}). Regular monos are preserved also by pushouts. Again, since (co)limits are computed pointwise \color{red}{(?)}\color{black}, and in $\Set$ monos are preserved by pushouts (since $\Set$ is adhesive), we can conclude what follows.
...
\begin{proof}
    ...
    \todo{Generalizzare il fatto che i limiti in $\mathbf{EqGrph}$ rimangono limiti in $\mathbf{Graph}$} 
\end{proof}
