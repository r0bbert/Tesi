...
A first obvious definition of graph is given via sets and set-theoretic tools. The next definition recalls how a graph intuitively is.
...
\begin{definition}[Category of Graphs]
...
\begin{definition}[Graph with Equivalence]
    A \emph{graph with equivalence} is a pair $\mathcal{G} = (G, \sim_G)$ where $G$ is a graph and $\sim_G \subseteq V(G)\times V(G)$ is an equivalence relation over its set of nodes.
\end{definition}
