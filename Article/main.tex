...
>>>>>>> 0d40aa3 (Pushouts in Set, Full and Faithful functors)
...
\begin{remark}\label{rem:small_cats}
    ...
    Another clarification must to be done, still considering $\Set$. Given two sets $A$ and $B$, $\Set(A, B)$ is also a set.
    {\color{red}{non esattamente}}
...
\begin{obs}\label{obs:terminal_are_isomorph}
    ...
    The reason why this argument works is because initial and terminal objects have the \emph{universal property} (Definition~\ref{def:univ_prop}), i.e., {\color{red} ?????}
\end{obs}
...
=======
<<<<<<< HEAD
    $\Set$ has both products and coproduts. Given two sets $A$ and $B$, the categorical product is the set-theoretic cartesian product $A \times B$, together with the two projections $\pi_A$ and $\pi_B$, while the coproduct is the disjount sum $A + B = \{ (x, 0) \mid x \in A\} \cup \{(y, 1) \mid y \in B \}$, together with the two canonical injections $\iota_A$ and $\iota B$, where $\iota_A(a) = (a, 0)$ and $\iota_B(b) = (b, 1)$. 
=======
...
=======
<<<<<<< HEAD
\begin{definition}[Stability under pullbacks, pushouts]
    Given a category $\cat C$, a subclass $\mathcal{A} \subseteq \Hom(\cat C)$ is said to be \emph{stable under pullbacks} if, for every pullback square as the one in \eqref{diag:p_square}, if $n \in \mathcal{A}$, then $m \in \mathcal{A}$.
    $\mathcal A$ is said to be \emph{stable under pushouts} if, for every pushout square as the one in \eqref{diag:p_square}, if $m \in \mathcal{A}$, then $n \in \mathcal{A}$.
=======
...
\begin{definition}
<<<<<<< HEAD
    Let $\cat C$ be a category and $\mathcal M$ a subclass of $\Mono(\cat C)$ containing all isomorphisms, closed under composition and stable under pullbacks and pushouts (Definition~\ref{def:stab_under_pb_po}).
=======
