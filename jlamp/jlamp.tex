...
\author{Davide Castelnovo\corref{cor1}%
}
...
\affiliation{organization = {Department of Computer Science, University of Pisa},
	city = {Pisa}, 
	country = {Italy}}

\fntext[miur]{Research partially supported by the Italian MUR project PRIN
20228KXFN2 ``STENDHAL''.}
...
Besides the usual benefits of a formal presentation, which makes the 
properties of a data structure precise, describing the addition of equivalent 
...
framework, e.g. that every elementary topos is adhesive \cite{lack2006toposes}, and for the $\mathcal{M}$-adhesivity~\cite{CastelnovoGM24}.
%
...
EGGs can be concisely defined as (acyclic) term graphs equipped with an equivalence on nodes
...
possibly sharing subtrees, with an equivalence relation $\equiv$ on nodes that 
is closed under composition. In plain words and with a toy example:
...
the benefits \emph{per se} of a formal presentation, making precise the properties of the data structure. 
...
\emph{Differenza con la versione da conferenza}


\section{Facts about $\mathcal{M}$-adhesive categories and kernel pairs}\label{sec:ade}

We open the background section by fixing notation. Given a category $\X$ we do not distinguish notationally between $\X$ and its class of objects, so
``$X\in \X$'' means that $X$ is an object of $\X$. We let $\mor(\X)$, $\mon(\X)$ and $\reg(\X)$ denote the class of all arrows, monos and regular monos of $\X$, respectively.  
Given an object $X$, we  denote by $?_X$ the unique arrow from an initial object into $X$ and by $!_X$ that  unique arrow from $X$ into a terminal one. 
The notation $e\colon X\eto Y$ denotes that an arrow $e\colon X\to Y$ is a regular epi. 
...
\begin{defi}[\cite{azzi2019essence,ehrig2012,ehrig2014adhesive,heindel2009category,lack2005adhesive}]
	...
	if $\mathcal{M}$-pushouts are Van Kampen squares. We write $m\colon X \mto Y$ to denote that an arrow $m\colon X\to Y$ belongs to $\mathcal{M}$.
\end{defi}
...
\begin{prop}
	...
	\begin{enumerate}
		\item if $\Y$ is an (strict) $\mathcal{N}$-adhesive category, $L\colon \Y\to \A$ a functor preserving $\mathcal{N}$-pushouts and $R\colon \X\to \A$ 
		a functor preserving $\mathcal{M}$-pullbacks, then $\comma{L}{R}$ is (strictly) $\comma{\mathcal{N}}{\mathcal{M}}$-adhesive, where
		...
		\[\mathcal{M}/X:=\{m\in \mor(\X/X) \mid m\in \mathcal{M}\} \hspace{25pt} X/\mathcal{M}:=\{m\in \mor(X/\X) \mid m\in \mathcal{M}\}\]
...
Regular epis are particularly well-behaved with respect to their kernel pairs.
...
\begin{minipage}[l]{.83\linewidth}
	\begin{rem}\label[remark]{rem:length} It is well known that $(\mathbb{N}, \{\delta_n\}_{n\in \mathbb{N}})$ is a coproduct for $\aleph_0$ copies of $1$, where $\delta_n\colon 1\to \mathbb{N}$ is the function picking the element $n$. Thus by the first point of \Cref{prop:fact} there is a canonical isomorphism $\Delta\colon \mathcal{N}\to 1^\star$. 
	In particular, for every set $X$ the arrow $!_\X\colon X\to 1$ induces a \emph{length function} $\lgh_{X}\colon X^\star \to \mathbb{N}$, which sends a word to its length, as in the diagram aside.
	\end{rem}
\end{minipage}\hfill \begin{minipage}[r]{.15\linewidth}\noindent
\xymatrix@R=10pt{1 \ar[r]^{\id{1}} \ar[d]^{u_n}& 1 \ar[d]^{v_n}\\ \mathbb{N} \ar@{.>}[r]^{\Delta}& 1^\star} \hspace{1pt}\\
\xymatrix@R=10pt@C=22pt{\mathbb{N} \ar[r]^{\Delta} \ar@{.>}[dr]_{\lgh_{X}}& 1^\star\\ & X^\star \ar[u]_{!^\star_X}}
\end{minipage}
...
\begin{prop}[prop]\label{prop:com}
...
\Cref{prop:left} and \Cref{prop:com} allow us to deduce immediately the following.
...
and edges, respectively, such that  $h_1, h_2$ have no source and $h_3, h_4$ have a pair of nodes as soruce
	\begin{center}
        \begin{tikzpicture}
...
\begin{minipage}[l]{.75\linewidth}
	\begin{defi}
		...
		We define the \emph{hypergraph $\mathcal{G}_\Sigma$ associated with $\Sigma$} taking $O_\Sigma$ as set of hyperedges, $1$ as set of nodes and $\Delta \circ \ari_\Sigma$ as the source function and $ \Delta \coar_{\Sigma}$ as target function, where $\Delta \colon \mathbb{N}\to 1$ is the isomorphism of \Cref{rem:length}.
	\end{defi}
...
\begin{rem}It is worth mentioning that limits in $\hyp$ are not computed componentwise. By \Cref{prop:fact,prop:com,lim} we know that the two forgetful functors $U_{\hyp}, H_{\hyp}\colon \hyp \to \Set$, sending an hypergraph to, respectively, its set of nodes and that of hyperedges only jointly creates conneted limits. For instance, let $\phi\colon \mathbb{N}\to 1^\star$  $(\mathbb{N}, 1, \phi)$
\end{rem}
...
\begin{defi}Let $\catname{H}$ be the category such that
	\begin{itemize}
...
\begin{defi} Given a set $X$, we say that two words $X^\star$ are \emph{disjoint} is they do not share any letter. An hypergraph $\mathcal{G}=(E_{\mathcal{G}}, V_{\mathcal{G}}, s_{\mathcal{G}}, t_{\mathcal{G}})$ is \emph{separated} if:
	\begin{enumerate}
...
\end{prop}
\begin{proof}
Let $(h_1, h_2)\colon \mathcal{G}\to \mathcal{H}$ be a morphism of hypergraph. For every $a$
...
Now, let $\mathcal{H}=(E_{\mathcal{H}}, V_{\mathcal{H}}, s_{\mathcal{H}}, t_{\mathcal{H}})$ be another separated hypergraph with an arrow $(h_1, h_2)\colon S(\mathcal{H})\to \mathcal{G}$.  We can define
\begin{align*}
\hat{h}\colon V_{\mathcal{H}}&\to V_{\sp(\mathcal{G})} \\
v & \mapsto \begin{cases}
	\iota_{is}(h_2(v)) & h_2(v)\in \is{(V_{\mathcal{G}})}\\
	\iota_e(h_2(v)) & h_2(v) \text{ is a letter of } s_{\mathcal{G}}(e) \text{ for some } e\in E_{\mathcal{G}}\\
		\iota_e(h_2(v)) & h_2(v) \text{ is a letter of } t_{\mathcal{G}}(e) \text{ for some } e\in E_{\mathcal{G}}\\
\end{cases}
\end{align*}

We first have to show that the previous definition gives us a well-defined function.  By definition of isolated node we know that the three cases are exhaustive. Now, suppose that there are $e_1$ and $e_2$, not necessarily distinct, in $E_\mathcal{G}$ such that $h_2(v)$ is a letter of both 


\noindent
\begin{minipage}[l]{.75\linewidth}
\setlength{\parindent}{1.5em}
\indent 	Since $\mathcal{H}$ is a separated hypergraph, $\hat{h}$
	is a well-defined function. Moreover,  by construction $(h_1, \hat{h})$ defines a morphism $\mathcal{H}\to V_{\sp(\mathcal{G})}$ fitting in the diagram aside.
\end{minipage} \hfill 
\begin{minipage}[r]{.25\linewidth}
	\xymatrix@C=40pt@R=10pt{S(\mathcal{H}) \ar[dr]^{(h_1, h_2)} \ar[d]_{S(h_1, \hat{h})}\\ S(\sp(\mathcal{G}))  \ar[r]_-{(\id{E_{\mathcal{G}}}, f_{\mathcal{G}})}& \mathcal{G}}
\end{minipage}

To prove uniqueness of such an arrow, let $(k_1, k_2)$ be a morphism $(\mathcal{H})\to \sp(\mathcal{G})$ such that 
\[(h_1, h_2)=(\id{E_{\mathcal{G}}}, f)\circ (k_1, k_2)\]
Clearly this implies $k_1 = h_2$.   Let now $v$ be an element of $V_\mathcal{H}$. Then we have three cases.
\begin{itemize}
	\item $v\in \is{(V_{\mathcal{H}})}$. In this case we have:
\begin{align*}
	j(h_2(v))&=f(\iota_{is}(h_2(v)))\\&=f(\hat{h}(v))\\&=h_2(v)\\&=
\end{align*}
	\item 
\end{itemize}

The previous three points allow us to conclude.
\end{proof} 
...
\begin{rem}Notice that  $\lmo$ is not complete: for instance it lacks a terminal object. The reasoning is similar to that of \Cref{ex:noterm}: if $\mathcal{T}$ is terminal, since  $\mathcal{H}:=(\emptyset, 1, ?_{1^\star}, 1_{1^\star} )$ is left-monogamous, we have that $V_{\mathcal{T}}$ must be a singleton. We have two possibilities:
