...
It is possible to show that hypergraphs corresponds to presheaves over $\catname{H}$ (\cite{bonchi2022string,castelnovo2023thesis,CastelnovoGM24}). In \Cref{proof:equi} we report a proof of this claim for the interested reader.
...
\xymatrix@R=10pt@C=40pt{F(k,l) \ar@{>->}[d]_{\iota^F_{k,l}}\ar[r]_{s^F_{k,l}} & F(\bullet)^k \ar@{>->}[d]^{v^F_k} \\ E_F \ar@{.>}[r]^{s^F} & F(\bullet)^\star}
\xymatrix@R=10pt@C=40pt{F(k,l) \ar[r]_{t^F_{k,l}} \ar@{>->}[d]_{\iota^F_{k,l}} & F(\bullet)^l\ar@{>->}[d]^{v^F_l}  \\ E_F \ar@{.>}[r]^{t^F} & F(\bullet)^\star}
\end{minipage}
