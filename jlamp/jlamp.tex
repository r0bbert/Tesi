...
\begin{rem} The equivalence relation on the  nodes of the labelling hypergraph put constraints on the possible relations on the labelled hypergraphs. Indeed, consider the following hypergraphs with equivalences $\mathcal{G}$ and $\mathcal{H}$.
...
\[ f(q_{\mathcal{H}}(w))=v \neq w =q_{\mathcal{G}}(w)=q_\mathcal{G}(\id{V_{\mathcal{G}}}(w)) \qquad  g(q_{\mathcal{H}}(v))=w \neq v =q_{\mathcal{G}}(v)=q_\mathcal{G}(\id{V_{\mathcal{G}}}(v))  \]
	...
	Intuitively, labelling with $\mathcal{G}$ force us to keep all the sorts (i.e.~the labellings of the nodes) distinguished, while labelling with $\mathcal{H}$ allows us to merge different sorts.
\end{rem}
...
\begin{defi}Let $\mathcal{G}$ be an hypergraph and $(\mathcal{H},(f,g))$ a labelled hypergraph with equivalence.  We say that $(\mathcal{H},(f,g))$ is a \emph{left-monogamous} (resp.~\emph{separated, pruned}) $\mathcal{G}$-hypergraph if $\mathcal{H}$ is left-monogamous (resp.~separated, pruned).
 ...
 In particular, the previous  proposition, together with Lemmas \ref{lem:equ}, \ref{lem:coequ}, \ref{colim} and \Cref{lim}, yields the following
...
We begin by providing an equivalent characterization to labelled e-hypergraphs (see \Cref{proof:fh} for details).

\begin{restatable}{lem}{fh}\label{lem:fh}
Let $(a,b,c)\colon \mathcal{H}\to \mathcal{G}$ be an object of $\hyp_{\eq, \mathcal{G}}$ and let $(A, \pi^A_1, \pi^A_2)$, $(B, \pi^B_1, \pi^B_2)$ be kernel pairs for, respectively, $(q^\star\circ s_{\mathcal{H}}, a),(q^\star\circ t_{\mathcal{H}}, a)\colon E_{\mathcal{H}}\to Q_{\mathcal{H}}\times E_{\mathcal{G}}$. Then the following are equivalent:
\begin{enumerate}
...
Putting together \Cref{cor:eti} with \Cref{prop:varie} we get the following.
