...
\noindent
\begin{minipage}[l]{.6\linewidth}
	\begin{exa}The hypergraphs $\mathcal{G}$ and $\mathcal{H}$ below are examples of non left-monogamous hypergraph.  In the first the left-monogamy fails because $\outd{\mathcal{G}}(a, e) = 2$, while in the second we have \[\outd{H}(u, e_1) + \outd{H}(u, e_2)=1+1 = 2\]
	\end{exa}
\end{minipage} \hfill 
\begin{minipage}[r]{.35\linewidth}
	\begin{tikzpicture}
...
\begin{defi}
A morphism $(f,g)\colon \mathcal{G}\to \mathcal{H}$ of hypergraphs  is \emph{downward-closed} if  whenever $t_{\mathcal{H}}(h)$ has a letter belonging to the image of $g$, then $h$ is in the image of $f$. 
\end{defi}
...
Being downward-closed allows us to guarantee the existence of some pushouts in $\lmo$, as shown by the following proposition, whose proof is postponed to \Cref{proof:pusho}. 
...
\begin{prop}\label[proposition]{prop:mono} Let $\Sigma$ be an algebraic signature. Then the following facts hold
	\begin{enumerate}
...
\end{minipage}
\hfill 
\begin{minipage}[r]{.38\linewidth}		
\xymatrix@C=10pt@R=6pt{&K_a\ar[dd]|\hole_(.65){\pi^1_a}\ar[rr]^{k_{f'}} \ar@{>->}[dl]_(.55){k_{m'}} && K_b\ar[dd]^{\pi^1_b} \ar@{>->}[dl]_(.55){k_{n'}} \\ K_c  \ar[dd]_{\pi^1_c}\ar[rr]^(.7){k_{g'}} & & K_d \ar[dd]_(.3){\pi^1_d}\\&A'\ar[rr]|\hole^(.65){f'} \ar@{>->}[dl]^(.4){m'} && B' \ar@{>->}[dl]^{n'}  \\C' \ar[rr]_{g'} & & D' }
\end{minipage}
...
\hfill 
\begin{minipage}[r]{.23\linewidth}
\xymatrix@R=15pt{A \ar@{>->}[r]^{m}  \ar[d]_{g}& B \ar[d]^{h}\\ C \ar@{>->}[r]_{n} & D}
\end{minipage}
\begin{proof}
