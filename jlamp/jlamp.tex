...
\Cref{lem:equi} not only entails that $\hyp$ is complete, but it provides us with a recipe to compute limits, even if they are not connected.
	\begin{cor}\label{lem:prod}
		Let $F\colon \D\to \hyp $ be a  discrete diagram and let $F(D)$ be $(E_D, V_D, s_D,  t_D)$. Suppose that $(V, \{\pi^D_V\}_{D\in \D})$ is the product of $\{V_D\}_{D\in \D}$. For every $D\in \D$ and $(k,l)\in \mathbb{N}\times \mathbb{N}$, define
...
We can use \Cref{prop:pusho} to completely characterize regular monos in $\lmo$.

\begin{restatable}{lem}{regmono}\label{lem:regmon}
	The following are equivalent for a morphism $(f,g)\colon \mathcal{G}\to \mathcal{H}$ in $\lmo$:
	\begin{enumerate}
		\item $(f,g)$ is a regular mono;
		\item $(f,g)$ is a mono and sends input nodes to input nodes;
		\item $(f,g)$ is a mono and for every $h\in E_{\mathcal{H}}$, if $t_{\mathcal{H}}(h)$ has a letter belonging to the image of $g$, then $h$ is in the image of $f$. 
	\end{enumerate}
\end{restatable}

Putting together \Cref{thm:slice-functors} with \Cref{prop:pusho}  and \Cref{lem:regmon}, we get the following.
...
\begin{proof}\label{proof:count}
	...
	Given an hyperedge $e$, if $t_{\mathcal{G}}(e)$ has a repeated letter, then there are indexes $1\leq i \leq \lgh_{V_{\mathcal{G}}}(t_{\mathcal{L}(e)})$, $1\leq k \leq \lgh_{V_{\mathcal{G}}}(t_{\mathcal{L}(e')}) $ such that $(t_{\mathcal{G}}(e))_i=(t_{\mathcal{G}}(e) )_k$ and so
...
\xymatrix@R=10pt@C=30pt{  E^i_{k,l} \ar@{>->}[r]^{\iota^i_{k,l}} & E_{\mathcal{G}_i} \ar[r]^{t_{\mathcal{G}_i}} & V^\star_{\mathcal{G}_i} & E_{k,l} \ar[r]^{\pi^i_{k,l}} \ar@{>->}[dd]_{\iota_{k,l}} & E^{i}_{k,l} \ar@{>->}[dd]_{\iota^i_{k,l}}\\ E_{k,l} \ar@{>->}[d]_{\iota_{k,l}} \ar[u]^{\pi^i_{k,l}}  & & \\   E_{\mathcal{P}}   \ar@{>}[rr]_{t_\mathcal{P}}& &V^\star \ar[uu]^{(\pi^V_{\mathcal{G}_i})^\star} & E_{\mathcal{P}} \ar@{>}[r]_{\pi^E_{\mathcal{G}_i}} & E_{\mathcal{G}_i}}
	 \end{minipage}
	
	
	Moreover, we also know that $(E_{\mathcal{P}}, \iota_{}$
\end{proof}
