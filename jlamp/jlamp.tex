...
\begin{minipage}[r]{.83\linewidth}
	\begin{rem}\label{rem:len} It is worth noticing that for $X$ a set and every $n\in \mathbb{N}$, $X^n$ is the preimage of $n$ along $\lgh_{X}$. This amount to say that the square aside is a pullback. This follows at once from the fact that $\Set$ is \emph{extensive} (see \cite{carboni1993introduction} and also \Cref{word:ext}). 
	\end{rem}
...
\subsubsection{Two  subcategories of $\hyp$}\label{sub:sub}

We are now going to introduce two classes of hypergraphs, and the relative subcategories, which will provide the base for our definitions of EGGs  (cfr.~\Cref{sec:sepegg,sec:lmegg}). 
...
\begin{rem}\label{rem:con}Notice that  $\lmo$ is not complete: it lacks e.g. a terminal object. To see this, notice that the $\mathcal{H}:=(\emptyset, 1, ?_{1^\star}, 1_{1^\star} )$ is left-monogamous and that 
...
We can now introduce the second  subcategory of $\hyp$ in which we are interested in and that will be used in \Cref{sec:sepegg} to give us the underlying hypergraphs for EGGs with minimally identified tokens.
...
\begin{rem} Let $\mathcal{G}$ be an hypergraph. By \Cref{rem:elem}, right monogamy amount to the request that for every vertex $v\in V_{\mathcal{G}}$ there exists at most one $e_V\in E_{\mathcal{G}}$ such that $\ind{G}(v, e) \neq 0$ and  for such hyperedge we have $\ind{G}(v,e)=1$.
 ...
 \begin{rem}\label{rem:elem2}
 	...
 	applying \Cref{rem:elem} we this is equivalent to say that, for every hyperedge $e$:
 	\begin{itemize}
...
We define the \emph{separated graph associated to $\mathcal{G}$}  as $\sp(\mathcal{G}):= (E_{\mathcal{G}}, V_{\sp(\mathcal{G})}, s_{\sp(\mathcal{G})}, t_{\sp(\mathcal{G})})$.
\end{defi}
\begin{rem}\label{rem:ob}
 ...
 \begin{itemize}
 	...
 	\item a letter of $s_{\mathcal{G}}(e)$ are all contained in the image of $\iota_{e}\circ \iota^e_i$  for $i\leq n_e$, while every letter of $t_{\mathcal{G}}(e)$ lies in the image of $\iota_{e}\circ \iota^e_j$  for $n_e < j$, guaranteeing disjointness;
 	\item if $e$ is different from $e'$, then all the letters of $s_{\mathcal{G}}(e)$ and $t_{\mathcal{G}}(e)$ are contained in the image of $\iota_e$, while those of  $s_{\mathcal{G}}(e)$ and $t_{\mathcal{G}}(e)$ are in the image of $\iota_{e'}$ and so, again, we get the wanted disjointness. 
 \end{itemize}
\end{rem}
...
\begin{minipage}[r]{.27\linewidth}
	\begin{tikzpicture}
		...
		\begin{pgfonlayer}{edgelayer} 
			\draw[style=none,out=180, in= 270] (v) to (timesin.center);
			\draw[style=none,out=0, in= 270] (v) to (divin.center);
			\draw[style=none, in=135, out=345] (timesout.center) to (v);
			\draw[style=none, in=45, out=195] (divout.center) to (v);
...
\smallskip
If we restrict our attention to separated hypergraph without isolated nodes, then $\sp$ becomes a right adjoint. The interested reader can consult \Cref{proof:dx} for a proof.

\begin{defi} Let $\mathcal{G}$ be an hypergraph. A node $v\in V_{\mathcal{G}}$ is \emph{isolated} if for every $e\in E_{\mathcal{G}}$ $v$ does not occur as a letter in neither $s_{\mathcal{G}}$ nor in $t_{\mathcal{G}}$. $\mathcal{G}$ is \emph{pruned} if it does not have isolated nodes. We denote by $\phyp$ and $\pshyp$ the full subcategories of $\hyp$ given by, respectively, pruned hypergraphs and those which are pruned and separated.
\end{defi}

\begin{rem}\label{rem:prun}
	By construction, $\sp{G}$ is pruned for every hypergraph $\mathcal{G}$.
\end{rem}

\begin{restatable}{prop}{dx}\label{prop:dx}
	The inclusion functor $P\colon \pshyp\to \phyp$ has a right adjoint $\sp_P$.
\end{restatable}
...
Our interest for hypergraphs stems from their use as a graphical representation of algebraic terms. As  noted in \Cref{sub:hyp_alg}, an hypergraph can be thought as a colored monoidal signature. Thus labelling correspnds to the operationn of taking the slice category over a suitable hypergraph. 

\begin{defi}
	Given an hypergraph $\mathcal{G}$, the category $\hyp_\mathcal{G}$ of hypergraphs \emph{labelled by $\mathcal{G}$}  or of \emph{$\mathcal{G}$-hypergraphs} is $\hyp/\mathcal{G}$.
\end{defi}


\Cref{cor:mono} and \Cref{thm:slice-functors} give us immediately an adhesivity result for $\hyp_{\mathcal{G}}$ and a characterisation of monos in it.
...
As we have done in \Cref{sub:sub}, we can restrict our attention to two subcategories of labelled hypergraphs.

\begin{defi}
	Let $\mathcal{G}$ be an hypergraph and $(h,k)\colon \mathcal{H}\to \mathcal{G}$  a $\mathcal{G}$-hypergraph. We say that $(h,k)$ is a \emph{left-monogamous} (resp.~\emph{separated}) $\mathcal{G}$-hypergraph if $\mathcal{H}$ is left-monogamous (resp.~separated).
	
	We will denote by $\lmo_{\mathcal{G}}$ and $\shyp_{\mathcal{G}}$  the corresponding subcategories of $\hyp_{\mathcal{G}}$.
\end{defi}

We can easily realize $\lmo_{\mathcal{G}}$ and $\shyp_{\mathcal{G}}$ as comma categories.

\begin{prop}
Let $M\colon \lmo \to \hyp$ and $S\colon \shyp \to \hyp$ be the inclusion functors. Let, moreover, $\Delta_{\mathcal{G}}\colon 1\to \hyp$ be the constant functor in $\mathcal{G}$. Then:
\begin{enumerate}
	\item $\lmo_{\mathcal{G}}$ is equivalent to $\comma{M}{\Delta_\mathcal{G}}$;
	\item $\shyp_{\mathcal{G}}$ is equivalent to $\comma{S}{\Delta_\mathcal{G}}$; 
\end{enumerate}
\end{prop}

\Cref{lim} allows us to easily characterize regular monos in $\hyp_{\mathcal{G}}$.
\begin{prop}
...
In turn, by \Cref{thm:slice-functors}, Corollaries \ref{cor:eq}, \ref{cor:pusho} and \Cref{lem:regmon} we get the following.

\begin{thm} Given an hypergraph $\mathcal{G}$, the following hold true:
	\begin{enumerate}
		\item an arrow $(f,g)$  if and only if is so in $\lmo$.
		\item $\lmo_{\mathcal{G}}$ is quasiadhesive.
	\end{enumerate}
\end{thm}

\begin{rem}\label{rem:tg}
	Let $\Sigma$ be an algebraic signature. Then we know that every hypergraph labelled by $\Sigma$ must be algebraic. By \Cref{rem:elem} this means that an hypergraph $(h,k)\colon \mathcal{H}\to \mathcal{G}^\Sigma$ labelled by $\Sigma$ is left-monogamous if and only if $t_{\mathcal{H}}$ is injective. This is precisely the definition of (cyclic) \emph{term graphs} in \cite{Plu:TGR-ENTCS,CorradiniG05,CastelnovoGM24}. 
\end{rem}
...
\begin{prop}\label{prop:lim}Consider the forgetful functor $V_{\Sigma}\colon \EqHyp_\Sigma\to \EqHyp$. Then the following facts hold
	\begin{enumerate}
...
\begin{cor}\label{prop:monos_in_eqhyps} Let $h=(h_{E}, h_V, h_Q)$ be an arrow in $\EqHyp_\Sigma$. Then the following facts hold
	\begin{enumerate}
...
\hfill 
\begin{minipage}[r]{.30\linewidth}		
\xymatrix@C=10pt@R=2pt{&K_a\ar[dd]|\hole_(.7){\pi^1_a}\ar[rr]^{k_{f'}} \ar@{>->}[dl]_(.55){k_{m'}} && K_b\ar[dd]^{\pi^1_b} \ar@{>->}[dl]_(.55){k_{n'}} \\ K_c  \ar[dd]_{\pi^1_c}\ar[rr]^(.7){k_{g'}} & & K_d \ar[dd]_(.3){\pi^1_d}\\&A'\ar[rr]|\hole^(.65){f'} \ar@{>->}[dl]^(.4){m'} && B' \ar@{>->}[dl]^{n'}  \\C' \ar[rr]_{g'} & & D' }
\end{minipage}
...
\end{itemize}
 ...
 Suppose now that there are $e_1, e_2\in E_{\mathcal{P}}$ such that $t_{\mathcal{P}}(e_1)_n=t_{\mathcal{P}}(e_2)_m$ for some indexes $1\leq n \leq \lgh_{V_{\mathcal{P}}}(e_1)$ and $1\leq m \leq \lgh_{V_{\mathcal{P}}}(e_2)$. To conclude, we have to show that $e_1=e_2$. We can again use \Cref{lem:po_set} to split the cases.
...
\[\iota_V\circ \psi = g \qquad \iota_E\circ \phi = f\] 
		\end{minipage}\hfill 	 \begin{minipage}[r]{.3\linewidth}
	 	\xymatrix@R=8pt{E_{\mathcal{G}} \ar@/^.3cm/@{>->}[rr]^{f} \ar[d]_{s_{\mathcal{G}}}\ar@{.>}[r]_-{\phi}&  E_{h_1, h_2} \ar@{.>}[d]_{s_{\mathcal{E}}} \ar@{>->}[r]_-{\iota_E} & E_{\mathcal{H}} \ar[d]_{s_{\mathcal{H}}}\\V^\star_{\mathcal{G}} \ar@/_.3cm/@{>->}[rr]_{g^\star} \ar@{.>}[r]^-{\psi^\star}& V^\star_{k_1, k_2} \ar@{>->}[r]^-{\iota^\star_V}& V^\star_{\mathcal{H}}}
	 	\xymatrix@R=8pt{E_{\mathcal{G}} \ar@/^.3cm/@{>->}[rr]^{f} \ar[d]_{t_{\mathcal{G}}}\ar@{.>}[r]_-{\phi}&  E_{h_1, h_2} \ar@{.>}[d]_{t_{\mathcal{E}}} \ar@{>->}[r]_-{\iota_E} & E_{\mathcal{H}} \ar[d]_{t_{\mathcal{H}}}\\V^\star_{\mathcal{G}}\ar@/_.3cm/@{>->}[rr]_{g^\star} \ar@{.>}[r]^-{\psi^\star}& V^\star_{k_1, k_2} \ar@{>->}[r]^-{\iota^\star_V}& V^\star_{\mathcal{H}}}
	 \end{minipage}
	...
	By \Cref{prop:com} and \Cref{lim} this entails that $(\mathcal{G}, (f,g))$ is the equalizer in $\hyp$ of $(p_1, p_2)$ and $(q_1, q_2)$. By \Cref{prop:pusho}, $\mathcal{P}$ is left-monogamous, thus by \Cref{cor:eq} $(f,g)$ is regular mono as wanted.
\end{proof} 
...
\begin{proof}\label{proof:sep}
	...
	the universal property of coproducts, there are arrows $f_e\colon V_e\to V_{h_1(e)}$ fitting in the middle of the diagram below. These, in turn, yield the dotted $\hat{h}_1\colon V_{\sp(\mathcal{G})} \to V_{\sp(\mathcal{G})}$ on the bottom.
	...
	which entails preservation of composition.
\end{proof} 
...
and so $(\id{E_{\mathcal{G}}}, \epsilon_{G})\circ P(f, \hat{g})=(f,g)$.
	...
	Let now $(h,k)$ be another morphism $\mathcal{H}\to \sp(\mathcal{G})$ such that $(\id{E_{\mathcal{G}}}, \epsilon_{G})\circ P(h, k)=(f,g)$. This entails immediately that $h =f$.  Let $v$ be a node $V_{\mathcal{H}}$, since $\mathcal{H}$ is pruned, we have two cases:
	\begin{itemize}
		\item $v$ occurs in $s_{\mathcal{H}}(e)$ for some $e\in E_{\mathcal{H}}$. Let $1\leq i\leq \lgh_{V_{\mathcal{H}}}(s_{\mathcal{G}}(e))$ be such that $v=s_{\mathcal{G}}(e)_i$, then we have:
		\[\hat{g}(v)=\hat{g}(s_{\mathcal{H}}(e)_i)=(g^\star(s_{\mathcal{H}}(e)))_i=s_{\sp(\mathcal{G})}(f(e))_i=(k^\star(s_{\mathcal{H}}(e)))_i=k(s_{\mathcal{H}}(e)_i)=k(v)\]
		...
		\[\hat{g}(v)=\hat{g}(t_{\mathcal{H}}(e)_i)=(g^\star(t_{\mathcal{H}}(e)))_i=t_{\sp(\mathcal{G})}(f(e))_i=(k^\star(t_{\mathcal{H}}(e)))_i=k(t_{\mathcal{H}}(e)_i)=k(v)\]
	\end{itemize}
