...
$\EqHyps$ is equivalent to $\EqHyp \downarrow R(\mathcal{G})$.
\end{prop}
...
\begin{prop}\label{prop:varie}
	...
	\begin{enumerate}
		...
		\item there exists a functor $W_\mathcal{G}\colon \eggl\to \egg$ forgetting the labeling which creates all colimits, pullbacks and equalizers. 
	\end{enumerate}
\end{prop}
...
\begin{cor}
	$\lelm$ is $\epbl_{\mathcal{G}}$-adhesive, while $\leps$ is $\epbs_{\mathcal{G}}$-adhesive.
\end{cor}
...
\begin{proof}\label{proof:epo}
	...
	\begin{minipage}[l]{.7\linewidth}
			\setlength{\parindent}{1.5em}
		Consider now the cube on the right. By hypothesis and \Cref{rem:kleene} its left face is a pullback, moreover its bottom face, while not a pushout it is still a pullback by the first point of \Cref{prop:regmono} and the third one of \Cref{prop:fact}.
	\end{minipage}\hfill 
