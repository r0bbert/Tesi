...
Intuitively, the e-term graph obtained after the application of the rule represents a recursive term such 
as $rec_x. 1 \times x$, thus highlighting the difficulty of extracting a term representation out of an e-term graph.
...
prove the same property for e-hyper-graphs and e-term graphs , i.e. where the equivalence satisfies 
...
as they hold for DPO graph transformation.  Moreover, our construction is technically simpler, which not only facilitates its practical application and theoretical analysis, but also aligns with the traditional methodology of endowing certain (hyper)graphs with an additional \emph{structure}.
