...
\begin{defi}
	Let $\mathcal{G}$ be an hypergraph with equivalence. The category $\EqHyps$ of hypergraph with equivalence \emph{labelled by $\mathcal{G}$}  is $\EqHyp/\mathcal{G}$.
\end{defi}
...
\begin{minipage}[l]{.8\linewidth}
	\setlength{\parindent}{1.5em}
\begin{exa}
	Let $\Sigma$ be a single typed monoidal signature (cfr~\Cref{def:alg}). Since $\mathcal{G}_{\Sigma}$ has a single node, then to label another hypergraph $\mathcal{H}$ it is enough to provide a function $l\colon E_{\mathcal{H}}\to O_{\Sigma}$ such that the diagrams aside commute.
...
\begin{defi}Let $\mathcal{G}$ be an hypergraph and $(\mathcal{H},(f,g))$ a labelled hypergraph with equivalence.  We say that $(\mathcal{H},(f,g))$ is a \emph{left-monogamous} (resp.~\emph{separated, pruned}) $\mathcal{G}$-hypergraph with equivalence if $\mathcal{H}$ is left-monogamous (resp.~separated, pruned).
...
Let $\mathcal{G} = (E, V, Q, s, t, q)$ be an hypergraph with equivalence and $(A, \pi^A_1, \pi^A_2)$, $(B, \pi^B_1, \pi^B_2)$ kernel pairs for, respectively, $q^\star\circ s$ and $q^\star\circ t$. Then the following facts are equivalent
\begin{enumerate}
...
We want now to show that pullbacks and pushout in $\eggl$ are computed as in $\hyp_{\eq, \mathcal{G}}$. See \Cref{proof:varie} for a proof.
...
\begin{proof}\label{proof:epo} To fix the notation, let $\mathcal{P} $ be $(E, V, Q, s, t, q)$ and, for $i\in \{1, 2, 3\}$ let $\mathcal{G}_i$ be $(E_{i}, V_i, Q_i, s_i, t_i, q_i)$.  Let, moreover, $(K_i, \pi_i^1, \pi_i^2)$
	...
	\begin{minipage}[l]{.7\linewidth}
			\setlength{\parindent}{1.5em}
		Consider now the cube on the right. By hypothesis and \Cref{rem:kleene} its left face is a pullback. Its bottom face is not a pushout but it is still a pullback by the first point of \Cref{prop:regmono} and the third one of \Cref{prop:fact}.
