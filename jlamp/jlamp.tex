...
\begin{rem}\label{rem:inv}
	contenuto...
\end{rem}

In particular, \Cref{lem:equi} entails the following result. 

\begin{prop}\label{prop:cocomp}
	$\hyp$ has all limits and colimits.
\end{prop}

It is worth mentioning that limits in $\hyp$ which are not connected  are not computed componentwise. The following lemma shows how to compute products (see \Cref{proof:produ} for the proof).
...
\begin{proof}\label{proof:equi}
	...
	By construction $f_{k,l}= \iota^G_{k,l} \circ \eta_{(k,l)}$ and so $(\hat{\eta}, \eta_{\bullet})=(f,g)$ as wanted. 
		...
		\begin{minipage}[r]{.25\linewidth} \vspace{-.1cm}
			\xymatrix@C=50pt@R=22pt{F_{\mathcal{G}}(k,l) \ar[d]_{!_{G(k,l)}} \ar@{>->}[r]^{\iota^G_{k,l}}& E_G \ar[d]_{(\lgh_{G(\bullet)}\circ s_{G}, \lgh_{G(\bullet)}\circ t_{G})} \\ 1\ar@{>->}[r]_{(\delta_{k}, \delta_{l})}&\mathbb{N}\times \mathbb{N}}
		\end{minipage}

Moreover, by definition of $F_{\mathcal{G}}(k,l)$, the solid part of the diagrams commute below and so \Cref{rem:len} gives us the dotted arrows $s_{k,l}^{\mathcal{G}}\colon F_{\mathcal{G}}(k,l)\to V^k_{\mathcal{G}}$. 
\[	\xymatrix@C=35pt{F_{\mathcal{G}}(k,l) \ar@/_.3cm/[ddr]_{!_{F_\mathcal{G}(k,l)}} \ar@{>->}[r]^-{\iota^{\mathcal{G}}_{k,l}} \ar@{.>}[dr]^{s_{k,l}^{\mathcal{G}}}& E_{\mathcal{G}} \ar[dr]^{s_\mathcal{G}}& &  F_{\mathcal{G}}(k,l)\ar@/_.3cm/[ddr]_{!_{F_\mathcal{G}(k,l)}} \ar@{.>}[dr]^{t_{k,l}^{\mathcal{G}}} \ar@{>->}[r]^-{\iota^{\mathcal{G}}_{k,l}} & E_{\mathcal{G}} \ar[dr]^{t_\mathcal{G}}  \\ & V^k_{\mathcal{G}} \ar[d]_{!_{V^k_{\mathcal{G}}}} \ar@{>->}[r]^{v_k^{F_\mathcal{G}}}& V^\star_{\mathcal{G}}  \ar[d]^{\lgh_{V_{\mathcal{G}}}}& &V^l_{\mathcal{G}} \ar[d]_{!_{V^l_{\mathcal{G}}}} \ar@{>->}[r]^{v_l^{F_\mathcal{G}}}& V^\star_{\mathcal{G}} \ar[d]^{\lgh_{V_{\mathcal{G}}}} \\ & 1 \ar@{>->}[r]_{\delta_{k}} & \mathbb{N} & & 1  \ar@{>->}[r]_{\delta_{l}}& \mathbb{N}}
\]

Now for every $i<k$ and $j <l$ we have:
\begin{gather*}
	\pi^{F_\mathcal{G}}_{k,i} (s_{k,l}^{\mathcal{G}}(e)) = (v^{F_\mathcal{G}}_k(s_{k,l}^{\mathcal{G}}(e)))_i = F_{\mathcal{G}}(f_i)(e) =\pi^{F_\mathcal{G}}_{k,i} (s_{k,l}^{F_\mathcal{G}}(e)) = \pi^{F_\mathcal{G}}_{k,i} ( v^{F_\mathcal{G}}_k(s_{F_\mathcal{G}}(\iota^{F_\mathcal{G}}_{k,l} (e))))   \\
\pi^{F_\mathcal{G}}_{l,j} (t_{k,l}^{\mathcal{G}}(e)) = (v^{F_\mathcal{G}}_l(t_{k,l}^{\mathcal{G}}(e)))_j = F_{\mathcal{G}}(f_{j+k})(e) =\pi^{F_\mathcal{G}}_{l,j} (t_{k,l}^{F_\mathcal{G}}(e)) = \pi^{F_\mathcal{G}}_{l,j} ( v^{F_\mathcal{G}}_l(t_{F_\mathcal{G}}(\iota^{F_\mathcal{G}}_{k,l} (e))))  
\end{gather*}
	but this means that
	\[v^{F_{\mathcal{G}}}_k\circ s^{\mathcal{G}}_{k,l} = v^{F_{\mathcal{G}}}_k\circ s_{F_{\mathcal{G}}}\circ \iota^{F_{\mathcal{G}}}_{k,l}=\] 
...
\iffalse 
\produ*
\begin{proof}\label{proof:produ}
	Let us build the wanted arrows. The existence of  $\pi^D_E\colon E_{\mathcal{L}}\to E_D$ follows at once from the universal property of coproducts. For the other two,  recall that by the first point of \Cref{prop:fact}  we know that $(V^\star, \{v_n\}_{n\in \mathbb{N}})$ and $(V_D^\star, \{v_{D, n}\}_{n\in \mathbb{N}})$ are coproducts for, respectively, $\{V^n\}_{n\in \mathbb{N}}$ and $\{V^n_D\}_{n\in \mathbb{N}}$. 
	...
	\begin{minipage}[r]{.6\linewidth}
		...
		\medskip
		Now,  by \Cref{rem:len} the bottom halves of the two diagrams aside are pullbacks while their outer boundaries are commutative by the definition of $E^D_{k,l}$. The universal property of pullbacks now yields us the two dotted arrows $s^{k,l}_{D}\colon E_{k,l} \to V^k_D$ and  $t^{k,l}_{D}\colon E_{k,l} \to V^l_D$. Since limits commute with limits  $(V^n, \{\pi^n_D\}_{D\in \D})$ is a product for the family $\{V_D^n\}_{D\in \D}$ and so we also get $s^{k,l}_{\mathcal{L}}\colon E_{k,l} \to V^k_D$ and  $t^{k,l}_{\mathcal{L}}\colon E_{k,l} \to V^l_D$ fitting in the diagrams below.
	\end{minipage}\hfill 
	\begin{minipage}[l]{.3\linewidth}
	\xymatrix@R=15pt{ E^D_{k,l} \ar@{.>}[d]_{s^{k,l}_D}\ar@/^.5cm/[dd]^(.3){!_{E^D_{k,l}}}|(.52)\hole \ar@{>->}[r]^{i^D_{k,l}} & E_D \ar[d]^{s_D} &  E^D_{k,l} \ar@{.>}[d]_{t^{k,l}_D} \ar@{>->}[r]^{i^D_{k,l}} \ar@/^.5cm/[dd]^(.3){!_{E^D_{k,l}}}|(.52)\hole& E_D \ar[d]^{t_D}\\ V^k_D \ar@{>->}[r]_(.55){v_{D, k}} \ar[d]_{!_{V^k_D}}&  V_D^\star  \ar[d]^{\lgh_{V_D}}  & V^l_D \ar[d]_{!_{V^l_D}} \ar@{>->}[r]_(.55){v_{D, l}} & V_D^\star \ar[d]^{\lgh_{V_D}}\\ 1 \ar@{>->}[r]_{\delta_k}& \mathbb{N}  &1 \ar@{>->}[r]_{\delta_l} & \mathbb{N}}
	\end{minipage}

			\[\xymatrix@R=15pt@C=35pt{E^D_{k,l} \ar[drr]^(.6){s^{k,l}_D}\ar@{>->}[r]^{i^D_{k,l}} & E_D \ar[r]^{s_D} & V^\star_D  &E^D_{k,l} \ar[drr]^(.6){s^{k,l}_D}\ar@{>->}[r]^{i^D_{k,l}} & E_D \ar[r]^{t_D} & V^\star_D \\  E_{k,l} \ar@{>->}[d]_{\iota_{k,l}} \ar[u]^{\pi^D_{k,l}} \ar@{.>}[r]^{s^{k,l}_\mathcal{L}} & V^k \ar@/_.3cm/@{>->}[dr]_(.3){v_k} \ar[r]_{\pi^k_D}& V^k_D \ar@{>->}[u]^{v^D_k}&E_{k,l} \ar@{>->}[d]_{\iota_{k,l}} \ar[u]^{\pi^D_{k,l}} \ar@{.>}[r]^{s^{k,l}_\mathcal{L}} & V^l \ar@/_.3cm/@{>->}[dr]_(.3){v_l} \ar[r]_{\pi^l_D}& V^l_D \ar@{>->}[u]^{v^D_l}\\  E_{\mathcal{L}}   \ar@{.>}[rr]_{s_\mathcal{L}}& &V^\star \ar@/_.5cm/[uu]_(.35){(\pi^D_V)^\star} &  E_{\mathcal{L}}   \ar@{.>}[rr]_{t_\mathcal{L}}& &V^\star \ar@/_.5cm/[uu]_(.35){(\pi^D_V)^\star}}
	\]
...
\end{minipage} \hfill
\begin{minipage}[r]{.25\linewidth}
	\xymatrix@R=10pt@C=55pt{E^{\mathcal{G}}_{k,l} \ar[d]_{!_{E^{\mathcal{G}}_{k,l}}} \ar@{>->}[r]^{i^{\mathcal{G}}_{k,l}}& E_{\mathcal{G}}\ar[d]_{(\lgh_{V_{\mathcal{G}}}\circ s_{\mathcal{G}}, \lgh_{V_{\mathcal{G}}}\circ t_{\mathcal{G}})}\\ 1 \ar@{>->}[r]_{(\delta_k, \delta_l)}& \mathbb{N}\times \mathbb{N}}

\xymatrix@R=10pt@C=55pt{E^{D}_{k,l} \ar[d]_{!_{E^{D}_{k,l}}} \ar@{>->}[r]^{i^{D}_{k,l}}& E_D\ar[d]_{(\lgh_{V_{D}}\circ s_{D}, \lgh_{V_{D}}\circ t_{D})}\\ 1 \ar@{>->}[r]_{(\delta_k, \delta_l)}& \mathbb{N}\times \mathbb{N}}

	\xymatrix@R=10pt@C=35pt{ E_{\mathcal{G}} \ar@{.>}[r]^{h_1} \ar@/_.5cm/[dd]_(.3){h_{1,D}}& E_{\mathcal{L}} \\ E^{\mathcal{G}}_{k,l} \ar@{>->}[u]_{\iota^{\mathcal{G}}_{k,l}} \ar@{.>}[r]^{f_{k,l}} \ar@{.>}[dr]_{f^D_{k,l}} & E_{k,l} \ar[d]^{\pi^D_{k,l}} \ar@{>->}[u]_{\iota_{k,l}}\\ E_{D}  & E^D_{k,l} \ar@{>->}[l]^{\iota^D_{k,l}}}
\end{minipage}
