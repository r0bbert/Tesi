...
In this  section we introduce a family of categories of hypergraphs, which will play a key role in our modelling of EGGs.
Given the many different flavours of hypergraphs we need, it is worthwhile to give a visual map of all the categories that 
we are going to discuss and of the relationships between them. 
...
$\hyp$ is the standard category of hypergraphs; $\lmo$ is the subcategory of left-monogamous hypergraph,
ie. satisfying suitable constraints on the number of edges entering a node, and which are used here to present 
...
\subsection{Background: Kleene star}
...
\subsection{Background: The category of hypergraphs}\label{sub:hyp}
...
We are now going to introduce some classes of hypergraphs, and the relative subcategories, which will provide the base for our definitions of EGGs  (cf.~\Cref{egg}). 
...
We can now introduce another  subcategory of $\hyp$ that we are interested in and that is going to be used in \Cref{sec:sepegg} to give the underlying hypergraphs for EGGs with minimally identified tokens.
...
\vspace{.1cm}
As we have done for $\pbc$, we must now show that $\pbp$ is a suitable class for adhesivity. A proof is in \Cref{proof:pbmonoplus}.
...
\begin{minipage}[l]{.75\linewidth}
	\begin{exa}
		Let $\mathcal{G}$ be the graph aside. Notice that $\id{\mathcal{G}}$ is a labelled e-hypergraph even if its underlying hypergraphs with equivalence $\mathcal{G}$ is not in $\egg$.
	\end{exa}
...
\subsection{Rewriting} The theory of $\mathcal{M}$-adhesivity ensures that if the rules are spans of 
...
are spans $L \leftarrow L \rightarrow R$, where the left-hand side is the identity.
