...
The pictures above depict four left-monogamous (i.e. term graphs) hypergraphs with equivalence
labelled over  $\Sigma$, thus living in $\lmo_{\eq, \Sigma}$.
%all of them pruned.
%
In the picture (1) each equivalence class contains only one node, and its
image along the forgetful functor $O_{\eq,\Sigma}: \lmo_{\eq, \Sigma} \to \lmo_\Sigma$ 
...
\subsubsection{Issue 1: $\mathcal{M}$-adhesivity}
...
advanced in~\cite{xx} is not completely satisfactory for our purposes. Indeed, while rules can 
be spans of any kind od morphisms, the match is required to belong to $\mathcal{M}$, which is 
quite restrictive. As an example, the e-term graph below states that the constant $a$ is actually 
identified 
