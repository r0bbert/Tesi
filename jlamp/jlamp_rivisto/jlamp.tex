...
The two rows on the top refer to categories of labelled hypergraphs, the bottom two to their unlabelled variants.
$\hyp$ is the standard category of hypergraphs; Sep is the subcategory of \emph{separated} hypergraphs 
(i.e., where xxx); and LMH is the category of 

Finally, the prefix \emph{p} stands for \emph{pruned}, meaning hypergraphs with no isolated nodes. 

FINIRE MINI-SPIEGA DELLA MAPPA
...
\begin{rem} Let $\mathcal{G}$ be a hypergraph. By \Cref{rem:elem}, right monogamy amounts to the request that for every vertex $v\in V_{\mathcal{G}}$ there exists at most one $e_V\in E_{\mathcal{G}}$ such that $\ind{G}(v, e) \neq 0$ and  for such hyperedge we have $\ind{G}(v,e)=1$.
 \end{rem}
