...
as summarised by the table below
%
\[\mbox{Qua ci vorrebbe una tabella riassuntiva}\]


The fact holds for various classes of hypergraphs, and 
...
The rule lives in the category $\lepsBis$, since the intermediate and the right-hand side labelled e-term graphs 
are separated and pruned. As such, the same rule can be used also for DPO rewriting in $\lelmBis$ and $\eggll$,
...
captures EGGs approach: since it requires that a rule must preserve the $L$ component, 
...
\subsubsection{Issue 2: Application conditions [DA AGGIUSTARE]}
...
If you consider e.g. the rule $x / x \to 1$, once $1$ has been added once to the equivalence class of $x / x$, it is not necessary to
keep on adding it.
