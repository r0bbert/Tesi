...
a separated term graph which is the minimal representation of $a / a$ in  $\lepsBis$.
...
and $\lepsBis$, we favour a solution which recalls of the 
actual heuristcs of the EGG framework by relying on the 
classical solution adopted in the term graph literature, that is, the 
introduction of specific collapsing rules~\cite{AriolaKP00}.
For example, for the simple algebraic signature containing only 
the constants $a$ and $1$ and the binary oprator $/$, it suffices to introduces 
the three collapsing rules below:
\begin{gather*}
\hspace{.25cm}
\xymatrix{        
\begin{tikzpicture}[baseline=(nil.center)]\begin{pgfonlayer}{nodelayer}
...
of EGGs can be pushed to model their rewriting via the DPO approach. We have seen that 
