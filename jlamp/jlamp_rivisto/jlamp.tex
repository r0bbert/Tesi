...
Let us begin this section with the definition of hypergraphs~\cite{xxx}.
...
%\vspace{.1cm}
\noindent
According to the double-pushout (DPO) approach to rewriting in $\mathcal{M}$-adhesive
categories~\cite{xxx},
a rule is given by two arrows $l: K \to L$ and $r: K \rightarrow R$
and its application requires a match $m: L \to G$: the rewriting step from $G$
to $H$ is then given by the diagram below, whose halves are pushouts
...
among the two proposals, which appear to be related yet quite different. In~\cite{ghica}, equivalence of nodes is captured by a special kind of \emph{hierarchical} edges together with a partial order a consistency relation, using a formalism which is reminiscent of hierarchical hypergraphs. Moreover, by considering term graphs as arrows of a symmetric monoidal category enriched over semi-lattices,~\cite{ghica} connects e-hypergraphs with the theory of PROPs. While~\cite{ghica}, as us, exploits DPO rewriting, it lacks an examination of the adhesivity properties of the category of e-hypergraphs studied.
%
%In fact, for both proposals the key intuition is  to use string diagrams to represents term graphs. 

We adopted a more down-to-earth approach by equipping
...
intuitively ensure the correspondence between hierarchies and equivalence classes. They do not explore
$\mathcal{M}$-adhesivity, embedding instead their hypergraphs into categories of string diagrams with an enrichment 
over semi-lattices, along the lines of~\cite{bonchi2022string}, and proving a soundness and  completeness result.
Even if we both consider the DPO approach for rewriting, our constructions are technically simpler, more in tune with
the set-theoretical approach of EGG, which facilitates its practical application 
and theoretical analysis, and aligns with the focus on $\mathcal{M}$-adhesivity
currently pusued by the graph transformation community.

=======
at the same time extending and generalising it~\cite{concur2006} in the contemporary jargon of adhesive categories  allowing us to exploit the features of the framework
as they hold for DPO graph transformation.
 Moreover, our construction is technically simpler and more easily related to the concrete operational model of EGGs, thus facilitating its practical application 
...
>>>>>>> 25115fc78958c0e742657160e016a13356f35c6b
