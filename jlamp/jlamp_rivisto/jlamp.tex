...
\newcommand{\pbp}{\mathsf{Pb}^{+}}


\newcommand{\pbe}{\mathsf{ePb}^{+}}
\newcommand{\pbl}{\mathsf{LPb}}
\newcommand{\pblp}{\mathsf{LPb}^{+}}
\newcommand{\pbs}{\mathsf{SPb}}
\newcommand{\pbsp}{\mathsf{SPb}^{+}}
...
\newcommand{\elm}{\mathsf{e}\text{-}\catname{LMH}_\eq}
\newcommand{\eps}{\mathsf{e}\text{-}\catname{pSep}_\eq}
\newcommand{\lelm}{\mathsf{e}\text{-}\catname{LMH}_{\eq, \mathcal{G}}}
\newcommand{\leps}{\mathsf{e}\text{-}\catname{pSep}_{\eq, \mathcal{G}}}
\newcommand{\lelmBis}{\mathsf{e}\text{-}\catname{LMH}_{\eq, \Sigma}}
...
if the pushout complement $C \to G$ exists then it is essentially unique, hence a rewriting step can be deterministically performed.
%
...
If only the left-hand side belongs to $\mathcal{M}$, the theory is still under development:
It has been addressed for the 
as witnessed e.g.
by~\cite{BaldanC0G24,BehrHK23}.
%
...
%
For structures such as \emph{hypergraphs with equivalence} in~\cite{concur2006}, the question 
...
representation and efficient implementation of equality saturation. Albeit being a classical data structure \cite{DetlefsNS05}, EGGs received 
...
if $a$ and $b$ are two constants such that $a \equiv b$, then $f(a) \equiv f(b)$ for every unary operator $f$.
...
Building on the criterion developed in~\cite{CastelnovoGM24}, this work 
investigate $\mathcal{M}$-adhesivity for EGGs. The path proceeds by 
specialisation, starting from standard hypergraphs and exploring a family of 
categories of hypergraphs with equivalence, proving along the way that 
they form an $\mathcal{M}$-adhesive category for suitable choices of $\mathcal{M}$.
The advantages from this approach are two-fold. On the one side, 
...
Given the interest in hypergraphical structures, the intermediate
results seems of independent interest beyond the core proofs for EGGs.
...
of hypergraphs with equivalence and in Section~\ref{eggs}, where equivalences are 
closed with respect to operator application, thus subsuming EGGs.
%
In Section~\ref{rewriting} we put the machinery to work, showing how the optimisation steps
...
even if we found technically convenient to present term graphs
in a different, yet equivalent way as an instance of the newly 
introduced category of  left-monogamous hypergraphs. Also the notion of separated 
hypergraphs is original, thus Section~\ref{sub:sub} and Section~\ref{sssect:hyp_alg_sign}
are largely new, and consequently so Section~\ref{hypereq}
and Section~\ref{eggs}.
%
...
\begin{restatable}{lem}{kpp}\label{lemma:kern_pairs_pres_pullbacks}
	...
	\[\xymatrix@R=15pt{X \ar[r]^{h}\ar[d]_{f}& Z \ar[d]^{g} & K_f \ar@{.>}[r]^{k_h} \ar[d]_{\pi^1_f}& K_g \ar[d]^{\pi^1_g} & K_f  \ar@{.>}[r]^{k_h} \ar[d]_{\pi^2_f}& K_g \ar[d]^{\pi^2_g}& K_f  \ar@{.>}[r]^{k_h} \ar[d]_{(\pi^1_f,\pi^2_f)} & K_g \ar[d]^{(\pi^1_g, \pi^2_g)}\\ Y \ar[r]_t& W & X \ar[r]_{h} & Z & X\ar[r]_{h} &Z & X\times X \ar[r]_{h\times h}& Z\times Z}\]
...
In this  section we introduce a family of categories of hypergraphs, 
Most of them have been already considered in the literature, and we
add the relevant references inline. Our approach offers a uniform treatment,
though, and proves original results concerning their adhesivity..
Most importantly, the presentation is tailored for introducing the novel categories 
of hypergraphs with equivalence and
EGGs in Section~\ref{hypereq} and Section~\ref{eggs}, respectively.
%
However, given the many different flavours of hypergraphs we need, it is worthwhile to give a visual map of the categories that 
we are going to discuss and of their relationships. 
We use the curved arrows to denote full and faithful inclusions, while the vertical arrows are forgetful functors.
...
The two bottom rows refer to categories of unlabelled structures.
$\hyp$ is the standard category of hypergraphs; $\lmo$ is the subcategory of left-monogamous hypergraphs,
satisfying suitable constraints on the number of edges entering a node, and which are used here to present 
an alternative characterisation for term graphs;
and  $\shyp$ is the subcategory of \emph{separated} hypergraphs, which are both 
left- and right-monogamous.
Finally, the prefix \emph{p} stands for \emph{pruned}, meaning hypergraphs with no isolated nodes. 
%
The two top rows concern labelled hypergraphs, which are as usual obtained via the comma category construction.
%

\subsection{Kleene star}
To present the notion of \emph{hypergraph}, a pivotal role is played by the \emph{Kleene star monad} $(-)^\star\colon \Set\to \Set$, also known as 
\emph{list monad},
...
\begin{prop}\label[prop]{prop:fact}
	...
	\begin{enumerate}
		...
		\item for every arrow $f\colon X\to Y$, $f^\star\colon X^\star \to Y^\star$ is the coproduct of the family $\{f^n\}_{n\in \mathbb{N}}$, where $f^n$ is the image of $f$ under the functor $(-)^n$;
		\item $(-)^\star$ preserves all \emph{connected limits} \cite{carboni1995connected}, in particular pullbacks and equalizers.
	\end{enumerate}
\end{prop}
...
\begin{rem}\label{rem:le} Consider a function $f\colon X\to Y$  and suppose that $(X^\star, \{x_n\}_{n\in \mathbb{N}})$ and $\{Y^\star, \{y_n\}_{n\in \mathbb{N}}\}$ are the coproducts given by the first point of \Cref{prop:fact}.
...
Let us begin this section with the definition of hypergraphs~\cite[Sec.~3]{bonchi2022string}.
...
Let $\pro^\star$ be the functor $\Set\to \Set$ sending $X$ to the product $X^\star\times X^\star$.  We can use it to present $\hyp$ as a comma category \cite[Prop.~$6.2.2$]{castelnovo2023thesis}.
...
Applying \Cref{thm:slice-functors} we also get the next corollary (cf.~\cite[Fact 4.17]{ehrig2006fundamentals}).

\begin{cor}\label[corollary]{prop:hypadh}
	 $\hyp$ is an adhesive category.
\end{cor}

Propositons \ref{prop:left} and \ref{prop:com} allow us to deduce immediately the following.
...
Sources and targets are represented by lines between dots and squares, respectively; arrowheads denote the targets.
	...
	\begin{center}
        \begin{tikzpicture}
                ...
                \begin{pgfonlayer}{edgelayer}
                        \draw[->] (h1b.center) to (v1);
                        \draw (v1) to (th2.center);
                        \draw[->] (2c.center) to (v2);
                        \draw[in=180, out=30] (v2) to (bh2.center);
                        \draw[->] (h2b.center) to (v3);
                        \draw (v3) to (th3.center);
...
\begin{minipage}[l]{.75\linewidth}
	\begin{defi}\label{def:alg}
		...
		We say that $\Sigma$ is \emph{algebraic} if $\coar_{\Sigma}$ is the constant function with value $1$.
	...
	\begin{tikzpicture}
		\begin{pgfonlayer}{nodelayer}
			...
			\node[style=none] (vlab) at (0.4, 0){$v$};
			...
			\node[style=medium box] (times) at (0.4, 1.9) {};
			\node[style=none] (timeslab) at (0.4, 2.7) {$*$};
			\node[style=none] (timesin1) at (0.3, 2.2) {};
			\node[style=none] (timesin2) at (0.3, 1.6) {};
			\node[style=none] (timesout) at (0.6, 1.9){};
			
			\node[style=medium box] (div) at (0.4, -1.9) {};
			\node[style=none] (divlab) at (0.4, -2.7) {$/$};
			\node[style=none] (divin1) at (0.3, -2.2) {};
			\node[style=none] (divin2) at (0.3, -1.6) {};
			\node[style=none] (divout) at (0.6, -1.9){};
		\end{pgfonlayer}
		
		\begin{pgfonlayer}{edgelayer}
%			\draw[style=diredge, in=180, out= 112.5] (v) to (timesin2.center); 
			\draw[in=180, out= 90] (v) to (timesin2.center); 
			\draw[out=110, in= 180] (v) to (timesin1.center);
			\draw[->, in=150, out=0] (nattach.center) to (v);
			
			\draw[->, out=0, in=-150] (constattach.center) to (v);
			\draw[in=180, out= -90] (v) to (divin2.center); 
			\draw[out=-110, in= 180] (v) to (divin1.center);
			
			\draw[->, in=35, out=0] (timesout.center) to (v);
			\draw[->, in=-35, out=0] (divout.center) to (v);
		\end{pgfonlayer}
	\end{tikzpicture}
	\end{minipage}
...
\begin{defi}
A hypergraph $\mathcal{G}=(E_{\mathcal{G}}, V_{\mathcal{G}}, s_{\mathcal{G}}, t_{\mathcal{G}})$ is \emph{algebraic} if $t_{\mathcal{G}}(e)$ has length $1$ for every $e\in E_{\mathcal{G}}$.
\end{defi}
...
\subsection{Some subcategories of $\hyp$}\label{sub:sub}
...
\subsubsection{Left-monogamous hypergraphs}\label{par:lm}
We first introduce \emph{left-monogamous} hypergraphs \cite{milosavljevic2023string,fritz2023free}, a generalization of \emph{term graphs} \cite{CorradiniG05,Plu:TGR-ENTCS}.  

\begin{defi}
	Let $\mathcal{H}=(E_{\mathcal{H}}, V_{\mathcal{H}}, s_{\mathcal{H}}, t_{\mathcal{H}})$ be a hypergraph. We define two functions $\ind{H}, \outd{H}\colon V_{\mathcal{H}}\times E_{\mathcal{H}}\rightrightarrows \mathbb{N}$ where $\ind{H}(v,e)$ is the number of occurences of $v$ in $s_{\mathcal{H}}(e)$ and $\outd{H}(v,e)$ that of $v$ in $t_{\mathcal{H}}(e)$. If $v$ is such that $\outd{H}(v,e)=0$ for every hyperedge $e$ we say that $v$ is an \emph{input} node. We will denote the set of input nodes by $\ino{H}$.
...
\begin{restatable}{prop}{elem}\label{rem:elem} Given a hypergraph $\mathcal{H}$, the following facts are equivalent
	\begin{enumerate}
...
\noindent
\begin{minipage}[l]{.6\linewidth}
	\begin{exa}The two hypergraphs aside are examples of non left-monogamous hypergraph.  In the top hypergraphs, left-monogamy fails because $\outd{\mathcal{G}}(v, e) = 2$, 
	while in the hypergraph in the bottom we have \[\outd{H}(u, e_1) = \outd{H}(u, e_2)=1\]
	\end{exa}
\end{minipage} \hfill 
\begin{minipage}[r]{.35\linewidth}
	\begin{tikzpicture}
...
\medskip 	
The remainder of this section is devoted to the proof of quasiadhesivity of $\lmo$. We begin by computing pullbacks and equalizers of left-monogamous hypergraphs.


\begin{rem}\label{rem:con}Notice that  $\lmo$ is not complete: it lacks e.g. a terminal object. To see this, notice that $\mathcal{H}:=(\emptyset, 1, ?_{1}, ?_{1} )$ is left-monogamous and that 
...
\vspace{.1cm}
	\begin{minipage}[l]{.65\linewidth}
		\begin{itemize}
			\item  all the hyperedges of $\mathcal{T}$ have the empty word as target; or
...
\begin{defi}
A morphism $(f,g)\colon \mathcal{G}\to \mathcal{H}$ of hypergraphs  is \emph{downward-closed} if whenever a letter occuring in $t_{\mathcal{H}}(h)$ lies in the image of $g$, then $h$ belongs to the image of $f$. 
\end{defi}

Being downward-closed is related to the preservation of input nodes, as shown by the following lemma (see \Cref{proof:inp} for the proof).
...
Let $(f_1,g_1)\colon \mathcal{G}\to \mathcal{H}$ and $(f_2, g_2)\colon \mathcal{G}\to \mathcal{K}$ be two morphisms in $\lmo$. If $(f_1,g_1)$ is mono and downward-closed, then the pushout $\mathcal{P}$ of $(f_1,g_1)$ along $(f_2, g_2)$ in $\hyp$ is left-monogamous.
\end{restatable}
...
We can use \Cref{prop:pusho} to completely characterise regular monos in $\lmo$. We refer the reader to \Cref{proof:regmon} for the proof.
...
Combining \Cref{thm:slice-functors} with \Cref{cor:pusho}  and \Cref{lem:regmon}, we get the following.

\begin{cor}\label{cor:lmquasi}$\lmo$ is quasiadhesive.
\end{cor}


\subsubsection{Pruned and separated hypergraphs}
We can now introduce a novel  subcategory of $\hyp$ that is going to be used in \Cref{sec:sepegg} to give the underlying hypergraphs for EGGs with minimally identified tokens.
...
\begin{rem} Let $\mathcal{G}$ be a hypergraph. By \Cref{rem:elem}, being right-monogamous amounts to the request that for every vertex $v\in V_{\mathcal{G}}$ there exists at most one $e_V\in E_{\mathcal{G}}$ such that $\ind{G}(v, e) \neq 0$ and  for such hyperedge we have $\ind{G}(v,e)=1$.
 \end{rem}
 ...
 \begin{rem}\label{rem:count}
 	By \Cref{rem:elem2} it is clear that separated hypergraphs enjoy a property similar to the one enjoyed by left-monogamous ones (cf.~\Cref{proof:count}): if $\mathcal{G}$ is separated and $(f,g)\colon \mathcal{H}\to \mathcal{G}$ is a mono, then $\mathcal{H}$ is separated too.
 \end{rem}

We can provide a construction which ``splits'' a given hypergraph to produce a separated one.

\begin{defi}
Given a hypergraph $\mathcal{\mathcal{G}}=(E_{\mathcal{G}}, V_{\mathcal{F}}, s_{\mathcal{G}}, t_{\mathcal{G}})$, for every hyperedge $e\in E_{\mathcal{G}}$ with $n_e=\lgh_{V_{\mathcal{G}}}(s_{\mathcal{G}}(e))$ and $m_e=\lgh_{V_{\mathcal{G}}}(t_{\mathcal{G}}(e))$, we can consider the coproduct $(V_e, \{\iota^e_{i}\}_{i=1}^{n_e+m_e})$ of $n_e+m_e$ copies of $1$.
...
\begin{exa}\label{ex:alg}
	Let $\Sigma$ be a monoidal signature, then $\sp(\mathcal{G}^\Sigma)$ collects all the operations of $\Sigma$ as edges with disjoint sources and targets. For instance let $\Sigma$ be the algebraic signature of monoids, with $O_\Sigma = (\cdot, e )$ with 
	...
	\begin{center}	\begin{tikzpicture}
			\begin{pgfonlayer}{nodelayer}
				...
				\node[style=medium box] (times) at (0.3, 1.5) {};
				\node[style=none] (timeslab) at (0.3, 2.2) {$\cdot$};
				\node[style=none] (timesin1) at (0, 1.8) {};
				\node[style=none] (timesin2) at (0, 1.2) {};
				\node[style=none] (timesout) at (0.5, 1.5){};
			...
			\begin{pgfonlayer}{edgelayer}
				\draw[style=none, in=180, out= 100] (v) to (timesin2.center); 
				\draw[style=none,out=130, in= 180] (v) to (timesin1.center);
...
\begin{rem}Notice that $\sp$ is neither right nor left adjoint to $S$.
	\begin{itemize}
		...
		Let $\mathcal{H}$ be $(\emptyset, 1, ?_{1^\star}, ?_{1^\star})$, which is clearly a separated hypergraph. Now,  $\shyp(\mathcal{H}, \sp(\mathcal{D}))$ has the same cardinality as  $\shyp(\mathcal{H}, \mathcal{G})$, which is $4$, while $\hyp(S(\mathcal{H}), \mathcal{D})$ is a singleton.
		\item Consider now the ``single loop'' $\mathcal{L}=(1, 1, \id{1}, \id{1})$. Then $\sp(\mathcal{L})$ is isomorphic to the graph $(2, 4, i_{even}, i_{odd})$, where
		\[i_even(x) = \begin{cases}
			0 & x= 0\\
			2 & x = 1
		\end{cases} \qquad i_odd(x) = \begin{cases}
		1 & x= 0\\
		3 & x = 1
		\end{cases} \]
		
		The graph $\sp(\mathcal{L})$ consists of the two ``pieces'' depicted alongside. By construction there are no arrows $\mathcal{L}\to S(\sp(\mathcal{L}))$. In particular, this shows that $\sp$ cannot be left adjoint to $S$. 
	\end{itemize} 
\end{rem}
\end{minipage}
\hfill
...
\begin{defi} Let $\mathcal{G}$ be a hypergraph. A node $v\in V_{\mathcal{G}}$ is \emph{isolated} if it does not occur as a letter in either $s_{\mathcal{G}}(e)$ or in
...
\begin{restatable}{prop}{dx}\label{prop:dx}The following properties hold
	\begin{enumerate}
...
Finally, we conclude our study of separated hypergraphs proving a result regarding connected limits. A proof of it is \Cref{proof:seplim}
...
In general $\shyp$ is not complete. For instance it does not have a terminal object. To see this notice that the $\mathcal{H}:=(\emptyset, 1, ?_{1^\star}, ?_{1^\star} )$ is separated and that 
...
\begin{exa}\label{ex:nopush}
	In general $\shyp$ does not have pushouts. For instance consider the solid part of diagram aside. Let us call the two unnamed hypergraphs on the top row $\mathcal{H}$ and $\mathcal{K}$, respectively, and call the third one $\mathcal{Q}$.
...
\smallskip
If we restrict to pruned separated hypergraphs the previous issue disappeares.  The crucial observation is that the a pruned separated hypergraph is characterized by its set of hyperedges and the length of their sources and targets. See \Cref{proof:pruned} for a proof.
...
Our interest for hypergraphs stems from their use as a graphical representation of algebraic terms. As  noted in \Cref{sub:hyp_alg}, a hypergraph can be thought as a colored monoidal signature. Thus labelling corresponds to the operation of taking the slice category over a suitable hypergraph. 

\begin{defi}
	Let $\mathcal{G}$ be a hypergraph. The category $\hyp_\mathcal{G}$ of hypergraphs \emph{labelled by $\mathcal{G}$}  or of \emph{$\mathcal{G}$-hypergraphs} is $\hyp/\mathcal{G}$.
\end{defi}
...
\begin{prop}\label{prop:mono} Let $\mathcal{G}$ be a hypergraph. Then the following facts hold
	\begin{enumerate}
...
\begin{exa} A useful example is given by an algebraic signature $\Sigma$. Consider the hypergraph of \Cref{exa_3}, the category $\hyps$ of
...
\begin{minipage}[l]{.8\linewidth}	
	\begin{rem}	\label{rem:label}
			...
			Consider an algebraic signature $\Sigma$ and a hypergraph $\mathcal{H}=(E, V, s, t)$. By definition we know that $U_{\hyp}(\mathcal{G}^{\Sigma})$ is the terminal object $1$, so an arrow $\mathcal{H}\rightarrow \mathcal{G}^{\Sigma}$ is determined by a function $h\colon E_\mathcal{H}\to O_\Sigma$  making the two squares on the right commutative (cf.~\Cref{rem:length}).
...
\begin{exa}\label[example]{lab_2}
	Consider again the signature $\Sigma$ of \Cref{exa_3}, then the hypergraph $\mathcal{G}$ of \Cref{exa_2} can be labeled by a morphism
...
\begin{defi}
	Let $\mathcal{G}$ be a hypergraph and $(h,k)\colon \mathcal{H}\to \mathcal{G}$  a $\mathcal{G}$-hypergraph. We say that $(h,k)$ is a \emph{left-monogamous} (resp.~\emph{separated, pruned}) $\mathcal{G}$-hypergraph if $\mathcal{H}$ is left-monogamous (resp.~separated, pruned).
...
\begin{thm}\label{thm:tg} Let $\mathcal{G}$ be a hypergraph. Then %the following fact hold
%	\begin{enumerate}
...
\end{rem}
In particular it does not have pushouts along all monos. For instance, if we take the three term graphs of the previous remark, then have two arrows
...
This section presents the framework through adhesive categories, including its extension to the various kinds of hypergraphs introduced in the previous section, as a stepping stone towards an analogous characterization for EGGs.

\subsection{Hypergraphs with equivalence}

Let us start with the case of general hypergraphs. These were introduced in~\cite{concur2006}, even if no general consideration about their structure as a category was provided, and adhesivity, 
...
\subsection{Some subcategories of $\EqHyp$}
\label{labelledSubs}

We can now focus to the various kind of hypergraphs introduced in \Cref{sub:sub}.

\begin{defi}Given a hypergraph with equivalence $\mathcal{H}:=(E_\mathcal{G}, V_{\mathcal{G}}, Q_\mathcal{G}, s_\mathcal{G}, t_\mathcal{G}, q_\mathcal{G})$, we say that $\mathcal{H}$ is \emph{left-monogamous} (resp.~\emph{separated, pruned}) if $T(\mathcal{H})$ is left-monogamous (resp.~\emph{separated, pruned}). We will denote by $\Eqlmo$, $\EqsHyp$ and $\EqpsHyp$ the  subcategories  given by left-monogamous, separated, pruned and separated hypergraphs with equivalence.
\end{defi}

\begin{rem}\label{rem:obv1}
By definition, there exist forgetful functors $O_{\eq}$, $R_{\eq}$ and $W_{\eq}$ as in the diagrams below, where the horizontal arrows are full and faithful inclusions
...
We are now going to generalize the work done in the previous sections equipping hypergraphs with an equivalence relation. First of all we need a notion of labelling for $\EqHyp$. 

\begin{defi}
	Let $\mathcal{G}$ be a hypergraph with equivalence. The category $\EqHyps$ of hypergraphs with equivalence \emph{labelled by $\mathcal{G}$}  is $\EqHyp/\mathcal{G}$.
\end{defi}

\begin{rem} The equivalence relation on the  nodes of the labelling hypergraph put constraints on the relations between the labelled hypergraphs. Consider the following hypergraphs with equivalences $\mathcal{G}$ and $\mathcal{H}$.

\smallskip
\noindent
\begin{minipage}[l]{.6\linewidth}
	\setlength{\parindent}{1.5em}
	Notice that there is no morphism $\mathcal{H}\to \mathcal{G}$. Indeed $\id{T(\mathcal{G})}$ is the unique morphism $T(\mathcal{G})\to T(\mathcal{H})$ while there are only two morphisms $f, g\colon Q_{\mathcal{H}}\rightrightarrows Q_{\mathcal{G}}$: the first one is constant in $v$, while the second one is constant in $w$.
\end{minipage}\hfill \qquad 
\begin{minipage}[r]{.4\linewidth}
\begin{tikzpicture}[baseline=(v1)]
	\begin{pgfonlayer}{nodelayer}
		\node[style=small box](a1) at (-.5, 1){};
		\node[style=none] (a1att) at (-.5, 1){};
		\node[style=node](v1) at (-.5, 0){};
		\node[style=small box](a2) at (.5, 1){};
		\node[style=none] (a2in) at (.22, 1){};
		\node[style=none] (a2out) at (.78, 1){};
		\node[style=node](v2) at (.5, 0){};
		\node[style=none] (l1) at (-.5, 1.5){$a$};
		\node[style=none](l2) at (.5, 1.5){$b$};
		\node[style=none] (l1) at (-1, 0){$v$};
		\node[style=none](l2) at (1, 0){$w$};
	\end{pgfonlayer}
	\begin{pgfonlayer}{edgelayer}
		\draw[->] (a1att.south) to (v1);
		\draw[style=none, out=180, in=180](v2) to (a2in.west);
		\draw[<-, style=none, out=0, in=0](v2) to (a2out.east);
	\end{pgfonlayer}
	\begin{pgfonlayer}{eqlayer}
		\draw[dashed, rounded corners] (-.8, -0.3) rectangle (-.2, 0.3);
		\draw[dashed, rounded corners] (.2, -0.3) rectangle (.8, 0.3);
	\end{pgfonlayer}
...
\begin{tikzpicture}[baseline=(v1)]
	...
	\begin{pgfonlayer}{edgelayer}
		\draw[->] (a1att.south) to (v1);
		\draw[style=none, out=180, in=180](v2) to (a2in.west);
		\draw[<-, out=0, in=0](v2) to (a2out.east);
	\end{pgfonlayer}
	\begin{pgfonlayer}{eqlayer}
		\draw[dashed, rounded corners] (-.8, -0.3) rectangle (.8, 0.3);
	\end{pgfonlayer}
\end{tikzpicture}
\end{minipage}

 For both cases we have
\[ f(q_{\mathcal{H}}(w))=v \neq w =q_{\mathcal{G}}(w)=q_\mathcal{G}(\id{V_{\mathcal{G}}}(w)) \qquad  g(q_{\mathcal{H}}(v))=w \neq v =q_{\mathcal{G}}(v)=q_\mathcal{G}(\id{V_{\mathcal{G}}}(v))  \]
...
\begin{minipage}[l]{.8\linewidth}
	\setlength{\parindent}{1.5em}
\begin{exa}
	Let $\Sigma$ be a single typed monoidal signature (cf.~\Cref{def:alg}). Since $\mathcal{G}_{\Sigma}$ has a single node, then to label another hypergraph $\mathcal{H}$ it is enough to provide a function $l\colon E_{\mathcal{H}}\to O_{\Sigma}$ such that the diagrams aside commute.
...
\begin{defi}Let $\mathcal{G}$ be a hypergraph and $(\mathcal{H},(f,g))$ a labelled hypergraph with equivalence.  We say that $(\mathcal{H},(f,g))$ is a \emph{left-monogamous} (resp.~\emph{separated, pruned}) $\mathcal{G}$-hypergraph with equivalence if $\mathcal{H}$ is left-monogamous (resp.~separated, pruned).
 ...
 \begin{prop}\label{prop:eti}
 	Let $\mathcal{G}$ be an hypergraph with equivalence and consider the inclusion functors $M_{\eq}\colon \Eqlmo\to \EqHyp$, $S_{\eq}\colon \EqsHyp \to \EqHyp$ and $S_{p, \eq}\colon \pshyp_{\eq}\to \EqHyp$. Let $\Gamma_{\mathcal{G}}\colon 1\to \EqHyp$ be the constant functor in $\mathcal{G}$. Then
 	\begin{enumerate}
 ...
 In particular, the previous proposition, together with Lemmas \ref{lem:equ}, \ref{lem:coequ}, \ref{colim} and \Cref{lim}, yields the following result.
...
We can  now use \Cref{thm:slice-functors} together with Lemmas \ref{lem:equ} and \ref{lem:coequ} to get the following.

\begin{cor}\label{cor:lim}$\lmo_{\eq, \mathcal{G}}$ is $\pbl_{\mathcal{G}}$-adhesive and $\pshyp_{\eq, \mathcal{G}}$ is $\pbs_{\mathcal{G}}$-adhesive
\end{cor}
...
encompass equivalence classes which are closed under the target arrow, i.e.~under operator composition, thus precisely capturing EGGs.
...
We can reformulate the definition of e-hypergraphs in two other ways. See \Cref{proof:eh} for a proof

\begin{restatable}{prop}{eh}\label{prop:eh}
Let $\mathcal{G} = (E, V, Q, s, t, q)$ be a hypergraph with equivalence and $(A, \pi^A_1, \pi^A_2)$, $(B, \pi^B_1, \pi^B_2)$ kernel pairs for, respectively, $q^\star\circ s$ and $q^\star\circ t$. Then the following facts are equivalent
\begin{enumerate}
...
As a next step, we we need to prove that connected limits in $\egg$ are computed as in $\EqHyp$. Full proofs are in \Cref{proof:elim}.
...
In order to provide conditions under which pushouts in $\egg$ are computed as in $\EqHyp$, we need to restrict ourselves to a suitable class of morphisms.

\noindent
\begin{minipage}[l]{.83\linewidth}
	\begin{defi}
	We define $\pbp$ as the class of arrows $(h_E, h_V, h_Q)\colon \mathcal{G}\to \mathcal{H}$ in $\EqHyp$ which belongs to $\pbc$ and such that the square on the right is a pullback.
\end{defi}
\end{minipage} \hfill 
\begin{minipage}[r]{.15\linewidth}
\xymatrix@R=12pt{
	E_\mathcal{G} \ar[r]_{s_\mathcal{G}} \ar@{>->}[d]_{h_E}& V_\mathcal{G} \ar@{>->}[d]^{h_V}\\
	E_\mathcal{H} \ar[r]^{s_\mathcal{H}}  & V_\mathcal{{H}}
}
\end{minipage}

\vspace{.1cm}
As for $\pbc$, we must now show that $\pbp$ is a suitable class for adhesivity. A proof is in \Cref{proof:pbmonoplus}.

\begin{restatable}{lem}{pbsuit}\label{lem:pbmonoplus}
The class $\pbp$ contains all isomorphisms, it is closed under composition, decomposition and it is stable under pullbacks and pushouts.
\end{restatable}
...
\begin{minipage}[l]{.83\linewidth}
	\begin{restatable}{lem}{epo}\label{lem:epo}
		Suppose that the square on the right is a pushout in $\EqHyp$ and that $m$ is a mono in $\pbp$. If $\mathcal{G}_1$, $\mathcal{G}_2$ and $\mathcal{G}_3$ are e-hypergraphs, then $\mathcal{P}$ is 
		is an e-hypergraph.
		%so too.
...
Let now $\pbe$ be the class of arrows in $\egg$ that are sent to $\pbp$ by $E$. By definition and \Cref{lem:pbmonoplus}, we have that this class is closed under composition and decomposition, it contains all isomorphisms and it is stable under pullbacks. Using \Cref{lem:pbmonoplus} and \Cref{lem:epo} we can further deduce that it is stable under pushouts. Then \Cref{thm:slice-functors} and \Cref{cor:eqade} yield the following result.
...
\begin{defi}We will define $\elm$ and $\eps$ as the full subcategories of $\egg$ given by those e-hypergraphs that are, respectively, left-monogamous and pruned and separated. We will denote by $M_{\mathsf{e}}$ and by $S_{\mathsf{e}}$ the corresponding functors.
\end{defi}

To study the adhesivity of these subcategories we need to introduce analogs of $\pbp$.
\begin{defi}We define two classes of arrows
	\begin{itemize}
		\item $\pblp$ is the class of all arrows in $\Eqlmo$ sent by $M_{\eq}$ to $\pbp$;
		\item $\pbsp$ is the class of all arrows in $\EqpsHyp$ sent by $S_{p,\eq}$ to $\pbp$.
	\end{itemize}
\end{defi}

Let $\epbl$ and $\epbs$ be the classes of arrows in  $\elm$ and $\eps$ lying in $\pblp$ and $\pbsp$, respectively. By Lemmas \ref{lem:equ}, \ref{lem:coequ} and \Cref{cor:pbe} we obtain immediately the following result.
...
\begin{defi} Let $\mathcal{G}$ be a hypergraph with equivalence. We say that an object $(a,b,c)\colon \mathcal{H}\to \mathcal{G}$ of $\EqHyps$ is a \emph{labelled e-hypergraph} if every two hyperedges $e,e'\in E_{\mathcal{H}}$ such that
...
\begin{minipage}[l]{.75\linewidth}
	\begin{exa}
		Let $\mathcal{G}$ be the graph aside. Note that $\id{\mathcal{G}}$ is a labelled e-hypergraph even if its underlying hypergraphs with equivalence $\mathcal{G}$ is not in $\egg$.
	\end{exa}
\end{minipage} \hfill \begin{minipage}[r]{.21\linewidth}
	\begin{tikzpicture}[baseline=(v1)]
		\begin{pgfonlayer}{nodelayer}
			\node[style=small box](a1) at (-1.4, 0){};
			\node[style=none] (a1att) at (-1.4, 0){};
			\node[style=node](v1) at (-.5, 0){};
			\node[style=small box](a2) at (1.4, 0){};
			\node[style=none] (a2in) at (1.4, 0){};
			\node[style=node](v2) at (.5, 0){};
			\node[style=none] (l1) at (-1.4, .5){$a$};
			\node[style=none](l2) at (1.4, .5){$b$};
			\node[style=none] (l1) at (-.5, -.5){$v$};
			\node[style=none](l2) at (.5, -.5){$w$};
		\end{pgfonlayer}
		\begin{pgfonlayer}{edgelayer}
			\draw[->] (a1att) to (v1);
			\draw[<-, style=none](v2) to (a2in.south);
		\end{pgfonlayer}
		\begin{pgfonlayer}{eqlayer}
			\draw[dashed, rounded corners] (-.8, -0.3) rectangle (-.2, 0.3);
			\draw[dashed, rounded corners] (.2, -0.3) rectangle (.8, 0.3);
...
\smallskip
We now provide an equivalent characterisation to labelled e-hypergraphs (see \Cref{proof:fh} for details).

\begin{restatable}{lem}{fh}\label{lem:fh}
Let $(a,b,c)\colon \mathcal{H}\to \mathcal{G}$ be an object of $\hyp_{\eq, \mathcal{G}}$ and $(A, \pi^A_1, \pi^A_2)$, $(B, \pi^B_1, \pi^B_2)$ kernel pairs for, respectively, $(q^\star\circ s_{\mathcal{H}}, a),(q^\star\circ t_{\mathcal{H}}, a)\colon E_{\mathcal{H}}\to Q_{\mathcal{H}}\times E_{\mathcal{G}}$. Then the following are equivalent
\begin{enumerate}
	\item $(a,b,c)$ is a labelled e-hypergraph;
	\item $q^\star\circ t_{\mathcal{H}}\circ \pi^A_1=q^\star\circ t_{\mathcal{H}}\circ \pi^A_2$;
	\item there exists a unique arrow $k\colon A\to B$ such that 
	\[\pi^B_1\circ k = \pi^A_1 \qquad \pi^B_2\circ k = \pi^A_2 \]
\end{enumerate}
\end{restatable}
...
\begin{defi}
	Let $V_\mathcal{G}\colon \EqHyps\to \EqHyp$ be the forgetful functor, we define $\pbp_{\mathcal{G}}$ as the class of arrows in $\EqHyps$ whose image in $\EqHyp$ is in $\pbp$. Moreover, we will denote by 
	$\pbe_\mathcal{G}$ the class of morphisms in $\eggl$ whose image through $E_\mathcal{G}$ is in $\pbp_\mathcal{G}$.
\end{defi}

We want now to show that pullbacks and pushouts along arrows in $\pbe_\mathcal{G}$ in $\eggl$ are computed as in $\hyp_{\eq, \mathcal{G}}$. See \Cref{proof:varie} for a proof.


\begin{restatable}{lem}{varie}\label{prop:varie}
	Let $\mathcal{G}$ be a hypergraph with equivalence. Then $\eggl$ is closed in $\hyp_{\eq, \mathcal{G}}$ under connected limits and pushouts along arrows in $\pbe_\mathcal{G}$. 
\end{restatable}
...
Putting together \Cref{cor:eti} with \Cref{prop:varie} we get the following result.

\begin{lem}Let $\mathcal{G}$ be a hypergraph with equivalence, then $\lelm$ and $\leps$ are closed in $\hyp_{\eq, \mathcal{G}}$ under pullbacks and pushouts along arrows in $\pbc_{\mathcal{G}}$.
\end{lem}
...
\begin{table}[ht]
	...
	\begin{tabular}{|l|c|r|}
		\hline 
		Category & Adhesivity property & Proof\\
		\hline 
			$\hyp_{\mathcal{G}}$ & adhesive & \Cref{prop:mono} \\
		\hline 
		$\pshyp_{\mathcal{G}}$ & adhesive & \Cref{thm:tg}\\
		\hline 
		$\lmo_{\mathcal{G}}$ & quasiadhesive & \Cref{thm:tg} \\
		\hline 
	$\EqHyps$ &  $\pbc_\mathcal{G}$-adhesive & \Cref{prop:lim} \\
		\hline 
		$\pshyp_{\eq, \mathcal{G}}$ &  $\pbs_{\mathcal{G}}$-adhesive & \Cref{cor:lim} \\
		\hline 
		$\lmo_{\eq, \mathcal{G}}$ & $\pbl_{\mathcal{G}}$-adhesive   & \Cref{cor:lim}\\
		\hline 
			$\eggl$ & $\pbe_\mathcal{G}$-adhesive & \Cref{cor:eggade1}\\
			\hline 
				$\lelm$ & $\epbl_{\mathcal{G}}$-adhesive & \Cref{cor:last}\\
				\hline 
				 $\leps$ & $\epbs_{\mathcal{G}}$-adhesive & \Cref{cor:last} \\
				 \hline 
			
	\end{tabular}
\end{table}

$\mathcal{M}$-adhesivity holds for various classes of hypergraphs, 
most importantly for the class of e-hypergraphs,
...
To this end, we now discuss which categories are adequate for 
simulating EGG rewriting via the DPO approach.
%
...
Classically, the introduction of term graphs (that is, left-monogamous algebraic hypergraphs) was motivated
by the desire to provide an alternative representation of terms, based on graphs 
instead of trees,  to exploit the sharing of sub-terms for efficiency reasons. Consider e.g. a signature $\Sigma$ 
with a constant $a$ and a binary operator $/$: the term $a / a$ admits two different representations 
as a term graph in $\lmo_{\Sigma}$. The image on the left below is the usual representation of the term as a tree, the 
image on the right is an hypergraph where the occurrence of the constant $a$ is shared
...
The introduction of an equivalence relation on nodes adds another layer of expressiveness, so that e.g.
the term $a / a$ actually has many different presentations in $\Eqhyps$. Four of them are represented below
%
...
The pictures above depict four left-monogamous hypergraphs (i.e. term graphs) with equivalence
labelled over  $\Sigma$, thus living in $\lmo_{\eq, \Sigma}$.
%all of them pruned.
%
In the picture (1) each equivalence class contains only one node, and its
image along the forgetful functor $\lmo_{\eq, \Sigma} \to \lmo_\Sigma$ 
(the labeled extension of the functor $O_{\eq}$ in Remark~\ref{rem:obv1}) 
is the standard representation of $a / a$ as a tree.
%
The picture (2) represents a separated term graph with two occurrences of the constant $a$ 
and a non-trivial equivalence on nodes.
However, they do not live in $\lelmBis$, that is, neither of them is an e-term graph:
...
%Intuitively, picture (1) is the representation of $a / a$ 
%among labelled hypergraphs, i.e. objects of $\hyps$, which has the minimal amounts of sharing between nodes.
%Instead, picture (2) is the minimally-shared representation 
%among labelled hypergraphs with equivalence, i.e. objects of $\Eqhyps$.
%
Consider now the pictures indicated by (3) and (4). They are both e-term graphs. Picture (3) is 
the representation in $\lelmBis$ of $a / a$ as a term graph with a shared constant; while picture (4) is 
% the minimally-shared representation among 
a separated and pruned term graph which is the minimal representation of $a / a$ in  $\lepsBis$.
...
resulting in some ambiguity in which is the canonical EGG associated to a term. To this end, we consider adequate both $\lelmBis$ and $\lepsBis$.
In particular, e-term graphs are the counterpart of the original definition; while the sub-category of pruned, separated 
e-term graphs guarantees a minimal representation for a term, and it seems more in touch with recent implementations~\cite{eggs}.
...
\subsection{DPO rewriting on e-term graphs}
%\vspace{.1cm}
\noindent
\begin{minipage}[l]{.80\linewidth} According to the double-pushout (DPO) approach to rewriting in $\mathcal{M}$-adhesive
categories~\cite{corradini1997algebraic,ehrig1999handbook,heindel2009category},
a rule is given by two arrows $l: K \to L$ and $r: K \rightarrow R$
and its application requires a match $m: L \to G$: the rewriting step from $G$
to $H$ is then given by the diagram aside, whose halves are pushouts. \end{minipage} \hfill \begin{minipage}[r]{.20\linewidth}
    \xymatrix@C=.5cm@R=.5cm{
      L \ar[d]_{m}
      & K \ar[r]^r \ar[l]_{l} \ar[d]_{k} & R \ar [d]^h \\
      G & C \ar[r]_{g} \ar[l]^{f}                    & H
...
The situation is simpler in the EGGs approach since, as we recalled in the introduction, it chooses to
just add new terms and link them to the older ones via the equivalence relation,
instead of removing sub-terms~\cite{DetlefsNS05}.
% until an optimal program is 
%reached and extracted.
So, the corresponding DPO rules 
are spans $L \leftarrow L \rightarrow R$ whose left-hand side is the identity,
and the DPO diagram can be suitably described by just a single pushout.
% thus in $\pbc_\Sigma$, while the second component may not belong to $\pbc_\Sigma$. 

Consider the term rewriting rule $x / x \to 1$, from the key example in~\cite{WillseyNWFTP21}.
%This is a problematic rule to be simulated using classical term graph rewriting, since it is not left-linear
...
The rule lives in the category $\lepsBis$, since the left-hand and right-hand sides are labelled e-term graphs 
that are separated and pruned. As such, the same rule can be used also for DPO rewriting in $\lelmBis$ and $\eggll$,
since these categories are related by inclusion.
%
%\begin{defi}We will define $\elm$ and $\eps$ as the full subcategories of $\egg$ given by those e-hypergraphs that are, respectively, left-monogamous and pruned and separated. We will denote by $M_{\mathsf{e}}$ and by $S_{\mathsf{e}}$ the corresponding functors.
%\end{defi}
%
...
%thus the rule could be applied to any of the term graph with equivalence
%depicted above with a match that is injective on equivalence classes.
%
However, the rule cannot be applied to the term graphs with equivalence
 in the pictures (1) and (2) above: there is no match $m: L\to G$ due to the 
 constraints on the equivalence classes.
 Instead, the result of the application of the rule 
...
An EGGs rewriting system is thus a family of rules in $\lepsBis$ of the shape
$L \leftarrow L \rightarrow R$: the left-hand side is the identity,
while the right-hand side is a mono. The format precisely 
captures the EGGs approach: since it requires that a rule must preserve the $L$ component, 
that is nodes and edges of the e-hypergraph to which $L$ is mapped must not be deleted, 
the left-hand side must be the identity, while the arrow $L \rightarrow R$ must be an injection
on the underlying term graph, hence a mono in $\lepsBis$ (thus, also in $\lelmBis$). 

%The underlying category coud be 
%either $\lepsBis$ or $\lepsBis$ 

\subsubsection{Issue 1: Left-linear rules}

The properties of $\mathcal{M}$-adhesivity ensure that if the rules are spans of 
arrows in $\mathcal{M}$, then we can lift the standard properties
of the DPO approach that hold in the category of graphs.

As noted above, the rules of an EGGs rewriting system have the shape
...
The arrow $L \rightarrow R$ does not  belong to $\epbl_{\Sigma}$ 
(hence,  neither in $\epbs_{\Sigma}$), thus it is not currently possible 
to lift to the categories at hand all the properties of the $\mathcal{M}$-adhesive framework.

The asymmetry between left-hand and right-hand sides falls in the research about left-linear rules for 
$\mathcal{M}$-adhesive categories, as pursued e.g. in~\cite{BaldanC0G24,BehrHK23}. 
The proposal in~\cite{BaldanC0G24} identify conditions on the rules such that a rewriting system 
satisfies the Church-Rosser property, which is a preliminary step on the way of 
addressing the key properties of sequential and parallel independence.
%
The solution advanced in~\cite{BehrHK23} rely on a more general framework based on
double categories, yet it does not seem to be directly applicable for our purposes. Indeed, while 
in their double-categorical framework the rules can 
be spans of morphisms with no constraints, the match is required to belong to $\mathcal{M}$, which is 
restrictive in our context. Consider again the e-term graph on picture (4) above, and look 
once more at the application of the rule above
\[
	\xymatrix@R=6pt@C=25pt{\begin{tikzpicture}[baseline=(w.base)]\begin{pgfonlayer}{nodelayer}
				\node[style=node](v1) at (-0.9, 0.3){};
				\node[style=node](v2) at (-0.9, -0.3){};
				\node[style=medium box] at (0, 0){$/$};
				\node[style=none](divfst) at (-0.3, 0.3){};
...
\]
The match is regular mono, since it is an inclusion on the
underlying (pruned and separated) term graphs and an isomorphism 
on the equivalence classes, yet it does not belong to $\epbs_{\Sigma}$,
due to the occurrence of the constant $a$.
In fact, all the arrows of the diagram above are regular monos but none
is in $\epbs_{\Sigma}$.
%
%In fact, the restriction to regular mono matches seems
%adequate for rewriting in $\lepsBis$, since 

\subsubsection{Issue 2: Application conditions}
The fact that the left-hand side of a rule is the identity allows for its repeated application, hence the 
possibility to keep on performing the same rewriting step. 
This is problematic: If you consider e.g. the rule $x / x \to 1$, once $1$ has been added once to the equivalence class of 
$x / x$, it is not necessary to keep on adding it.
The issue is solved in the EGGs approach by suitable 
heuristics in the updating procedure. In the DPO approach, this is modelled by rules 
\emph{with negative application conditions}. Formally, an application condition is obtained 
by enriching the usual span 
$L \leftarrow K \rightarrow R$ with an additional arrow $n: L\rightarrow N$: A match $m: L \to G$ is admissible if it cannot
...
% since e.g. the two pictures below intuitively represent the same EGG
%\[xxx\]
%
The theory of $\mathcal{M}$-adhesivity carries on for linear rules with negative application conditions, as long as
the arrows of the span as well as the additional arrow $n: L\rightarrow N$ are in $\mathcal{M}$,
see e.g.~\cite{ehrig2014adhesive}.
%
In our context, we can still adopt the same approach with application condition, using them to forbid rewrites, 
yet the same considerations about $\mathcal{M}$-adhesivity with left-linear rules apply:
the framework of negative application conditions for left-linear rules is still an ongoing
venue of research.

\subsubsection{Issue 3: Removing duplicates}
The heuristics developed for the EGG framework ensures that
the chosen representation of a term graph 
is minimal: after each step, an housekeeping procedure 
is applied so that e.g. among the two pruned and separated 
e-term graphs below encoding the term $a/a$,  
the one on the left (again, picture (4) above)
is chosen
...
While it would be possible to obtain 
such a representation by suitable quotients on $\lelmBis$ 
and $\lepsBis$, we favour a solution which recalls the 
actual heuristics of the EGG formalism by relying on the 
classical solution adopted in the term graph literature, that is, the 
introduction of specific contraction rules~\cite{AriolaKP00}.
For example, for the simple algebraic signature containing only 
the constants $a$ and $1$ and the binary operator $/$, it suffices to introduces 
the three contraction rules below
\begin{gather*}
\hspace{.25cm}
\xymatrix{        
\begin{tikzpicture}[baseline=(nil.center)]\begin{pgfonlayer}{nodelayer}
		\node[style=small box] at (0, 0){$1$};
		\node[style=none](divfst) at (-0.3, 0.3){};
		\node[style=none](divsnd) at (-0.3, -0.3){};
		\node[style=none](divout) at (0.3, 0){};
		\node[style=node] (w) at (0.9, 0){};
	...
	\begin{pgfonlayer}{eqlayer}
		\draw[dashed, rounded corners](0.6, 0.3) rectangle (1.2, -1.3);
	\end{pgfonlayer}\begin{pgfonlayer}{background}
		\draw[color=white] (-.5, 0.2) rectangle (.5, -0.2);
\end{pgfonlayer}\end{tikzpicture}
	\ar@{=>}[r] &
	\begin{tikzpicture}[baseline=(nil.center)]\begin{pgfonlayer}{nodelayer}
			\node[style=small box] at (-1, -.4){$1$};
			\node[style=none](divout) at (-0.7, -.4){};
			\node[style=node] (w) at (-.1, -.4){};
		\end{pgfonlayer}        
		\begin{pgfonlayer}{edgelayer}
			\draw[->](divout.center) to (w);
		\end{pgfonlayer}
		\begin{pgfonlayer}{eqlayer}
			\draw[dashed, rounded corners](-0.4, -0.1) rectangle (0.2, -.7);
		\end{pgfonlayer}\begin{pgfonlayer}{background}
			\draw[color=white] (-.5, 0.2) rectangle (.5, -0.2);
	\end{pgfonlayer}\end{tikzpicture}} \quad \xymatrix{ \begin{tikzpicture}[baseline=(nil.center)]\begin{pgfonlayer}{nodelayer}
	\node[style=small box] at (0, 0){$a$};
	\node[style=none](divfst) at (-0.3, 0.3){};
	\node[style=none](divsnd) at (-0.3, -0.3){};
	\node[style=none](divout) at (0.3, 0){};
	\node[style=node] (w) at (0.9, 0){};
	\node[style=small box] at (0, -1){$a$};
	\node[style=none](one) at (0.3, -1){};
	\node[style=node](z) at (0.9, -1){};
	\node[style=none](nil) at (0, -0.5){};
	\end{pgfonlayer}        
...
\draw[color=white] (-.5, 0.2) rectangle (.5, -0.2);
\end{pgfonlayer}\end{tikzpicture}}\\ 
\xymatrix{	\begin{tikzpicture}[baseline=(divout)]
\begin{pgfonlayer}{nodelayer}
\node[style=node](w1) at (-0.9, -0.5){};
\node[style=medium box](a2) at (-2.3, 0){$/$};
\node[style=none] (a2att) at (-2.3, 0.5){};
\node[style=node](v1) at (-1.4, 0.5){};
\node[style=node](v2) at (-1.4, -0.5){};
\node[style=node](w2) at (-0.9, 0.5){};
...
\node[style=none](divfst) at (-0.3, -0.3){};
\node[style=none] (divsnd) at (-0.3, 0.3){};
\node[style=none] (divout) at (0, -0.5){};
\node[style=none] (divout2) at (-2.3, -0.5){};
\node[style=none](a2fst) at (-2, 0.3){};
\node[style=none] (a2snd) at (-2, -0.3){};
\node[style=node] (w) at (-0.9, -1.5){};
\node[style=node] (z) at (-1.4, -1.5){};
\end{pgfonlayer}
\begin{pgfonlayer}{edgelayer}
\draw[out=180, in=60] (v1) to (a2fst.center);
\draw[out=180, in=-60](v2) to (a2snd.center);
\draw[out=0, in=-120] (w1) to (divfst.center);
\draw[out=0, in=120](w2) to (divsnd.center);
\draw[->, out = -90, in = 0](divout.center) to (w);
\draw[->, out = -90, in = 180](divout2.center) to (z);
\end{pgfonlayer}
\begin{pgfonlayer}{eqlayer}
\draw[dashed, rounded corners] (-1.7, 0.2) rectangle (-0.6, 0.8);
\draw[dashed, rounded corners] (-1.7,-0.8) rectangle (-0.6, -.2);
\draw[dashed, rounded corners] (-1.7,-1.8) rectangle (-0.6, -1.2);
\end{pgfonlayer}\begin{pgfonlayer}{background}
\draw[color=white] (-.5, 0.2) rectangle (.5, -0.2);
\end{pgfonlayer}
\end{tikzpicture} \ar@{=>}[r] & \begin{tikzpicture}[baseline=(w)]
\begin{pgfonlayer}{nodelayer}
	\node[style=node](w1) at (-1.9, -0.5){};
	\node[style=node](w2) at (-1.9, 0.5){};
	\node[style=medium box](div) at (-1, 0){$/$};
	\node[style=none](divfst) at (-1.3, -0.3){};
	\node[style=none] (divsnd) at (-1.3, 0.3){};
	\node[style=none] (divout) at (-0.7, 0){};
	\node[style=node] (w) at (-0.1, 0){};
\end{pgfonlayer}
\begin{pgfonlayer}{edgelayer}
	\draw[out=0, in=-120] (w1) to (divfst.center);
	\draw[out=0, in=120](w2) to (divsnd.center);
...
\end{pgfonlayer}
\begin{pgfonlayer}{eqlayer}
	\draw[dashed, rounded corners] (-2.2, 0.2) rectangle (-1.6, 0.8);
	\draw[dashed, rounded corners] (-2.2, -0.8) rectangle (-1.6, -0.2);
	\draw[dashed, rounded corners] (-.4, -0.3) rectangle (0.2, 0.3);
\end{pgfonlayer}\begin{pgfonlayer}{background}
\draw[color=white] (-.5, 0.2) rectangle (.5, -0.2);
\end{pgfonlayer}
\end{tikzpicture}}
...
These rules can be performed in parallel with the 
rules of any rewriting system without interfering with them,
and ensures that a minimal representation can always be reached.

\subsubsection{Issue 4: Cycles}
Recall that DPO rules under consideration have the shape
$L \leftarrow L \rightarrow R$, where the left-hand side is the identity 
and the right-hand side arrow is a mono in $\lepsBis$.
%
%Despite their simplicity, though, such rules may raise some expressiveness issues.
%
Note that the the underlying term graphs underling the objects in $\lepsBis$
are by definition acyclic, so that for any match $m: L \to G$ either in $\lelmBis$ 
or $\lepsBis$, if $G$ is acyclic then also the result $H$ in acyclic.
%
Nevertheless, cycles may implicitly occur due to the presence of the equivalence 
on nodes, for example in what are called collapsing rules, i.e. where the right-hand side is a single variable. 
%
Looking again at the key example of~\cite{WillseyNWFTP21}, consider the rule $1 \times x = x$,
which can be modelled as the DPO rule below (whose right-hand side is a mono)
\[
\hspace{.25cm}
\xymatrix{        
	\begin{tikzpicture}[baseline=(w.base)]\begin{pgfonlayer}{nodelayer}
...
\]
%
Intuitively, the e-term graph obtained after the application of the rule seems to represent a recursive term such 
...
and that update steps are obtained via DPO rules (possibly with negative application conditions) whose 
left-hand side is in $\mathcal{M}$,
%, and in fact an identity. 
%and via a match that is in $\mathcal{N}$. 
and that paves the ways for exploiting the properties 
of the $\mathcal{M}$-adhesive framework.
% when discussing about the optimisation process.

\emph{Future works.}
Our results open a few threads of research. The first is to check how far the $\mathcal{M}$-adhesivity 
of EGGs can be pushed to model their updating via the DPO approach. We have seen that 
the rules adopted in the literature of EGGs appears to be spans whose left-hand side is an identity and 
right-hand side is a mono (possibly with negative application 
...
between the two proposals, which appear to be related yet quite different. 
%In~\cite{ghica}, equivalence of nodes is captured by a special kind of \emph{hierarchical} edges together with a partial order 
%a consistency relation, using a formalism which is reminiscent of hierarchical hypergraphs. Moreover, by considering term graphs 
%as arrows of a symmetric monoidal category enriched over semi-lattices,~\cite{ghica} connects e-hypergraphs with the theory 
%of PROPs. While~\cite{ghica}, as us, exploits DPO rewriting, it lacks an examination of the adhesivity properties of the category of e-hypergraphs studied.
%
%In fact, for both proposals the key intuition is  to use string diagrams to represents term graphs. 
%
We adopted a down-to-earth approach by directly equipping
the nodes of a term graph with an equivalence relation, thus mimicking the original presentation~\cite{DetlefsNS05},
at the same time extending and generalising~\cite{concur2006} to the contemporary jargon of adhesive categories. 
The route chosen in~\cite{ghica} is to consider a variant of hierarchical hypergraphs, with suitable conditions that 
intuitively ensure a correspondence between hierarchical edges and equivalence classes. They do not explore
$\mathcal{M}$-adhesivity, embedding instead their hypergraphs into categories of string diagrams with an enrichment 
over semi-lattices and proving a soundness and  completeness result, along the lines of~\cite{bonchi2022string}.
Even if we both consider the DPO approach for rewriting, our constructions are technically simpler, and we believe
more in tune with the set-theoretical approach of EGG, which facilitates its practical application 
and theoretical analysis, and aligns with the focus on $\mathcal{M}$-adhesivity
currently pursued by the graph transformation community.
...
\begin{proof}\label{proof:equi}
	...
	By construction $f_{k,l}= \iota^G_{k,l} \circ \eta_{(k,l)}$ and so $(\hat{\eta}, \eta_{\bullet})=(f,g)$ as wanted. 
		...
		\item We are left with essential surjectivity of $\mathcal{G}_{(-)}$. Given a hypergraph $\mathcal{G}=(E_\mathcal{G}, V_\mathcal{G}, s_\mathcal{G}, t_\mathcal{G})$ we can define 
...
\begin{proof}\label{proof:dx}
	\begin{enumerate}
\item Given a hypergraph $\mathcal{G}$, let $\iota_{\pr(\mathcal{G})}\colon V_{\pr(\mathcal{G})}\mto V_{\mathcal{G}}$ be the inclusion of non isolated nodes. By definition we know that there exists $s_{\pr(\mathcal{G})}, t_{\pr(\mathcal{G})}\colon E_{\mathcal{G}}\rightrightarrows V_{\pr(\mathcal{G})}$ such that
...
\item Let $\mathcal{G}$ be a hypergraph. For every $e\in E_{\mathcal{G}}$, let $n_e=\lgh_{V_{\mathcal{G}}}(s_{\mathcal{G}}(e))$ and $m_e=\lgh_{V_{\mathcal{G}}}(t_{\mathcal{G}}(e))$. Given $1\leq i\leq n_e+m_e$,  we can define $\delta^e_i\colon \mto V_{\mathcal{G}}$ as
...
Now let $(h,k)$ be another morphism $\mathcal{H}\to \sp(\mathcal{G})$ such that $(\id{E_{\mathcal{G}}}, \epsilon_{G})\circ S_p(h, k)=(f,g)$. This entails immediately that $h =f$ and so uniqueness follows from \Cref{rem:mprun}.	\qedhere 
	...
	Let $\mathcal{G}$ be a hypergraph. For every $e\in E_{\mathcal{G}}$, let $n_e=\lgh_{V_{\mathcal{G}}}(s_{\mathcal{G}}(e))$ and $m_e=\lgh_{V_{\mathcal{G}}}(t_{\mathcal{G}}(e))$. Given $1\leq i\leq n_e+m_e$,  we can define $\delta^e_i\colon \mto V_{\mathcal{G}}$ as
	\[\delta^e_i(0) =\begin{cases}
...
\begin{proof}\label{proof:pbmono}
	Closure under composition and decomposition follows from \Cref{lem:pb1}, while stability under pushout follows from the first point of \Cref{prop:kerset} because pushouts along injections are also pullbacks by \Cref{prop:regmono}. We are left with stability under pullbacks. 
...
\begin{proof}\label{proof:elim}
	...
	Take a limit $(L, (l_i)_{i \in \cat I})$ for $K \circ F$. By \Cref{prop:eqhyp_complete} there exists a mono $m\colon Q_{\mathcal{L}} \mto L$ such that $c_D = l_D \circ m$. Since $\D$ is connected then by \Cref{prop:fact} we know that $(L^\star, \{l^\star_D\}_{D\in \D})$ is a limiting cone. Computing we get
	\begin{align*}
	...
	\end{align*} 
	Thus we deduce that $m^\star(q_{\mathcal{L}}^\star(t_{\mathcal{L}}(e)))=m^\star(q_{\mathcal{L}}^\star(t_{\mathcal{L}}(e')))$. But by \Cref{prop:fact}, $(-)^\star$ sends monos to monos and so we can conclude.
\end{proof}
...
\begin{proof}\label{proof:ecim} Let $F\colon \D\to \egg$ be a connected diagram. Let $F(D)$ be $(E_D, V_D, Q_D, s_D, t_D, q_D)$ and suppose that $(\mathcal{L}, \{(a_D, b_D, c_D)\}_{D\in \D})$ is a limiting cone for $E\circ F$. If we show that $\mathcal{L}$ is an e-hypergraph we are done. Let $e, e'\in E_{\mathcal{L}}$ be such that $q^\star_{\mathcal{L}}(s_{\mathcal{L}}(e))=q^\star_{\mathcal{L}}(s_{\mathcal{L}}(e'))$. Thus for every $D\in \D$ we have:
	...
	\end{align*}
	and so we conclude that $q^\star_D(t_D(a_D(e)))=q^\star_D(t_D(a_D(e')))$. Thus $a_D(e)$ and $a_D(e')$ are $q_D$-joined and we can conclude by \Cref{lem:elimc}.
\end{proof}

\pbsuit*

\begin{proof}\label{proof:pbmonoplus}
	As in the case of \Cref{lem:pbmono}, closure under composition and decomposition follows from \Cref{lem:pb1} and stability under pushouts follows from the first point of \Cref{prop:kerset}. 
	
	Let now $h\colon \mathcal{G}_1\to \mathcal{G}_3$ be an arrow in $\pbp$ and $k\colon \mathcal{G}_2\to \mathcal{G}_3$ be another morphism of $\EqHyp$, with $\mathcal{G}_{i}=(E_i, V_i, Q_i, s_i, t_i, q_i)$.
	
	Let $r\colon \mathcal{P} \to \mathcal{G}_2$ and $p\colon \mathcal{P}\to \mathcal{G}_1$ be the projection of a pullback of $h$ along $k$. We have to show that $r$ is in $\pbp$. 
	
	By \Cref{lem:pbmono} we already know that it is in $\pbc$. Now, let $z_1\colon Z\to E_{2}$, $z_2\colon Z\to V_\mathcal{P}$ be two arrows such that 
	\[s_2\circ z_1=r_V\circ z_2\]
	
	Now, by \Cref{cor:mono1} and since $h$ belongs to $\pbp$, all the square below beside the left half of the right-most diagram are pullbacks.
	\[\xymatrix{E_{\mathcal{P}} \ar@/^.3cm/[rr]^{p_V\circ s_{\mathcal{P}}} \ar[d]_{r_E}\ar[r]_{p_E}& E_1 \ar[d]_{h_E} \ar[r]_{s_1} & V_1\ar[d]^{h_V} & E_{\mathcal{P}} \ar@/^.3cm/[rr]^{s_1\circ p_E}\ar[d]_{r_E} \ar[r]_{s_{\mathcal{P}}}& V_{\mathcal{P}} \ar[d]_{r_V} \ar[r]_{p_V} & V_1 \ar[d]_{h_V}\\E_2\ar@/_.3cm/[rr]_{k_V\circ s_{2}}\ar[r]^{k_E} & E_3 \ar[r]^{s_3} & V_3 & E_{2} \ar[r]^{s_2} \ar@/_.3cm/[rr]_{s_3\circ k_E}& V_2 \ar[r]^{k_V} & V_3}\]
	
	But by \Cref{lem:pb1} this last square is a pullback too, proving the thesis. 
\end{proof}
...
\begin{rem}\label{rem:kleene}
	Let $\delta_1\colon 1\to \mathbb{N}$ be the arrow which picks $1\in \mathbb{N}$. Let $f\colon X\to Y$ be an arrow and consider the inclusions $v_1\colon X\to X^\star $, $u_1\colon Y\to Y^\star $ defined by the first point of \Cref{prop:fact}. Then we can build the cube aside which, by extensivity, has pullbacks as back, front and right faces. Thus, by \Cref{lem:pb1} its left face is a pullback too.
	\end{rem}
\end{minipage}
\hfill
\begin{minipage}[r]{.45\linewidth}
\xymatrix@C=15pt@R=5pt{&X\ar[dd]|\hole_(.65){v_1}\ar@{->}[rr]_{!_X} \ar@{>}[dl]_{f} && 1 \ar[dd]^{v_1} \ar[dl]^{\id{1}} \\ Y  \ar[dd]_{u_1}\ar@{>}[rr]^(.7){!_Y} & & 1 \ar[dd]_(.3){\delta_1}\\&X^\star\ar@{>->}[rr]|(.53)\hole^(.70){\lgh_{X}} \ar@{>->}[dl]_{f^\star} && \mathbb{N} \ar[dl]^{\id{N}} \\Y^\star \ar@{>->}[rr]^(.7){\lgh_{Y}} & & \mathbb{N}}
\end{minipage}
...
\begin{proof}\label{proof:epo} To fix the notation, let $\mathcal{P} $ be $(E, V, Q, s, t, q)$ and, for $i\in \{1, 2, 3\}$ let $\mathcal{G}_i$ be $(E_{i}, V_i, Q_i, s_i, t_i, q_i)$.  Let, moreover, $(K_i, \pi_i^1, \pi_i^2)$
	...
	\begin{minipage}[l]{.7\linewidth}
			\setlength{\parindent}{1.5em}
		Consider now the cube on the right. Since $m$ is in $\pbp$ then the squares below are both pullbacks.
		\[\xymatrix{E_1 \ar@{>->}[r]^{m_E} \ar[d]_{s_1} & E_3 \ar[d]^{s_3} & V_1 \ar@{>->}[r]^{m_V} \ar@{>>}[d]_{q_1}& V_3 \ar@{>>}[d]_{q_3}\\ V_1 \ar@{>->}[r]^{m_V} & V_3 & Q_1 \ar@{>->}[r]^{m_Q} & Q_3}\]
	\end{minipage}\hfill 
	\begin{minipage}[r]{.4\linewidth}
	...
	\begin{minipage}[l]{.2\linewidth}
		\xymatrix@R=20pt{K_1\ar[r]_{k_{h_E}}\ar@{>->}[d]_{k_{m_E}}&K_2\ar@{>->}[d]^{k_{n_E}}\\K_3\ar[r]^{k_{z_E}}&U}
	\end{minipage} \hfill \begin{minipage}[r]{.8\linewidth}
		\setlength{\parindent}{1.5em}
	By \Cref{rem:kleene}, then, left face of the cube is a pullback too. Its bottom face is not a pushout but it is still a pullback by the first point of \Cref{prop:regmono} and the third one of \Cref{prop:fact}. But by hypothesis and the first point of \Cref{prop:eqhyp_complete} the top face is a pushout. Thus we can apply \Cref{prop:kerset} to deduce that the square on the left is a pushout.	
	\end{minipage}
...
\vspace{-.3cm}
	...
	We can therefore deduce that $q^\star \circ t \circ u_1 = q^\star \circ t \circ u_2$, and the claim now follows from \Cref{prop:eh}.
\end{proof}

\fh*
\begin{proof}\label{proof:fh} We proceed more or less as in the proof of \Cref{prop:eh}. Define
...
\begin{proof}\label{proof:varie}
	...
	\begin{enumerate}
		...
		Thus $a_D(e)$ and $a_D(e')$ are $q_D$-joined and we can conclude by \Cref{lem:elimc}.
		\item We are going to adapt the argument exploited in the proof of \Cref{lem:epo}.
		...
		\begin{minipage}[l]{.4\linewidth}
			\setlength{\parindent}{1.5em}
			Consider the cube on the right. By hypothesis and \Cref{rem:kleene} its left face is a pullback. Moreover its bottom face is not a pushout but it is still a pullback by the first point of \Cref{prop:regmono} and the third one of \Cref{prop:fact} and by the fact that, since limits commute with limits,  $(-)\times E_{\mathcal{G}}$  preserves pullbacks.
		\end{minipage}\hfill 
		\begin{minipage}[r]{.5\linewidth}
			\xymatrix@C=20pt@R=10pt{&E_1\ar[dd]|\hole_(.65){(q_1^\star \circ s_1, a_1)}\ar[rr]^{h_E} \ar@{>->}[dl]_{m_E} && E_2 \ar[dd]_{(q_2^\star \circ s_2, a_2)} \ar@{>->}[dl]_(.6){n_E} \\ E_3 \ar[dd]_{(q_3^\star \circ s_3, a_3)}\ar[rr]^(.7){z_E} & & E \ar[dd]_(.3){(q^\star \circ s, a)}\\&{Q_1^\star}\times E_{\mathcal{G}}\ar[rr]|\hole^(.66){h_Q^\star\times \id{E_{\mathcal{G}}}}\ar@{>->}[dl]_(.55){m_Q^\star\times \id{E_{\mathcal{G}}}} && {Q_2^\star\times E_{\mathcal{G}}} \ar@{>->}[dl]^{n_Q^\star\times \id{E_{\mathcal{G}}}} \\{Q_3^\star}\times E_{\mathcal{G}} \ar[rr]^(.5){z_Q^\star\times \id{E_{\mathcal{G}}}} & & Q^\star\times E_{\mathcal{G}}}	\end{minipage}
		...
		\end{minipage} \hfill \begin{minipage}[r]{.7\linewidth}
			\setlength{\parindent}{1.5em}
			By hypothesis and the first point of \Cref{prop:eqhyp_complete} the top face is a pushout. Thus we can apply \Cref{prop:kerset} to deduce again that the square on the left is a pushout.
		\end{minipage}
	...
	\end{align*}
		...
		From these identities we get that $q^\star \circ t \circ u_1 = q^\star \circ t \circ u_2$, and the claim follows from \Cref{prop:eh}. \qedhere 
	\end{enumerate}
\end{proof}

\section{Some properties of comma categories}
In this section we briefly recall the definition of the comma category \cite[Sec.~II.6]{mac2013categories} and some of its properties.
