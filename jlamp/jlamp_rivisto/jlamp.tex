...
while the right-hand side is a regular mono. The format precisely 
...
that is a regular mono in $\lepsBis$ (hence, also in $\lelmBis$). 
...
be spans of morphisms with no restriction, the match is required to belong to $\mathcal{M}$, which is 
...
$L \leftarrow K \rightarrow R$ plus an additional arrow $n: L\rightarrow N$: A match $m: L \to G$ is admissible if it cannot
...
%
In our context, we can still adopt the same approach with application condition, yet the same considerations
about $\mathcal{M}$-adhesivity with left-linear rules apply.

\subsubsection{Issue 3: Cycles [DA AGGIUSTARE! Ma la regola \`e regular mono?]}
...
%
The underlying term graphs of the rules are obviously acyclic, and since the right-hand side 
is a regular mono, if it is 


ensures that if the e-hypergraph to whom a rule is applied is acyclic,
