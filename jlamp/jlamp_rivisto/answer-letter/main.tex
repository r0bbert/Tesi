...
Besides these changes, Reviewer 2 raised a serious technical issue with our previous Lemma 5.7 (now Lemma 5.9).  Indeed, the lemma was false in its previous form. The issue is that there is no reason for the left face of the cube used in the proof to be a pullback. We resolved this issue by imposing an additional condition on the class $\mathsf{Pb}$ (see new Definition 5.7).
...
\begin{itemize}
	...
	This is well known. An hypergraph is a pair of arrows $E \to V^\star$, or equivalently, an arrow $E \to V^\star \times V^\star$, that is an object of the comma category. We have added a reference.
	
	
	\item \emph{ p.8, Lem.3.17 is a bit hard to parse and digest. Can you make
		it more accessible?}
	
	Due to the technical nature of the lemma we have decided to keep it in the current form. 
	...
	We moved Rem 3.49 to the appendix (now is Rem. A.6). However we prefer to leave Rem 3.42 in the main body.
	...
	\item \emph{ p.12. Prop.3.50: What do $Pr$ and $sp_P$ do? (pr=pruning?)}
		...
		The preceding sections contain results needed for this section. Similarly, Sec. 5 builds upon the results established in Sec. 4.
		...
		Kernel pairs are quite a standard tool in the categorical study of relations. We introduced them in Sec. 2, alongside the other general categorical preliminaries.
		...
		Rewrote paragraph on comparison between the two works.
