...
\todo{Di questo possiamo sbattercene perché tanto siamo su $\Set$}
...
In this section we will briefly recall the definition of the comma category associated to two functors and some of its properties.
...
We can notice another useful fact,  showing that in some cases we can guarantee  the existence of a left adjoint to $U_R$. 
...
\begin{definition}
	Let $(\X, \otimes, I)$ be a monoidal category. An \emph{monoid} is a triple $(X, m, e)$ made by an object $X$ equipped with arrows $m\colon X\otimes X\to X$ and $e\colon I\to X$ making the following diagram commute.
	
	\[\]
	
	A \emph{morphism of monoids} between $(X, m, e)$ and $(X', m', e')$ is an arrow $f\colon X\to X'$ fitting in the diagram below.
...
We are interested in provide sufficient conditions to guarantee the existence of a left adjoint to $\mo{U}$. The idea is to mimic the construction of the \emph{kleene star} functor $\Set\to \Set$ \cite{bibid}, i.e.~ the functor sending $X$ to the set of finite words over it.
...
\begin{theorem}
	Let $(\X, \otimes, I )$ be a countably distributive monoidal category. Then $\mo{U}$ has a left adjoint sending $\mo{F}$ to $\sum_{n\in \mathbb{N}} X^{\otimes n}$.
\end{theorem}

As a concluding step, we ask for a compatibility property between countable coproducts and pullbacks, to assure that $\mo{F}$ preserves pullbacks.

\begin{definition}
	Let $\X$ be a category and suppose that $(X, \{j_i\}_{i\in I})$ is a coproduct. We say that $(X, \{j_i\}_{i\in I})$ is \emph{stable under pullbacks} if, given an arrow $f\colon Y\to X$ such that for every $i\in I$ the pullback square below exists,  the cocone $(Y, \{k_i\}_{i\in I})$ is a coproduct.
	\[\xymatrix{Y_i \ar[r]^{k_i} \ar[d]_{p_i} & Y \ar[d]^{f}\\ X_i\ar[r]_{j_i} & X}\]
...
\begin{proposition}
	Let $(\X, \otimes, I)$ be a countably distributive monoidal category and suppose that countable  coproducts in $\X$ are stable under pullbacks. Then the functor $\mo{F}\colon \X\to \mono{X}$ preserves pullbacks.
\end{proposition}
\begin{proof}
	contenuto...
\end{proof}
