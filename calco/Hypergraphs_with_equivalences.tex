...
\subsubsection{Hierarchical hypergraphs with equivalences}
...
\begin{proof}
	...
	Let $D: \cat I \to \EqHyp$ be a diagram, and let $((c_i)_{i\in \cat I}, \mathcal{C})$ be the colimit of $Q \circ D$, where $\mathcal{C} = (A, C, q\circ s, q\circ t)$, and $D(i)$ is $(A_i, B_i, C_i, s_i, t_i, q_i)$.
	Let now $T: \EqHyp \to \Set$ be the functor mapping each graph with equivalence onto its second component, $T((X, Y, Z, x, y, z)) = Y$, and each morphims onto its second component.
	...
	Consider now the functor $W: \EqHyp \to \Set$ mapping each $(X, Y, Z, x, y, z)$ onto $X$, and each morphism on its first component. By \Cref{prop:eqgrph_complete} and \Cref{lemma:limits_of_presheaves}, we have that $((c_E^i)_{i \in \cat I}, A)$ is the colimit of $W \circ D$.
