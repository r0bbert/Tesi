...
\parbox{7.5cm}{\begin{restatable}{lemma}{mps}\label{prop:kerset}
	Suppose that in $\Set$ the cube in the diagram on the left is given, whose top face is a pushout, the left and bottom faces are pullbacks,  and $n\colon B\mto D$ is an injection. 
	Then it holds:
	\begin{enumerate}
...
Note that by hypothesis $(-)^\star$ preserves pullbacks, while $\pro$ is continuous by definition, thus the result below follows from  \Cref{thm:slice-functors} and \Cref{prop:com}.   
...
and edges, respectively, such that  $h_1, h_2$ have no source and $h_3, h_4$ a pair nodes
	\begin{center}
        \begin{tikzpicture}
...
%
We define the \emph{hypergraph $\mathcal{G}_\Sigma$ associated with $\Sigma$} taking $O_\Sigma$ as set of hyperedges, $\{1\}$ as set of nodes, so that $\{1\}^\star$ is $\mathbb{N}$, $\ari_\Sigma$ as the source function and $\gamma_1$ as target function, which always picks the element $1$. The category $\hyps$ of \emph{algebraically labelled hypergraphs} is the slice category $\hyp/\mathcal{G}^\Sigma$.
\end{definition}
...
Term graphs have been adopted as a convenient way to represents terms over a signature with an explicit sharing of sub-terms,
...
Notice that there is another functor $K: \EqHyp \to \Set$ sending $(E, V, Q, s, t, q)$ to $Q$, and a morphism $(h_E, h_V, h_Q)$ to $h_Q$. We  exploit it to compute limits and colimits in $\EqHyp$. In order to do so. A full proof of the following lemma can be found in \Cref{proof:comp}.
...
\end{definition}
	...
	\parbox{2cm}{
	     \xymatrix@R=18pt{
	        V_\mathcal{G} \ar@{>>}[d]_{q_\mathcal{G}} \ar@{>->}[r]_{h_V}& V_\mathcal{H} \ar@{>>}[d]^{q_\mathcal{H}} \\
	        Q_\mathcal{G} \ar@{>->}[r]^{h_Q} & Q_\mathcal{{H}}
              }
         }   
...
We now show that key lemma for $\pbc$-adhesivity of $\EqHyp$ is. A proof is in \Cref{proof:pbstable}.
...
are denoted by encircling nodes with a dotted line. All the arrows are regular mono. It is immediate to see that the bottom face is a pushout and the side faces are pullback. Unfortunately, 
the top face fails to be a pushout
  \end{minipage}\hfill
\begin{minipage}[r]{.60\linewidth}
                \xymatrix@C=10pt@R=6pt{
                &\emptyset \ar@{>->}[dd]|\hole \ar@{>->}[rr] \ar@{>->}[dl] &&
...
Let $(\mathcal{H}, l)$ be an object of $\EqHyp_\Sigma$, then, since $T$ has a right adjoint $R$ by \Cref{prop:forghyp}, $l\colon T(\mathcal{H})\to \mathcal{G}^\Sigma$ correponds to the arrow $(l, !_{Q_{\mathcal{H}}})\colon \mathcal{H}\to R(\mathcal{G}^\Sigma)$. It is immediate to see that this correspondence extends to an equivalence with the slice over $R(\mathcal{G}^\Sigma)$.

\begin{proposition}\label{prop:slice}
$\EqHyp_\Sigma$ is equivalent to $\EqHyp/R(\mathcal{G}^\Sigma)$.
\end{proposition}

Let $V_\Sigma\colon \EqHyp_\Sigma\to \EqHyp$ be the functor forgetting the labelling and $\pbc_\Sigma$ the class of morphisms in $\EqHyp_\Sigma$ whose image in $\EqHyp$ is in $\pbc$. From the second point of \Cref{thm:slice-functors,prop:eqhyp_complete,lim} we can deduce the following.
...
\end{remark}}\hfill \parbox{4cm}{\xymatrix@R=15pt{\EqTG_{\Sigma} \ar[r]_{J_\Sigma}  \ar[d]_{S_\Sigma}& \EqHyp_\Sigma \ar[d]^{T_\Sigma} \\ \tg \ar[r]^{I_\Sigma}& \hyp_{\Sigma}}}
...
\section{EGGs}
\label{eggs}
The previous section proved some adhesivity property for the categories $\EqHyp$ and $\EqTG_{\Sigma}$. We extends these results ito
...
We start introducing the notion of \emph{e-hypergraphs}, hypergraphs equipped with an equivalence relation which is closed under the target arrow:
in other words, whenever the relation identifies the source of two hyperedges, it identifies their targets too.

\begin{definition}\label[definition]{def:ehyp1}
...
A first result that we need is that limits in $\egg$ are computed as in $\EqHyp$. Full proof are in \Cref{proof:elim}.
...
\begin{corollary}\label{cor:ereg}
	If an arrow $h: \mathcal{G \to H}$ in $\egg$ is a regular mono in $\egg$ then $I(h)$ is so in $\EqHyp$. 
\end{corollary}
...
\parbox{11.5cm}{\begin{restatable}{lemma}{epo}\label{lem:epo}
	Suppose that the square on the right is a pushout in $\EqHyp$ with $m$ an arrow in $\pbc$. If $\mathcal{G}_1$, $\mathcal{G}_2$ and $\mathcal{G}_3$ are e-hypergraphs, then $\mathcal{P}$ is an object of $\egg$ too.
\end{restatable}}\hfill 
\parbox{4cm}{\xymatrix@R=15pt{\mathcal{G}_1 \ar[r]^{h}\ar[d]_{m}&\mathcal{G}_2\ar[d]^{n}\\\mathcal{G}_3\ar[r]_{z}&\mathcal{P}}}

Let now $\pbe$ be the class of arrows in $\egg$ which are sent to $\pbc$ by $I$. By definition and \Cref{lem:pbmono} such class is closed under composition, decomposition, contains all isomorphisms and it is stable under pullbacks. Using \Cref{lem:pbmono,lem:epo} we can also deduce that it is stable under pushouts. Then \Cref{thm:slice-functors,cor:eqade} yield the following.
...
We turn now to the labelled context.

\begin{definition} We say that an object $(\mathcal{H}, l)$ of $\EqHyp_\Sigma$ is a \emph{labelled e-hypergraph} if $\mathcal{H}$ is an e-hypergraph. We define the category $\egg_\Sigma$ as the full subcategory of $\EqHyp_\Sigma$ given by labelled e-hypergraphs. We will denote by $j_\Sigma$ the corresponding inclusion functor .
\end{definition}

To prove some adhesivity property of $\egg_\Sigma$ let us begin with the following elementary, but useful observation.
...
\begin{remark}\label{rem:algegg}
	...
	equipped with the two projections.  Onthe other hand $\id{\mathbb{N}} \circ \gamma_1 \circ \pi_1$ and $\id{\mathbb{N}}\circ \gamma_1 \circ \pi_2$ are both the function $O_\Sigma \to \mathbb{N}$ constant in $1$.
\end{remark}

\Cref{rem:algegg} now implies at once the following result.
\begin{proposition}\label{prop:varie}
	Let $\Sigma$ be an algebraic signature. Then the following hold true:
	\begin{enumerate}
		\item $\egg_\Sigma$ is equivalent to $\egg/R(\mathcal{G}^\Sigma)$.
		\item There exists a functor $W_\Sigma \colon \egg_\Sigma\to \Sigma$ forgetting the labeling which creates all colimits, pullbacks and equalizers. 
	\end{enumerate}
\end{proposition}
...
By \Cref{thm:slice-functors} we get the following.
...
\begin{definition}\label[definintion]{def:GEqTGs}
	Given a signature $\Sigma$,  we say that an object $(\mathcal{H}, l)$ of $\EqTG_{\Sigma}$ is a \emph{e-term graph} if $\mathcal{H}$ is an e-hypergraph. We define the category $\eg$ as the full subcategory of $\EqTG_{\Sigma}$ given by e-term graphs and denote by $k_\Sigma$ the corresponding inclusion.
\end{definition}

\begin{remark}
	By definition we also have an inclusion functor $K_\Sigma\colon \eg\to \egg_\Sigma$.
\end{remark}

\noindent 
\parbox{7.5cm}{\begin{remark}
	Now that we have introduced all the characters of this work, it is worth to give a visual map of all the categories that we have introduced in this paper and of the relationships between them. We will use the curved arrows to denote full and faithful inclusions.
\end{remark}}\hfill \parbox{4cm}{\xymatrix@R=15pt@C=15pt{\eg \ar@{^{(}->}[r]^-{K_\Sigma} \ar@{^{(}->}[d]_{k_\Sigma} & \egg_\Sigma \ar@{^{(}->}[d]^{j_\Sigma} \ar[r]^{W_\Sigma}& \egg \ar@{^{(}->}[d]^{I}\\ \EqTG_{\Sigma} \ar@{^{(}->}[r]_{J_\Sigma}\ar[d]_{S_\Sigma}& \EqHyp_\Sigma \ar[r]_{V_\Sigma} \ar[d]_{T_\Sigma}& \EqHyp \ar[d]_{T}\\ \tg \ar@{^{(}->}[r]_{I_\Sigma}& \hyp_{\Sigma} \ar[r]_{U_\Sigma}  &\hyp}}


We can now prove our last three results. The interested reader can find the proof in \Cref{proof:tlim}.

\begin{restatable}{proposition}{limt}\label{prop:limt}
	$\eg$ has equalizer, binary products and pullbacks and they are created by $k_\Sigma$.
\end{restatable}
...
\commentato{ 
\begin{remark}\label[remark]{rem:sig_egg}
	Given a signature $\Sigma = (O_\Sigma, \ari_\Sigma)$,
	the hypergraph 
	$R(\mathcal{G}^\Sigma) = (O_\Sigma, 1, 1, \ari_\Sigma, \delta_1, \id{1})$
	is an object of $\egg$.
	Indeed, if $(S, \pi_1, \pi_2)$ is the kernel pair of $\id{1}^\star \circ \ari_\Sigma$,
	we have that $\id{1}^\star \circ \delta_1 \circ \pi_1$ and $\id{1}^\star \circ \delta_1 \circ \pi_2$ are both the contant functions of value $1$.
\end{remark}

\begin{definition}\label[definition]{def:label_egraphs}
	Let $\Sigma$ be an algebraic signature, and $\mathcal{G}^\Sigma$ the hypergraph associated to it.
	A \emph{labeled e-hypergraph} is a pait $(\mathcal{E}, l)$, where $\mathcal{E}$ is an e-hypergraph and $l: T \circ I (\mathcal{H}) \to \mathcal{G}^\Sigma$ is a morphism of $\hyp$.
	A \emph{morphism of labeled e-hypergraphs} $(\mathcal{E}, l)\to (\mathcal{E}', l')$ is a morphism $h: \mathcal{E \to E}'$ such that $l = l' \circ (T \circ I)(h)$.
	We denote the resulting category by $\eggs$.
\end{definition}

%Functor from GEqhyp_S to EqHyp_S

With a similar argument as the one done in \Cref{prop:slice}, and by \Cref{rem:sig_egg} we get a characterization of $\eggs$.

\begin{proposition}
	The category $\eggs$ is equivalent to $\egg/R(\mathcal{G}^\Sigma)$
\end{proposition}

Consider now the functor that forgets the label $V_\eq : \eggs \to \egg$, and $\pbes$ the class of morphisms in $\eggs$ whose image through $V_\eq$ is in $\pbe$. From \Cref{thm:slice-functors}, \Cref{lim},  \Cref{lem:elim,lem:epo}, we can deduce what follows.

\begin{proposition}\label[proposition]{prop:eadh}
	The following facts hold true:
	\begin{enumerate}
	\item $\eggs$ has all limits and pushouts along regular monos, which are created by $V_\eq$;
	\item $\eggs$ is $\pbes$-adhesive.
	\end{enumerate}
\end{proposition}

Now, by \Cref{cor:rege,cor:mono1,cor:mono2}, we get what follows.

\begin{corollary}\label{cor:emon}
	Let $h = (h_E, h_V, h_Q)$ be an arrow in $\eggs$. Then the following holds.
	\begin{enumerate}
	\item $h$ is mono if and only if $h_E$ and $h_V$ are injective;
	\item $h$ is regular mono in and only if each component of it is injective.
	\end{enumerate}
\end{corollary}

We can now introduce term graphs with closed equivalences.

%\begin{definition}\label[definintion]{def:GEqTGs}
%	Given a signature $\Sigma$, we define the category $\GEqTGs$ as the full subcategory of $\eggs$ in which objects are arrows $l: \mathcal{G} \to R(\mathcal{G}^\Sigma)$ where $t_\mathcal{G}$ is mono. The inclusion functor is denoted $I_{\GEqTGs} : \GEqTGs \to \egg/R(\mathcal{G}^\Sigma)$.
%\end{definition}

\begin{definition}\label[definition]{def:GEqTGs}
	Let $\Sigma$ be an algebraic signature. An object $(\mathcal{T}, l)$ of $\eggs$ is an \emph{e-term graph} if $l: T(\mathcal{T})\to \mathcal{G}^\Sigma$ is a term graph. We denote by $\GEqTGs$ the full subcategory of $\eggs$ so defined, and denote by $I_{\GEqTGs}$ the corresponding inclusion functor.
\end{definition}
\noindent
\parbox{8.5cm}{\begin{remark}\label[remark]{rem:inclusions}
	The category $\GEqTGs$ is also a full subcategory of $\EqTGs$, since each object $(\mathcal{H}, l)$ is a term graph with equivalence too.
	We denote the corresponding inclusion functor as $S_\eq$. Hence, we have a situation similiar to the one in \Cref{rem:obv}, as the diagram aside shows, where $T_\eq$ is the inclusion functor of \Cref{def:ehyp1} lifted to the slice category, i.e., is such that $V_\Sigma \circ T_\eq = I \circ V_\eq$.
\end{remark}}
	\hfill\parbox{4.5cm}{\[\xymatrix@R=20pt@C=38pt{\GEqTGs \ar[r]^{I_{\GEqTGs}}  \ar[d]_{S_\eq}& \eggs \ar[d]^{T_\eq} \\ \EqTGs \ar[r]_{I_\eq}& \EqHyp_{\Sigma}}\]}
...
It can be modelled as the DPO rule aside, concisely represented as the arrow $L \rightarrow R$,
which can be applied with a match that is injective on equivalence classes to the legal EGGs
above.
\end{minipage}\hfill
\begin{minipage}[r]{.60\linewidth}
\xymatrix{        
...
\noindent
	...
	To do so, it suffices to build the cokernel pair of the three arrows, taking the pushout of $h_E$, $h_V$ and $h_Q$ along itself, obtaining the three diagrams below, which, by \Cref{prop:regmono}, are also pullbacks.
	\[     \xymatrix@R=15pt{
	...
	Now, in $\Set$ every mono is regular \cite{maclane2012sheaves}, so by the dual of \Cref{prop:reg_epi_coeq} we deduce that $h_E$, $h_V$ adn $h_Q$ are the equalizers of the arrows built above. By \Cref{prop:com,lim} we also know that $(h_E, h_V)$ is the equalizer in $\hyp$ of $(f_E, f_V)$ and $(g_E, g_V)$.  Hence, if $(z_E, z_V, z_Q)\colon \mathcal{Z}\to \mathcal{H}$ is an arrow such that 
...
\begin{proof}\label{proof:pbmono}
	...
	Again by \Cref{prop:eqhyp_complete} the two halves of the central rectangler are pullbacks and so, by \Cref{lem:pb1} the whole rectangle is a pullback too. This, in turn entails that the the whole right rectangle is a pullback and so, also its left half is a pullback (again by \Cref{lem:pb1}).
	\[
...
\parbox{5.2cm}{\hspace{15pt}Now, notice that  $m_3$ and $m_2$ are monos because $c_Q$ and $b_2$ are injections. By the proof of \Cref{prop:eqhyp_complete}, to conclude it is enough to show that
	\[m_3\circ q'_3 = a_3 \qquad m_2\circ q'_2=a_2\]
		...
		\hspace{15pt}Indeed, if the previous equations hold, then $Q'_3$ and   $Q'_2$ are images for $a_3$ and $a_2$ and the thesis follows from \Cref{prop:eqhyp_complete}.
...
\begin{proof}\label{proof:elim}
	...
	Let now $(A, B, Q, s, t, q)$, together with projections $(\pi_E^d, \pi_V^d, \pi_Q^d)_{d\in \D}$ be the limit of $I \circ F$. Suppose that $(U, u_1, u_2)$ is the kernel pair of $q^\star\circ s$ and let $(L, (l_i)_{i \in \cat I})$ be the limit of $K \circ I \circ F$. If we show that it lies in $\egg$ we are done.
...
\end{align*}}\hfill \parbox{4cm}{\vspace{-1cm}	\xymatrix@R=20pt{
	...
	By universal property of limits, we have that \- $m^\star\circ q^\star \circ t \circ u_1 = m^\star \circ q^\star \circ t \circ u_2$. Since $m$ is mono we deduce that $q^\star \circ t \circ u_1 = q^\star \circ t \circ u_2$, hence the thesis.
\end{proof}
...
\hfill \parbox{11.5cm}{ \vspace{-.25cm}\hspace{15pt}By hypothesis and the first point of \Cref{prop:eqhyp_complete} the top face is a pushout. Thus we can apply \Cref{prop:kerset} to deduce that the square on the left is a pushout.}
	...
	\end{align*}
	We can therefore deduce that $q^\star \circ t \circ u_1 = q^\star \circ t \circ u_2$, and the thesis follows.
\end{proof}


\limt*
\begin{proof}\label{proof:tlim}
Let $F\colon \D\to \eg$ be a diagram of one of the sapes mentioned in the proposition. Let $((\mathcal{L}, l), \{\pi_d\}_{d\in \D})$ be a limiting cone for $k_\Sigma \circ F$. By \Cref{lim,prop:term,proof:elim} we know that $\mathcal{L}$ is an e-termgraph and we can conclude.
\end{proof}
...
\end{lemma}
\begin{proof}
	Suppose that $U_L\circ F$ and $U_R\circ F$ have colimiting cocones  $\left(A, \{a_D\}_{D\in \D}\right)$ and $\left(B, \{b_D\}_{D\in \D}\right)$ respectively. By hypothesis $\left(L(A), \{L\left(a_D\right)\}_{D\in \D}\right)$ is colimiting for $L\circ U_L\circ F$. Now, if we define $F(D):=(A_D, B_D, f_D)$
...
\end{proposition}
\begin{proof} For an object $B\in \B$ we can define $\Delta(B)$ as $(0, B, ?_{B})$, where $0$ is an initial object in $\A$ and $?_{R(B)}$ is the unique arrow $L(0)\to R(B)$. Consider $\id{B}\colon B\to U_R(\Delta(B))$ be the 
	
	\noindent 
	\parbox{10cm}{\vspace{-.2cm}identity, and suppose that a $k\colon B\to U_R(A, B', f)$ in $\B$ is given. By initiality of $0$, there is only one arrow $?_A\colon 0\to A $ in $\A$ and, since $L$ preserves initial objects, the square aside commutes.}\hfill 
...
For eery object $\X$, $\X/X$ has all colimits and connected limits that $\X$ has. Moreover such limits and colimits are created by $\dom_X$.

In particular, if $\X$ has pullbacks, equalizers or pushouts, then for every object $X$, the slice $\X/X$ has such limits and colimits too.
\end{corollary}
