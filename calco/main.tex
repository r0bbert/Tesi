...
\section{$\mathcal{M}$-adhesive categories}\label{sec:ade}
...
\begin{restatable}{proposition}{regmono}\label[proposition]{prop:regmono}
...
In this section we recall the definition and some useful properties of \emph{kernel pairs} which will be useful in the next sections. 

\noindent 
\parbox{11cm}{
\begin{definition}
    A \emph{kernel pair} for an arrow $f\colon A \to B$ is an object $K_f$ together with two arrows $\pi^1_f, \pi^2_f\colon K_f \rightrightarrows A$, denoted as $(K_f, \pi^1_f, \pi^2_f)$, such that the following square is a pullback.
\end{definition}}\hfill 
\parbox{2cm}{\xymatrix@R=15pt{K_f \ar[r]_{\pi^1_f} \ar[d]_{\pi^1_f}& A \ar[d]^f \\ A \ar[r]^{f}& B}}
...
Together with \Cref{lem:pb1}, the two previous remarks while elementary, allow us to prove the following result.

\begin{proposition}\label{cor:kermono}
	Let $f\colon X\to Y$ be an arrow and $m\colon Y\to Z$ be a mono. If
	$(K_f, \pi_f^1, \pi_f^2)$ is a kernel pair for $f\colon X\to Y$, then it is also a kernel pair for $m\circ f$.
\end{proposition}

We are now going to explore some further properties of kernel pairs 

\begin{restatable}{lemma}{kpp}\label{lemma:kern_pairs_pres_pullbacks}
	Let $f\colon X \to Y$ and $g\colon Z \to W$ be two arrows admitting kernel pairs and suppose that the solid part of the three squares below is given. Then there exists a a unique arrow $k_h: K_f \to K_g$ compliting them.
    ...
    Moreover, if the leftmost is a pullback, then also the other ones are so.
\end{restatable}
...
\noindent
\parbox{7cm}{
\begin{restatable}{lemma}{mpo}\label{lem:mpo}
...
As a final step, we explore the relationship between regular epimorphisms and kernel pairs.

\begin{restatable}{proposition}{epic}\label{prop:reg_epi_coeq}
    Let $e\colon X \to Y$ be a regular epimorphism in a category $\X$ with a kernel pair $(K_e, \pi^1_e, \pi^2_e)$. Then, $e$ is the coequalizer of $\pi^1_e$ and $\pi^2_e$.
\end{restatable}
[Proof in \Cref{epic-proof}]

\begin{restatable}{corollary}{natepi}\label{cor:reg_epi_components_reg_epi_nat_trans}
    Let $\X$ be a category with pullbacks and $\phi\colon F \dot\to G$ be a natural transformation between two functors $F, G: \D \rightrightarrows \X$. If $\phi_d$ is a regular epi for every $d$ in $\D$, then $\phi$ is a regular epi.
\end{restatable}
[Proof in \Cref{natepi-proof}]

From the previous result we can now deduce that the class of regular epis is closed under colimits, as shown by the next lemma
\begin{restatable}{lemma}{epicol}\label{lemma:nat_trans_reg_epi_canonical_arrow_reg_epi}
    Let $F,G'\colon \D\rightrightarrows \X$ be two diagrams, and suppose that $\X$ has all colimits of shape $\D$. Let $(X, \{x_d\}_{d \in \D})$ and $(Y, \{y_d\}_{d\in \S})$ be, respectively, the colimit of $F$ and $G$.  If $\phi\colon  F \to G$ is a natural transformation in which all components are regular epimorphisms, then, the arrow induced by $\phi$ from $X$ to $Y$ is a regular epimorphism too.
\end{restatable}
...
This section contains the proofs which are omitted from the main body oof the paper. We begin stating  a well-known fact about composition and decomposition of pullbacks \cite{mac2013categories}.
...
\noindent 
	...
	We want to show that $\phi$ coequalizes $a$ and $n$. Let thus $h\colon X \to Z$ be an arrow such that $h \circ a = h \circ b$. Then, for every $d$, we have
	...
	Since $\phi_d$ is a regular epimorphism, we have $k' \circ y_d = h_d$ allowing us to conclude.
\end{proof}
