...
\Copyright{R.~Biondo, D.~Castelnovo, F.~Gadducci}

\ccsdesc[500]{Theory of Computation~Models of computation}
\ccsdesc[500]{Theory of Computation~Semantics and reasoning}

\keywords{Adhesive categories, EGGS} %TODO mandatory; please add comma-separated list of keywords
...
and investigate some simple results for their relationship with kernel pairs.
...
\subsection{Background on $\mathcal{M}$-adhesivity}\label{subsec:ade}
...
\subsection{Some Results on Kernel Pairs and Regular Epis}
...
\Cref{thm:slice-functors}, \Cref{lem:reg} and \Cref{prop:push} allow us to recover the following result, previously proved by direct computation in \cite[Thm.~4.2]{corradini2005term}.
\begin{corollary}
...
\todo{da fare? lasciare per una versione estesa?}
