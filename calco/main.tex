...
Notice that there is another functor $K: \EqHyp \to \Set$ sending $(E, V, C, s, t, q)$ to $C$, and a morphism $(h_E, h_V, h_C)$ to $h_C$. We  exploit it to compute limits and colimits in $\EqHyp$. In order to do so. A full proof of the following lemma can be found in \Cref{proof:comp}.
...
Consider a cube as the one below on the left, with regular monos as vertical arrows,, with pullbacks as back faces, pushouts as bottom and top faces and such that $h$ is in $\pbc$.
In lieu of \Cref{lemma:stab}, if we show that 	the front faces are pullbacks too we can conclude.

To fix the notation, let $\mathcal{G}_i = (A_i, B_i, Q_i, s_i, t_i, q_i)$, $\mathcal{G}'=(A_i', B_i', Q_i', s_i', t_i', q_i')$, for $i = 1, 2, 3, 4$. By \Cref{prop:eqhyp_complete} and \Cref{cor:mono2} we know that the central and right cube below have pushouts as bottom faces and pullbacks as back faces,
...
\begin{definition}
	Let $\Sigma$ be an algebraic signature and $\mathcal{G}^{\Sigma}$ the hypergraph associated to it. A \emph{labelled hypergraph with equivalence} is a pair $(\mathcal{H}, l)$ where $\mathcal{H}$ is an object of $\EqHyp$ and $l$ a morphism $T(\mathcal{H})\to \mathcal{G}^\Sigma$ of $\EqHyp$. A \emph{morphsim of labelled hypergraphs with equivalence} between $(\mathcal{H}, l)$ and $(\mathcal{H}', l')$ is an arrow $h\colon \mathcal{H}\to \mathcal{H}'$ such that $l= l'\circ T(\mathcal{H})$.
	
	We will denote the resulting category by $\EqHyp_\Sigma$.
\end{definition}
...
$V_\Sigma\colon \EqHyp_\Sigma\to \EqHyp$ be the functor forgetting the labelling and let $\pbc_\Sigma$ be the class of morphisms in $\EqHyp_\Sigma$ whose image in $\EqHyp$ is in $\pbc$. From the second point of \Cref{thm:slice-functors,prop:forghyp,lim} we can deduce the following.

\begin{proposition}\label{prop:lim}The following facts hold true:
	\begin{enumerate}
		\item $\EqHyp_\Sigma$ has all colimits and all connected limits, which are created by the forgetful functor $V_{\Sigma}\colon \EqHyp_\Sigma\to \EqHyp$, in particular it has all equalizers and pullbacks;
...
\begin{corollary}\label{prop:monos_in_eqhyps} Let $h=(h_{E}, h_V, h_Q)$ be an arrow in $\EqHyp_\Sigma$, then:
	\begin{enumerate}
		\item $h$ is mono if and only if $h_E$ and $h_V$ are injective;
		\item $h$ is a regular mono if and onky if $h_E$, $h_V$ and $h_Q$ are injective.
	\end{enumerate}
\end{corollary}
...
\begin{definition}Let $\Sigma$ be an algebraic signature.
	An object $(\mathcal{H}, l)$ of $\EqHyp_\Sigma$ is a \emph{term graph with equivalence} if $l\colon T(\mathcal{H})\to \mathcal{G}^\Sigma$ is a term graph. We will denote by $\EqTG_{\Sigma}$ the full subcategory of $\EqHyp_\Sigma$ so defined and by $I_{\eq}$ the correspoding inclusion functor.
 \end{definition}
...
The following proposition is now an easy corollary of \Cref{prop:push,prop:lim,rem:obv}. We provide the details in \Cref{proof:tade}.
...
while the second component may not belong to $\mathcal{M}$. 
...
\begin{proof}\label{proof:regmono}
	...
	\parbox{2cm}{
	     \xymatrix@R=18pt{
	        Q_\mathcal{G} \ar@{>->}[d]_{h_Q} \ar@{>->}[r]^{h_Q}& Q_\mathcal{H} \ar@{>->}[d]^{g_Q} \\
	        Q_H \ar@{>->}[r]_{f_Q} & Q_\mathcal{{K}}
              }
         }   
\end{proof}
