...
%
The theory of $\mathcal{M},\mathcal{N}$-adhesivity carries on with such extended rules, see e.g~\cite{xxx}.
...
Summing up, we proved that EGGs are objects of an $\mathcal{M},\mathcal{N}$-adhesive category $\GEqTGs$ 
and that optimisation steps are obtained via DPO rules (possibly with negative application conditions) whose 
arrows are in $\mathcal{M}$ and via a match that is in $\mathcal{N}$. This fact allows for exploiting all the properties 
...
and equipping them with an enrichment over semilattices, the resulting formalism being reminiscent of hierarchical hypergraphs.
