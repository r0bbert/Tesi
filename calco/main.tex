...
%
and to define it we introduce some terminology.  Let  $\X$ be a category. A subclass $\mathcal{A}$ of $\mor(\X)$ is
...
\begin{theorem}
	...
	\begin{enumerate}
		...
		\item if $\Y$ is a full subcategory of $\X$ closed in it under pullbacks and $\mathcal{M}$-pushouts, then $\Y$ is (strictly) $\mathcal{N}$-adhesive for every class of arrows $\mathcal{N}$ of $\Y$ contained in $\mathcal{M}$ and  stable under pullbacks and pushout, containing all isomorphisms and closed under composition and decomposition.
	\end{enumerate} 
\end{theorem}
...
\begin{restatable}{lemma}{kpp}\label{lemma:kern_pairs_pres_pullbacks}
	Let $f\colon X \to Y$ and $g\colon Z \to W$ be two arrows admitting kernel pairs and suppose that the solid part of the three squares below is given. Then there exists a a unique arrow $k_h: K_f \to K_g$ completing them.
...
As a final step, we explore the link between regular epis and kernel pairs.
...
\begin{restatable}{lemma}{epicol}\label{lemma:nat_trans_reg_epi_canonical_arrow_reg_epi}
    Let $F,G\colon \D\rightrightarrows \X$ be two diagrams, and suppose that $\X$ has all colimits of shape $\D$. Let $(X, \{x_d\}_{d \in \D})$ and $(Y, \{y_d\}_{d\in \S})$ be the colimits of $F$ and $G$, respectively.  If $\phi\colon  F \to G$ is a natural transformation whose components are regular epi, then the arrow induced by $\phi$ from $X$ to $Y$ is a regular epi too.
\end{restatable}
...
In this section we briefly recall the notion \emph{$\X$-hypergraph}. It is necessary to have a monad $(-)^\star$ playing a role analogous to the usual \emph{Kleene star}
 $(-)^\star\colon \Set\to \Set$, also known as \emph{list monad}, sending a set to the free monoid on it \cite{sakarovitch2009elements,Wadler95}. We recall some of its proprieties.
...
\begin{definition}Let $\catname{H}$ be the category isuch that
	\begin{itemize}
...
of trees with a lattice on hom-sets. Instead of going through 
