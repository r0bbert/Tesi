...
\begin{corollary}\label{cor:mono1}
	An arrow $(h_E, h_V, h_C): \mathcal{G \to H}$ in $\EqHyp$ is a mono if and only if $(h_E, h_V)$ is a mono in $\hyp$.
\end{corollary}
...
\begin{lemma}\label{lem:epo}
	Suppose that the square on the right is a pushout in $\EqHyp$ with $m$ a regular mono. If $\mathcal{G}_1$, $\mathcal{G}_2$ and $\mathcal{G}_3$ are e-hypergraphs, then $\mathcal{P}$ is an e-hypergraph too.
	following pushout square in $\EqHyp$.
...
\begin{corollary}\label{cor:rege}
		An arrow $h: \mathcal{G \to H}$  is a regular mono in $\egg$ if and only if $I(h)$ is a regular mono in $\EqHyp$. 
\end{corollary}

Let now $\pbe$ be the class of all regular monos in $\egg$ which are sent to $\pbc$ by $I$. By definition and \Cref{lem:pbmono} such class is closed under composition, decomposition, contains all isomorphisms and it is stable under pullbacks. Using \Cref{cor:rege} we can also deduce that it is stable under pushouts. Then \Cref{thm:slice-functors,cor:eqade} yield the following.
...
\begin{proof}\label{proof:pbmono}
	Closure under composition and decomposition follows from \Cref{lem:pb1}, while stability under pushouts is the first point of \Cref{prop:kerset}. We are left with stability under pullbacks. 
...
\end{align*}}\hfill \parbox{4cm}{	\xymatrix{
