...
We have now all the ingredient necessary to study the adhesivity properties of $\EqHyp$.  As a first step we need to introduce a class $\mathcal{M}$ of monos.
...
%	Let now $z: \graph{G}'_2 \to \graph H = (E, V, Q, s, t, q)$, $w: \graph G_3^1 \to \graph H$ be two morphisms such that $z \circ h' = w \circ k'$, and let $\phi:B'_4 \to V$ be the arrow induced by $z_V$ and $w_V$. We want to construct the arrow $v: C'_4 \to Q$ in the diagram below.
	...
	\begin{example} 
		We can consider the following four graphs with equivalence:
		\[\begin{split}
		\mathcal{G}_1&=(\emptyset, \emptyset, \emptyset, \id{\emptyset^\star}, \id{\emptyset^\star}, \id{\emptyset})\\
		\mathcal{G}_3&=(\emptyset, 2, 2, ?_{2^\star}, ?_{2^\star}, \id{2})
		\end{split} \qquad \begin{split}
		\mathcal{G}_2&=(\emptyset, 2, 1, ?_{2^\star}, ?_{2^\star}, !_2)\\
		\qquad \mathcal{G}_4&=(\emptyset, 4, 3, ?_{4^\star}, ?_{4^\star}, \pi)
		\end{split}\]
		where $\pi\colon 4\to 3$ is defined as
		\[0\mapsto 0 \quad 1\mapsto 0 \quad 2\mapsto 1 \quad 3\mapsto 2 \]
		
		Now, we have morphisms $h\colon \mathcal{G}_1\to \mathcal{G}_2$, $k\colon \mathcal{G}_1\to \mathcal{G}_3$, $z\colon \mathcal{G}_2\to \mathcal{G}_4$, $p\colon \mathcal{G}_3\to \mathcal{G}_4$ defined as:
		\[
		h:=(\id{\emptyset}, ?_2, ?_1)\quad k:=(\id{\emptyset}, ?_2, ?_2)
		\quad 
		z:=(\id{\emptyset}, j, z_Q)\quad p:=(\id{\emptyset}, j', p_Q)
		\]
		where 
		\[\begin{matrix}
			j\colon 2\to 4 & \begin{matrix}
			0\mapsto 0\\
			1  \mapsto 1
			\end{matrix} & z_Q\colon 1\to 3 &0\mapsto 0 & j'\colon 2\to 4 &\begin{matrix}0 \mapsto 2\\
			1 \mapsto 3
			\end{matrix} & p_Q\colon 2\to 3  &\begin{matrix}
		0\mapsto 1\\
		1\mapsto 2
			\end{matrix}
		\end{matrix} \]
		
		Notice that  $\pi\circ j=z_Q$ and$\pi\circ j'=p_Q$  Thus in this way $(\mathcal{G}_4, \{z, p\})$ becomes a pushout for $h$ along $k$.
...
And we can conclude that $b\circ \circ h_1\circ l^{-1}_1=t_2$ because $n$ is a mono. The thesis now follows from the fact that  also $n'$ is mono.
		...
		\item  By \Cref{lem:nodim} we know that there exists $K_0$, $K_1$, $K_2$ and $K_3$ fitting in the following four pullback diagrams and such that $\{K_d, \{j_i\}_{i=0}^3\}$ is a coproduct.
...
\[\xymatrix{K_0 \ar@/^.3cm/[drr]^{p_0} \ar@{.>}[dr]^{\phi_0} \ar@/_.3cm/[ddr]_{q_0} &&& K_3 \ar[r]^{p_3} \ar@{.>}[dr]^{\phi_3} \ar[d]_{q_3}& E' \ar[dr]^{\iota'} \\ &K_b \ar[r]^{\pi^1_b} \ar[d]_{\pi^2_b}& B' \ar[d]^{b} & E' \ar[dr]_{\iota'}&K_c \ar[r]^{\pi^1_c} \ar[d]_{\pi^2_c} & C'  \ar[d]^{c}\\ & B' \ar[r]_{b} & B && C' \ar[r]_{c} & C }\]

Notice, moreover, that 
\[(\pi^1_d, \pi^2_d)\circ k_{g'}\circ \phi_3=(g'\times g')\circ (\pi^1_c, \pi^2_c)\circ \phi_3=(g'\times g')\circ (\iota'\circ p_3, \iota'\circ q_3)\]
and so $k_{g'}\circ \phi_3=j_3$.
...
\end{align*}
			...
			Since the bottom face of the original cube is a pullback (again by \Cref{prop:regmono}), we conclude that there exist arrows $k_1\colon  K_1\to A$ and $k_2\colon K_2\to A$ such that
 ...
 are both empty. In particular, we can also deduce that $(K_d, \{j_0, j_3\})$ is a coproduct.
 ...
 \[\pi^1_{d}\circ k_{n'}\circ \phi_0=n'\circ \pi^1_b\circ \phi_0=n'\circ p_0=\pi^1_d\circ j_0 \qquad \pi^2_{d}\circ k_{n'}\circ \phi_0=n'\circ \pi^2_b\circ \phi_0=n'\circ p_0=\pi^2_d\circ j_0\]
 
 Thus $k_{n'}\circ \phi_0=j_0$ and we have
 \[(p_0, q_0)\circ \psi_0\circ \phi_0=(\pi^1_b, \pi^2_b)\circ \phi_0=(p_0, q_0)= \quad  j_0\circ \psi_0\circ \phi_0 = k_{n'}\circ \phi_0=j_0\]
We can conclude that $\phi_0$ is an isomorphism and that $k_{n'}$ is a coproduct injection.


 Furthermore, by definition of complement, $(C', \{\iota', m'\})$ is a coproduct. Applying again \Cref{lem:nodim} we can decompose $K_c$ in four parts, given by the pullbacks below:
 \[\xymatrix@C=45pt{H_0 \ar@{>->}[r]^{x_0}  \ar[d]_{(y_0, z_0)}& K_c   \ar[d]^{(\pi^1_c, \pi^2_c)}& H_1 \ar@{>->}[r]^{x_1}  \ar[d]_{(y_1, z_1)} & K_c \ar[d]^{(\pi^1_c, \pi^2_c)} \\
 	A'\times A' \ar@{>->}[r]_-{m'\times m'} & C'\times C' & A'\times E' \ar@{>->}[r]_-{m'\times  \iota'} & C'\times C'}\]
 \[ \xymatrix@C=45pt{H_2 \ar@{>->}[r]^{x_2}  \ar[d]_{(y_2, z_2)}& K_c   \ar[d]^{(\pi^1_c, \pi^2_c)}& H_3 \ar@{>->}[r]^{x_3}  \ar[d]_{(y_3, z_3)} & K_c \ar[d]^{(\pi^1_c, \pi^2_c)} \\
...
(\pi^1_d, \pi^2_d)\circ k_{g'}\circ x_1&=(g'\times g')\circ (\pi^1_c, \pi^2_c)\circ x_1\\=&(g'\times g')\circ (m'\times \iota')\circ (y_1, z_1)\\=&((g'\circ m') \times (g'\circ \iota'))\circ (y_1, z_1)\\=&((n'\circ f') \times (g'\circ \iota'))\circ (y_1, z_1)\\=&(n'\times  (g'\circ \iota'))\circ (f'\circ y_1, z_1)
 ...
 \end{split} \]
\normalsize
 thus we have arrows $h_1\colon H_1\to K_1$ and $h_2\colon H_2\to K_2$, showing that $H_1$ and $H_2$ are empty. On the other hand, let us consider the two diagrams below.

 \[ \xymatrix@C=45pt{ H_3 \ar@{.>}[d]^{\alpha_3} \ar[r]^{x_3} \ar@/_.4cm/[dd]_{(y_3, z_3)} & K_c \ar[d]_{k_{g'}}& K_3 \ar@{.>}[d]^{\beta_3} \ar@/^.3cm$ $/[dr]^{\phi_3} \ar@/_.4cm/[dd]_{(p_3, q_3)}\\K_3 \ar@{>->}[r]^{j_3}  \ar[d]^{(p_3, q_3)} & K_d \ar[d]^{(\pi^1_d, \pi^2_d)} &  H_3 \ar@{>->}[r]^{x_3}  \ar[d]^{(y_3, z_3)} & K_c \ar[d]^{(\pi^1_c, \pi^2_c)}  \\
...
Thus we get the dotted arrows $\alpha_3$ and $\beta_3$. Now, they are one the inverse of the other. Indeed on the one hand we have:
\begin{align*}
j_3\circ \alpha_3\circ \beta_3&=k_{g'}\circ x_3\circ \beta_3=k_{g'} \circ \phi_3=\phi_3 \\
(p_3, q_3)\circ \alpha_3\circ \beta_3&=(y_3, z_3)\circ \beta_3=(p_3, q_3) 
\end{align*}
 On the other, notice that
 ...
 Now, we have already proved that $(K_d, \{k'_n, j_3\})$ is a coproduct, thus there is a unique arrow $\theta\colon K_d\to Z$ such that 
 \[\theta \circ k'_n=\gamma \qquad \theta \circ j_3=\delta \circ \phi_3 \]
 ...
 On the other hand, 
 \[(\pi^1_d, \pi^2_d)\circ k_{g'}\circ x_0=(g'\times g')\circ (\pi^1_c, \pi^2_c)\circ x_0=(g'\times g')\times (m'\times m')\circ (y_0, x_0)=(n'\circ n')\ (f')\]
