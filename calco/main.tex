...
namely (labelled) hypergraphs and term graphs, and we provide a
functorial characterisation, which allows for proving their adhesivity properties.
This is further expended in Section~\ref{hypereq} for describing hypergraphs 
...
thus subsuming EGGs.
%
All the proofs have been moved to the appendix. For the sake of completeness, 
...
\begin{restatable}{lemma}{kpp}\label{lemma:kern_pairs_pres_pullbacks}
	Let $f\colon X \to Y$ and $g\colon Z \to W$ be two arrows admitting kernel pairs and suppose that the solid part of the three squares below is given. Then there exists a a unique arrow $k_h: K_f \to K_g$ completing them.     Moreover, if the leftmost is a pullback, then also the other two are so.
%
