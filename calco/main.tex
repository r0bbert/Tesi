...
\noindent
\parbox{11cm}{\hspace{15pt}Let now $\mathcal{T}$ be the class of morphism in $\EqTG_{\Sigma}$ whose image through $I_\eq$ is in $\pbc_\Sigma$ and whose image through $S_\Sigma$ is a regular mono in $\tg$.  Unwrapping the definition and using \Cref{lem:reg}, we have that a morphism $(h_E, h_V, h_Q)\colon (\mathcal{G},l)\to (\mathcal{H}, l')$ is in $\mathcal{T}$ if and only if all of its components are injections and the square on the right is a pullback.} \hfill \parbox{2cm}{\xymatrix{V_{\mathcal{G}}\ar@{>->}[r]^{h_V} \ar@{>>}[d]_{q_{\mathcal{H}}}& V_{\mathcal{H}} \ar@{>>}[d]^{q_{\mathcal{H}}}\\ Q_{\mathcal{G}} \ar@{>->}[r]^{h_Q}& Q_{\mathcal{H}}} }
...
We start introducing the notion of \emph{e-hypergraphs}, hypergraphs equipped with an equivalence relation which is closed under the source and target arrow. This means that whenever the relations identifies the source of an hyperedge, it identifies the targets too.
...
\subsection{Labelling e-hypergraphs}

\begin{remark}\label[remark]{sig_egg}
...
\begin{definition}\label[definintion]{def:GEqTGs}
	Given a signature $\Sigma$, we define the category $\GEqTGs$ as the full subcategory of $\egg/R(\mathcal{G}^\Sigma)$ in which objects are arrows $l: \mathcal{G} \to R(\mathcal{G}^\Sigma)$ where $t_\mathcal{G}$ is mono. The incusion functor is denoted $I_{\GEqTGs} : \GEqTGs \to \egg/R(\mathcal{G}^\Sigma)$.
\end{definition}
