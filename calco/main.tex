...
%
The theory of $\mathcal{M}$-adhesivity~\cite{xxx} extend the core framework, still ensuring that if the arrows 
...
transformation can still be obtained~\cite{xxx}. 
...
\begin{corollary}\label{cor:ereg}
	If an arrow $h: \mathcal{G \to H}$ in $\egg$ is a regular mono in $\egg$ then $I(h)$ is so in $\hyp$. 
\end{corollary}
...
of the $\mathcal{M}$-adhesive framework when discussing about the optimisation process.
...
on $\mathcal{M}$-adhesive categories, can be exploited in the context of EGGs implementation. 
