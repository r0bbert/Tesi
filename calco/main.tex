...
%\todo{'sta cragna che segue è falsa come una moneta da 3 euro: cancellare}
%\noindent
%\parbox{7.5cm}{
...
\begin{example}\label[example]{exa_3}
	Let $\Sigma = (O_\Sigma, \ari_\Sigma)$ be the algebraic signature with $O_\Sigma =  \mathbb{N} \uplus A \uplus \{*, /\}$, 
	let $n$ stands for any natural number and $a$ for any element in $A$, both sets interpreted as sets of constants, and
	%so that $\ari_\Sigma(n)=\ari_\Sigma(a) = 0$, 
	$\ari_\Sigma(*) = \ari_\Sigma(/) = 2$. Then the hypergraph $\mathcal{G}^\Sigma$ can be depicted as
	\begin{center}
\begin{tikzpicture}
        \begin{pgfonlayer}{nodelayer}
...
\begin{example}\label[example]{lab_2}
	...
	$f: \mathcal{G} \to \mathcal{G}^\Sigma$ that is characterised by the image of the edges. 
	If $f(h(1) = a$, $f(h_2) = 2$, $f(h_3) = *$, and $f(h_3) = /$, we represent it visually 
...
underlying term graph is \emph{acyclic}, and in this case if $\mathcal{G}$ has no empty nodes, then the unique arrow 
...
Note that if the starting term graph is acyclic, so is also the arriving one.
...
that is in $\mathcal{N}$ allows for exploiting all the 
...
Despite their wide acceptance, we are not aware of any recent attempt to provide an algebraic characterisation of EGGs,
the only exception being in~\cite{ghica}. We leave for future works also a in-depth comparison among the two proposals,
which appear to be related yet nevertheless quite different.
%
The key intuition is always to use string diagrams to represents (acyclic) term graphs or, in other words,
props over a symmetric monoidal category. In our case we adopted a more down-to-Earth approach by equipping
concretely the nodes of a term graph with an equivalence relation, thus extending and generalising~\cite{concur2006} 
in the contemporary jargon of adhesive categories.
The route chosen in~\cite{ghica} is to consider as models symmetric monoidal categories enriched over semilattices,
so that the 

Simplifying, the key is to equip categories 
...
\begin{proof}
		Let $E'\subseteq C'$ be the complement of the image of $m'$ and $\iota\colon E'\mto B'$ be the corresponding inclusion. By \Cref{lem:po_set} we know that $(D', \{n', g'\circ \iota\})$ is a coproduct. Thus by \Cref{lem:nodim} we also know that there exists $K_0$, $K_1$, $K_2$ and $K_3$ fitting in the following four pullback diagrams and such that $\{K_d, \{j_i\}_{i=0}^3\}$ is a coproduct.
...
\[\xymatrix{K_0 \ar@/^.3cm/[drr]^{p_0} \ar@{.>}[dr]^{\phi_0} \ar@/_.3cm/[ddr]_{q_0} &&& K_3 \ar[r]^{p_3} \ar@{.>}[dr]^{\phi_3} \ar[d]_{q_3}& E' \ar[dr]^{\iota} \\ &K_b \ar[r]^{\pi^1_d} \ar[d]_{\pi^2_d}& B' \ar[d]^{b} & E' \ar[dr]_{\iota}&K_c \ar[r]^{\pi^1_d} \ar[d]_{\pi^2_d} & C'  \ar[d]^{c}\\ & B' \ar[r]_{b} & B && C' \ar[r]_{c} & C }\]
	...
	Now, we also know that $(C', \{m', \iota\})$ is a coproduct, thus we can apply again \Cref{lem:nodim} to $K_c$ to conclude that $(K_c, \{k_i\}_{i=0}^4)$ is a coproduct, where $k_i\colon K^i_c\to K_c$ is defined by the following pullbacks.
