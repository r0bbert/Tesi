...
Let $F\colon \D \to \EqHyp$ be a diagram, and let $(E_d, V_d, Q_d, s_d, t_d, q_d)$ be the image of an object $d$. Then the following hold true:
\begin{enumerate}
...
\begin{proof}
	...
	and that suppose in the first diagram below,  all the vertical faces are pullbacks, the bottom face is a pushout and $h$ is regular mono. By \Cref{prop:eqhyp_complete} the same is true for the other two cubes, by \Cref{cor:mono2}, $h_E$ and $h_V$ are mono and so the top faces of these cubes are pushouts.
	\[
