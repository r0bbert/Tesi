...
\begin{frame}{Why EGGs?}
     ...
     \pause \medskip However, rewriting does not remove the left-hand side of the rule.
	...
	\xymatrix{        
		...
		\begin{tikzpicture}[baseline=(nil.center)]\begin{pgfonlayer}{nodelayer}
			...
			\end{pgfonlayer}\begin{pgfonlayer}{background}
				\draw[color=white] (-1.5, 0.2) rectangle (1.5, -0.2);
                \node [below=2cm, align=flush center,text width=5cm] at (v1)
                {
...
If a category is $\mathcal{M}$-adhesive, many properties for DPO rewriting can be proved once and for all: Church-Rosser, termination, concurrent semantics...

\pause \medskip
Also, $\mathcal{M}$-adhesivity is preserved by common categorical constructions:\pause
\begin{itemize}
	\item products\pause 
...
\end{center}
\pause
In the slice, ``labels'' must be preserved.
\begin{center}
	\begin{tikzpicture}
		\begin{pgfonlayer}{nodelayer}
...
A \emph{(labelled) hypergraph with equivalence} is a (labelled) hypergraph equipped with a surjection $q\colon V\eto Q$ from the set of nodes. \pause 
...
General equivalence relations are not enough. We want to restrict to \emph{congruences}: whenever the relation identifies the source of two hyperedges, it identifies their targets too. 
...
\begin{frame}{(labelled) e-hypergraphs}
	Let $\pbe$ and $\pbe_\Sigma$ be the restriction of $\pbc$ and $\pbc_\Sigma$  to the suitable subcategories of e-hypergraphs. 
...
\begin{frame}{The same key example}
