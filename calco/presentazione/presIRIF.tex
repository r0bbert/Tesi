...
\begin{theorem}
$\tg$ is RegMono$_{\Sigma}$-adhesive (aka quasiadhesive).
\end{theorem}
...
\begin{frame}{Labelling term graphs (was: Why labelling)}
\begin{center}
        \begin{tikzpicture}
                \begin{pgfonlayer}{nodelayer}
                        \node[style=none] (lab1) at (-1.8, 0.8) {$h_1$};
...
We get categories $\EqHyp$, $\EqHyps$ and  $\EqTG_{\Sigma}$ asking that the nodes part of a morphism induces a morphism between the quotients. 
