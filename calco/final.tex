...
%
The theory of $\mathcal{M}$-adhesivity~\cite{azzi2019essence,heindel2009category} extends the core framework, still ensuring that if the arrows 
...
can still be obtained~\cite{ehrig2012,ehrig2014adhesive}. 
...
$\mathcal{M}$-adhesivity is well-behaved with respect to  the construction of (co-)slice and functor categories \cite{mac2013categories}, as well with respect to subcategories, as shown by the following theorems~\cite[Thm.~4.15]{ehrig2006fundamentals}, \cite[Prop.~3.5]{lack2005adhesive} and  \cite[Thm.~2.12]{CastelnovoGM24}.
...
Let us look at the picture below. It represent an hypergraph $\mathcal{G}$ with sets $V_\mathcal{G} = \{v_1, v_2, v_3, v_4\}$ and $E_\mathcal{G} = \{h_1, h_2, h_3, h_4\}$ of nodes 
...
$\hyp_{\Sigma}$, has a forgetful functor $U_{\Sigma}\colon \hyp_{\Sigma}\to \X$ which sends $(h,k)\colon \mathcal{H}\to \mathcal{G}^{\Sigma}$ to $U_{\hyp}(\mathcal{H}$). Now, $U_{\hyp}(\mathcal{G}^{\Sigma})=1$ thus, for every set $X$ we can define $\Delta_{\Sigma}(X)\colon \Delta_{\hyp}(X)\to \mathcal{G}^{\Sigma}$  as $(?_{O_\Sigma}, !_{X})$. It is straightforward to see that in this way we get a left adjoint to $U_\Sigma$.
...
\begin{lemma}\label[lemma]{prop:push} Consider three term graphs $(l_0, !_{V_\mathcal{G}})\colon \mathcal{G}\to \mathcal{G}^{\Sigma}$, $(l_1, !_{V_\mathcal{H}})\colon \mathcal{H}\to \mathcal{G}^{\Sigma}$ and $(l_2, !_{V_\mathcal{K}})\colon \mathcal{K}\to \mathcal{G}^{\Sigma}$. Given $(f_1, g_1)\colon (l_0, !_{V_\mathcal{G}})\to (l_1, !_{V_\mathcal{H}})$, $(f_2, g_2)\colon (l_0, !_{V_\mathcal{G}})\to (l_2, !_{V_\mathcal{K}})$, if $(f_1, g_1)$ is a regular mono, then its pushout  $(p, !_{V_{\mathcal{P}}})\colon \mathcal{P}\to \mathcal{G}^{\Sigma}$ in $\hyp_{\Sigma}$ along $(f_2, g_2)$ is a term graph too.
\end{lemma}
...
\begin{example}
	Consider the hypergraph of $\mathcal{G}$ \Cref{exa_2} and consider as quotient the identity $\id{V_\mathcal{G}}\colon V_\mathcal{G}\colon \to\mathcal{G}$. Then the kernel pair of $\id{V^\star_\mathcal{G}} \circ s$ concide with the kernel pair of $s$ which is empty. Thus  $\mathcal{G}$ is, trivially, an e-hypergraph
\end{example}
...
\parbox{11.5cm}{\begin{restatable}{lemma}{epo}\label{lem:epo}
	Suppose that the square on the right is a pushout in $\EqHyp$ with $m$ an arrow in $\pbc$. If $\mathcal{G}_1$, $\mathcal{G}_2$ and $\mathcal{G}_3$ are e-hypergraphs, then $\mathcal{P}$ is so too.
...
\begin{definition} We say that an object $(\mathcal{H}, l)$ of $\EqHyp_\Sigma$ is a \emph{labelled e-hypergraph} if $\mathcal{H}$ is an e-hypergraph. We define the category $\egg_\Sigma$ as the full subcategory of $\EqHyp_\Sigma$ given by labelled e-hypergraphs. We denote by $J_\Sigma$ the corresponding inclusion functor .
\end{definition}
...
\begin{remark}\label{rem:algegg}
	...
	equipped with the two projections.  On the other hand $\id{\mathbb{N}} \circ \gamma_1 \circ \pi_1$ and $\id{\mathbb{N}}\circ \gamma_1 \circ \pi_2$ are both the function $O_\Sigma \to \mathbb{N}$ constant in $1$.
\end{remark}
...
\label{rewriting}
The previous sections have shown how term graphs with equivalence,
...
\end{center}
%
Admittedly, the third one is illegal, i.e. it is an object of $\EqTGs$ but not one of $\eg$:
%$\GEqTGs$: 
...
In general, for term graphs without equivalence, a maximally and a minimally shared representation
exists, and for our toy example are the right-most and the left-most diagram above, respectively.
These are standard results in term graph rewriting, see e.g.~\cite{AriolaKP00}, 
and they can easily seen as the final and the initial objects of a suitable comma category 
$\mathcal{G}  \downarrow \tg$, respectively.
%
The same properties carries on 
%when restricting to the full subcategory of acyclic term graphs.
%
for EGGSs, even if the minimally shared representation is now 
the middle diagram.
%
...
However, this is easily forbidden adopting rules \emph{with negative application conditions}, given as the usual span 
...
be factorised through $n$~\cite{HabelHT96}. For EGGs and rules $L \leftarrow L \rightarrow R$, $n$ is simply the right-hand side itself.
%
The theory of $\mathcal{M}$-adhesivity carries on with such rules, see~\cite{ehrig2012,ehrig2014adhesive}.
...
and fact allows for exploiting the properties 
...
%
In both cases, the key intuition is  to use string diagrams to represents term graphs. We adopted a
...
as they hold for standard graph transformation.
...
\noindent
	...
	%
	It suffices to build the cokernel pair of the three arrows, taking the pushout of $h_E$, $h_V$ and $h_Q$ along itself, obtaining the three diagrams below, which by \Cref{prop:regmono} are also pullbacks.
	\[     \xymatrix@R=15pt{
...
\end{lemma}
\begin{proof}
	Suppose that $U_L\circ F$ and $U_R\circ F$ have respectively colimiting cocones  $\left(A, \{a_D\}_{D\in \D}\right)$ and $\left(B, \{b_D\}_{D\in \D}\right)$. By hypothesis $\left(L(A), \{L\left(a_D\right)\}_{D\in \D}\right)$ is colimiting for $L\circ U_L\circ F$. Now, if we define $F(D):=(A_D, B_D, f_D)$
...
\end{proposition}
\begin{proof} For an object $B\in \B$ we can define $\Delta(B)$ as $(0, B, ?_{B})$, for $0$ is an initial object in $\A$ and $?_{R(B)}$ is the unique arrow $L(0)\to R(B)$. Consider $\id{B}\colon B\to U_R(\Delta(B))$ be the identity,
	...
	since $L$ preserves initial objects, the square aside commutes. Thus $(h,k)$ is the unique morphism $\Delta(B)\to (A, B', f)$ such that $U_R(h,k)=k$. }\hfill 
