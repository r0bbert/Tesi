...
If only the left-hand side belongs to $\mathcal{M}$, the current theory is still under development, as witnessed e.g.
by~\cite{BaldanC0G24}.
%
...
\begin{proposition}
	...
	Let $\X$ be an (strict) $\mathcal{M}$-adhesive category. Then the following holds
	\begin{enumerate}
...
\begin{restatable}{lemma}{kpp}\label{lemma:kern_pairs_pres_pullbacks}
	...
	\[\xymatrix@R=15pt{X \ar[r]^{h}\ar[d]_{f}& Z \ar[d]^{g} & K_f \ar@{.>}[r]^{k_h} \ar[d]_{\pi^1_f}& K_g \ar[d]^{\pi^1_g} & K_f  \ar@{.>}[r]^{k_h} \ar[d]_{\pi^2_f}& K_g \ar[d]^{\pi^2_g}& K_f  \ar@{.>}[r]^{k_h} \ar[d]_{(\pi^1_f,\pi^2_f)} & K_g \ar[d]^{(\pi^1_g, \pi^2_g)}\\ Y \ar[r]_t& W & X \ar[r]_{h} & Z & X\ar[r]_{h} &Z & X\times Z \ar[r]_{h\times h}& X\times Z}\]
		...
		\begin{enumerate}
			...
			\item if the leftmost square is a pullback, then also the central two are so;
			\item if $h$ is mono and the leftmost square is a pullback then the rightmost is a pullback too.
		\end{enumerate} 
\end{restatable}
...
Let $F\colon \D \to \EqHyp$ be a diagram and $(E_D, V_D, Q_D, s_D, t_D, q_D)$ the image of an object $D$. Then the following hold
\begin{enumerate}
...
Let $(\mathcal{H}, l)$ be an object of $\EqHyp_\Sigma$, then, since $T$ has a right adjoint $R$ by \Cref{prop:forghyp}, $l\colon T(\mathcal{H})\to \mathcal{G}^\Sigma$ corresponds to the arrow $(l, !_{Q_{\mathcal{H}}})\colon \mathcal{H}\to R(\mathcal{G}^\Sigma)$. It is immediate to see that this correspondence extends to an equivalence with the slice over $R(\mathcal{G}^\Sigma)$.
...
\parbox{11.5cm}{\begin{restatable}{lemma}{epo}\label{lem:epo}
	Suppose that the square on the right is a pushout in $\EqHyp$ with $m$ a mono in $\pbc$. If $\mathcal{G}_1$, $\mathcal{G}_2$ and $\mathcal{G}_3$ are e-hypergraphs, then $\mathcal{P}$ is so too.
...
and they can be easily seen as the final and the initial objects of a suitable comma category 
...
%
for EGGs, even if the minimally shared representation is now 
the middle diagram.
%
